...
_GLOBAL_HOOKS_FILE = Path("~/.muse/hooks.json").expanduser()
...
def _load_global_hooks_config() -> dict[str, Any]:
    """Load hooks from ~/.muse/hooks.json."""
...
def save_hooks_config(hooks: dict[str, Any]) -> Path:
    ...
    path.write_text(json.dumps(existing, option=orjson.OPT_INDENT_2) + "\n", encoding="utf-8")
    logger.debug("Saved hooks config to %s", path)
    return path


def save_global_hooks_config(hooks: dict[str, Any]) -> Path:
    """Persist hooks config to ~/.muse/hooks.json.

    Returns the path written.
    """
    path = Path(_GLOBAL_HOOKS_FILE)
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(hooks, option=orjson.OPT_INDENT_2) + "\n", encoding="utf-8")
