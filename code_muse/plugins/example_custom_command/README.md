...
> For **built-in commands**, see the built-in command files in `code_muse/command_line/`.

## Overview

This plugin demonstrates how to create custom commands using Muse's callback system.

**Important**: Custom commands use `register_callback()`, NOT `@register_command`.

## Command Types in Muse

### 1. Built-in Commands (Core Functionality)
- Use `@register_command` decorator
- Located in `code_muse/command_line/core_commands.py`, `session_commands.py`, `config_commands.py`
...
```
code_muse/plugins/example_custom_command/
...
```bash
mkdir -p code_muse/plugins/my_plugin
touch code_muse/plugins/my_plugin/__init__.py
touch code_muse/plugins/my_plugin/register_callbacks.py
```

### Step 2: Implement Callbacks

```python
# code_muse/plugins/my_plugin/register_callbacks.py
...
```bash
# Restart Muse to load the plugin
muse
...
- **Don't modify global state**: Use Muse's config system
...
from code_muse.plugins.my_plugin.register_callbacks import _handle_custom_command
...
- `code_muse/callbacks.py` - Callback system implementation
- `code_muse/command_line/command_handler.py` - Command dispatcher
- `code_muse/command_line/core_commands.py` - Example built-in commands
- `code_muse/command_line/command_registry.py` - Registry system

## Questions?

If you're unsure whether to create a custom command or a built-in command:

- **Is it core Muse functionality?** → Use `@register_command` (built-in)
