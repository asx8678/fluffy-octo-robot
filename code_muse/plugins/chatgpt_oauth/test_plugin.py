...
def test_config_paths():
    ...
    # XDG paths use "code_muse" (without dot) in ~/.local/share or ~/.config
    assert "code_muse" in str(token_path)

    config_dir = config.get_config_dir()
    # Default is ~/.muse; XDG paths only used when XDG env vars are set
    assert config_dir.name in ("code_muse", ".muse")
...
@patch("code_muse.plugins.chatgpt_oauth.utils.httpx.post")
...
@patch("code_muse.plugins.chatgpt_oauth.utils.httpx.get")
