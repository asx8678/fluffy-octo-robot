...
def list_directory(path: str = None) -> tuple[list[str], list[str]]:
    ...
    dirs = [e for e in entries if (Path(path) / e).is_dir()]
    files = [e for e in entries if not (Path(path) / e).is_dir()]
    return dirs, files
...
def safe_input(prompt_text: str = "") -> str:
    ...
    try:
        from code_muse.messaging.message_queue import get_global_queue
