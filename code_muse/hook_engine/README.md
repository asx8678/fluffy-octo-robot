...
```python
from code_muse.hook_engine import HookEngine, EventData
...
Hooks can be written using **either** the provider's tool name **or** code_muse's
internal tool name — the matcher treats them as equivalent.

### Claude Code → code_muse

| Claude Code (`matcher`) | code_muse internal | Notes |
|-------------------------|---------------------|-------|
