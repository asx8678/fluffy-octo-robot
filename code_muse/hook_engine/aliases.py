"""
Tool name alias registry — maps each AI provider's tool names to code_muse's
...
Each provider block defines a dict[str, str] mapping:
    "<Provider tool name>" -> "<code_muse internal tool name>"
...
# Source: `claude code serve` → tools/list  (verified against v2.1.52)
# ---------------------------------------------------------------------------
CLAUDE_CODE_ALIASES: dict[str, str] = {
    ...
    # NOTE: the tools below have no direct code_muse equivalent yet.
...
# ---------------------------------------------------------------------------
# Gemini  (Google)
# TODO: populate once Gemini tool names are verified.
# Run `gemini` serve (or equivalent) and inspect the tools/list response,
# then add entries following the same pattern as CLAUDE_CODE_ALIASES above.
# ---------------------------------------------------------------------------
GEMINI_ALIASES: dict[str, str] = {
    # Add Gemini → code_muse tool mappings here
}


# ---------------------------------------------------------------------------
# Codex  (OpenAI)
# TODO: populate once Codex tool names are verified.
# Run the Codex server, inspect tools/list, and add entries here.
# ---------------------------------------------------------------------------
CODEX_ALIASES: dict[str, str] = {
    # Add Codex → code_muse tool mappings here
}
...
SWARM_ALIASES: dict[str, str] = {
    # Add Swarm → code_muse tool mappings here
}
...
def resolve_internal_name(provider_tool_name: str) -> str | None:
    """
    Return the code_muse internal tool name for a given provider tool name,
