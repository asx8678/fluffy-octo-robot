...
> **Golden rule:** nearly all new functionality should be a **plugin** under `code_muse/plugins/`
> that hooks into core via `code_muse/callbacks.py`. Don't edit `code_muse/command_line/`.

## How Plugins Work

Create `code_muse/plugins/my_feature/register_callbacks.py` (builtin) or `~/.muse/plugins/my_feature/register_callbacks.py` (user):
...
| `version_check` | Version check | `(*args, **kwargs) -> None` |
...
Full list + rarely-used hooks: see `code_muse/callbacks.py` source.
...
7. **Never add a Claude co-author commit line**
