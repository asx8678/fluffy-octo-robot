...
This document describes the safety mechanisms built into Muse. It is meant to help users understand what the tool can read, write, execute, and trust.
...
- Sessions are saved as **JSON** files with a schema wrapper (`code_muse.session.v1`).
...
- If an existing token file has overly broad permissions, Muse warns and repairs the mode automatically.
...
### Built-in plugins
- Built-in plugins under `code_muse/plugins/` are loaded automatically.

### User plugins
- User plugins in `~/.muse/plugins/` are loaded but should not shadow built-in imports.
...
- `MUSE_DISABLE_RETRY_TRANSPORT` disables retry transport **without** disabling TLS verification.
- To explicitly disable TLS verification, use `MUSE_DISABLE_TLS_VERIFY`.
...
| Command | Purpose |
|---------|---------|
| `/safety` or `/status` | Show current safety posture (yolo, shell safety, workspace, hook trust, UC) |
