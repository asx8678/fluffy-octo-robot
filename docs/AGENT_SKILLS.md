...
Agent Skills are reusable, modular capabilities that extend Muse's functionality. Think of them as specialized training packets you can dynamically load when needed. [Run]
...
Skills are installed by placing them in designated skill directories. Muse scans these directories at startup to discover available skills.

### Default Skill Directories

By default, Muse looks for skills in:

1. **`~/.muse/skills/`** - User-level skills (global)
...
2. **Download or clone a skill** into the directory:
   ...
   cd ~/.muse/skills
   git clone https://github.com/example/muse-docker.git docker
...
3. **Verify the skill** has a `SKILL.md` file:
   ...
   ```bash
   ls ~/.muse/skills/my-custom-skill/SKILL.md
   ```
...
Muse provides an interactive TUI (Text User Interface) for managing skills.
...
When skills are enabled, Muse automatically injects available skills into the system prompt:
...
1. **Discovery** → Muse scans skill directories at startup
...
1. Place your skill in `~/.muse/skills/`
...
Agent Skills can be configured through Muse's configuration system.
...
| `skill_directories` | JSON list | `["~/.muse/skills", "./skills"]` | Directories to scan for skills |
...
/set skill_directories = "[\"/path/to/skills\", \"~/.muse/skills\"]"
...
Settings are stored in `~/.muse/muse.cfg`:

```ini
[muse]
skills_enabled = true
skill_directories = ["/home/user/.muse/skills", "./skills"]
...
⚠️ **Important:** Skills execute with the same permissions as Muse. Follow these security best practices:
...
- Any files Muse has access to

**Recommendation:** Run Muse with minimal necessary permissions.
...
3. For core skills functionality issues, report to Muse
