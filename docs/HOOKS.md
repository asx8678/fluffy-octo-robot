...
Hooks let you intercept and control every tool call the agent makes — before it runs, after it runs, or both. They are compatible with the Claude Code `.claude/settings.json` format, so any hook script that works in Claude Code works in Muse out of the box.
...
Restart Muse — any attempt to run a `git` command will be blocked cleanly.
...
**Muse tool name mapping:**

| Claude Code Name | Muse Internal Name |
|-----------------|--------------------------|
...
2. `~/.muse/hooks.json` — global user hooks

---

## Implementation Notes

The hook engine lives in `code_muse/hook_engine/` and is a self-contained library with no dependency on the rest of Muse.

Hooks are injected at the `pydantic-ai` `ToolManager._call_tool()` level via a patch in `code_muse/pydantic_patches.py`, so they fire on every tool call regardless of which agent or model is in use.
