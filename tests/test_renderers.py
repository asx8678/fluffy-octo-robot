...
from code_muse.messaging.message_queue import MessageQueue, MessageType, UIMessage
from code_muse.messaging.renderers import (
    InteractiveRenderer,
    MessageRenderer,
...
class TestInteractiveRendererMessageTypes:
    ...
    async def test_render_agent_response_markdown_fallback(self):
        ...
        with patch(
            "code_muse.messaging.renderers.Markdown",
...
class TestSynchronousInteractiveRendererHumanInput:
    ...
    def test_human_input_with_valid_prompt_id(self):
        ...
        with (
            patch("builtins.input", return_value="Claude"),
            patch(
                "code_muse.messaging.message_queue.provide_prompt_response"
            ) as mock_provide,
        ):
            renderer._render_message(msg)
    ...
    def test_human_input_displays_prompt(self):
        ...
        with patch("builtins.input", return_value="user input"):
            with patch("code_muse.messaging.message_queue.provide_prompt_response"):
                renderer._render_message(msg)
