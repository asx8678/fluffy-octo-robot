...
Muse prompt without any agent processing.
"""
...
class TestFormatBanner:
    ...
    def test_banner_uses_config_color(self):
        ...
        with patch(
            "code_muse.command_line.shell_passthrough.get_banner_color",
...
class TestExecuteShellPassthrough:
    ...
    @patch("code_muse.command_line.shell_passthrough.subprocess.run")
    @patch("code_muse.command_line.shell_passthrough._get_console")
    ...
    @patch("code_muse.command_line.shell_passthrough.subprocess.run")
    @patch("code_muse.command_line.shell_passthrough._get_console")
    @patch("code_muse.command_line.shell_passthrough.os.getcwd", return_value="/tmp")
...
class TestInitialCommandPassthrough:
    ...
    Regression tests for the bug where `muse "!ls"` or
    `muse -p "!ls"` would send the command to the AI agent
    ...
    def test_interactive_mode_initial_command_calls_passthrough(
        ...
        with (
            ...
            patch(
                "code_muse.cli_runner.runner.get_current_agent",
                return_value=mock_agent,
            ),
            patch(
                "code_muse.cli_runner.runner.run_prompt_with_attachments",
    ...
    def test_execute_single_prompt_calls_passthrough(self, mock_run, mock_console):
        ...
        with (
            patch("code_muse.cli_runner.runner.get_current_agent") as mock_agent,
            patch(
                "code_muse.cli_runner.runner.run_prompt_with_attachments"
            ) as mock_run_prompt,
        ):
    ...
    def test_execute_single_prompt_normal_prompt_skips_passthrough(
        ...
        with (
            ...
            patch("code_muse.messaging.message_queue.emit_info"),
        ):
