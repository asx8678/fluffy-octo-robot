...
from code_muse.cli_runner.runner import run_prompt_with_attachments
...
def test_parse_prompt_leaves_urls_untouched() -> None:
    url = "https://example.com/Muse-inspiration.png"
...
async def test_run_prompt_with_attachments_passes_binary(tmp_path: Path) -> None:
    ...
    with (
        patch("code_muse.cli_runner.runner.emit_warning") as mock_warn,
        patch("code_muse.cli_runner.runner.emit_system_message") as mock_system,
    ):
    ...
    _, kwargs = fake_agent.run.await_args
...
async def test_run_prompt_with_attachments_uses_spinner(tmp_path: Path) -> None:
    ...
    fake_agent = AsyncMock()
    fake_agent.run.return_value = AsyncMock()

    dummy_console = object()

    with (
        patch("code_muse.messaging.spinner.ConsoleSpinner") as mock_spinner,
        patch("code_muse.cli_runner.runner.emit_system_message"),
        patch("code_muse.cli_runner.runner.emit_warning"),
    ):
...
def test_parse_prompt_handles_long_paragraph_paste() -> None:
    ...
    long_text = (
        "File /Users/testuser/.muse-venv/lib/python3.13/site-packages/prompt_toolkit/layout/processors.py, "
