...
def _base_main_patches():
    ...
    return {
        "code_muse.cli_runner.find_available_port": MagicMock(return_value=8090),
        ...
        "code_muse.cli_runner.default_version_mismatch_behavior": MagicMock(),
        ...
        "code_muse.cli_runner.plugins": MagicMock(),
        "code_muse.config.load_api_keys_to_environment": MagicMock(),
    }
...
def _interactive_patches():
    return {
        ...
        "code_muse.cli_runner.loop.COMMAND_HISTORY_FILE": "/tmp/test_history",
        "code_muse.command_line.onboarding_wizard.should_show_onboarding": MagicMock(
            return_value=False
        ),
        "code_muse.config.auto_save_session_if_enabled": MagicMock(),
    }
...
async def _run_interactive(
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(
            patch(
                "code_muse.command_line.prompt_toolkit_completion.get_input_with_combined_completion",
        ...
        stack.enter_context(
            patch(
                "code_muse.command_line.prompt_toolkit_completion.get_prompt_with_active_model",
                return_value="> ",
            )
        )
        stack.enter_context(
            patch(
                "code_muse.cli_runner.loop.get_current_agent",
                return_value=agent,
            )
        )
        if extra_patches:
        ...
        from code_muse.cli_runner import interactive_mode
...
class TestMain:
    ...
    async def _run_main(self, argv, extra_patches=None, base_overrides=None):
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch(
                    "code_muse.messaging.SynchronousInteractiveRenderer",
                    return_value=_mock_renderer(),
                )
            )
            stack.enter_context(
                patch(
                    "code_muse.messaging.RichConsoleRenderer",
                    return_value=_mock_renderer(),
                )
            )
            stack.enter_context(
                patch("code_muse.messaging.get_global_queue", return_value=MagicMock())
            )
            stack.enter_context(
                patch("code_muse.messaging.get_message_bus", return_value=MagicMock())
            )
            _apply_patches(stack, patches)
            if extra_patches:
                _apply_patches(stack, extra_patches)
            from code_muse.cli_runner import main
    ...
    async def test_prompt_mode(self):
        ...
        await self._run_main(
            ["muse", "-p", "hello world"],
            extra_patches={"code_muse.cli_runner.execute_single_prompt": mock_exec},
        )
    ...
    async def test_no_available_port(self):
        await self._run_main(
            ["muse", "-p", "test"],
            base_overrides={
                "code_muse.cli_runner.find_available_port": MagicMock(
                    return_value=None
                ),
            },
        )
    ...
    async def test_keymap_error(self):
        from code_muse.keymap import KeymapError
    ...
    async def test_model_invalid(self):
        ...
        with pytest.raises(SystemExit):
            await self._run_main(
                ["muse", "-m", "bad-model", "-p", "hi"],
                extra_patches={
                    "code_muse.config.set_model_name": MagicMock(),
                    "code_muse.config._validate_model_exists": MagicMock(
                        return_value=False
                    ),
                    "code_muse.model_factory.ModelFactory": mock_mf,
                },
            )
    ...
    async def test_agent_valid(self):
        ...
        await self._run_main(
            ...
            extra_patches={
                "code_muse.cli_runner.execute_single_prompt": AsyncMock(),
                "code_muse.cli_runner.get_available_agents": MagicMock(
                    return_value={"muse": {}}
                ),
                "code_muse.cli_runner.set_current_agent": mock_set,
            },
        )
        mock_set.assert_called_with("muse")

    @pytest.mark.anyio
    async def test_agent_invalid(self):
        with pytest.raises(SystemExit):
            await self._run_main(
                ["muse", "-a", "bad-agent", "-p", "hi"],
                extra_patches={
                    "code_muse.cli_runner.get_available_agents": MagicMock(
                        return_value={"muse": {}}
                    ),
    ...
    async def test_version_check_with_callbacks(self):
        ...
        patches = _base_main_patches()
        patches["code_muse.cli_runner.callbacks"] = cb_mock
        with ExitStack() as stack:
            stack.enter_context(
                patch.dict(os.environ, {"NO_VERSION_UPDATE": ""}, clear=False)
            )
            stack.enter_context(patch("sys.argv", ["muse", "-p", "hi"]))
            stack.enter_context(
                patch(
                    "code_muse.messaging.SynchronousInteractiveRenderer",
                    return_value=_mock_renderer(),
            ...
            stack.enter_context(
                patch(
                    "code_muse.cli_runner.execute_single_prompt",
    ...
    async def test_version_check_no_callbacks(self):
        ...
        patches = _base_main_patches()
        patches["code_muse.cli_runner.callbacks"] = MagicMock(
            on_startup=AsyncMock(),
            on_shutdown=AsyncMock(),
            on_version_check=AsyncMock(),
            get_callbacks=MagicMock(return_value=[]),
...
class TestInteractiveMode:
    ...
    async def test_autosave_load_non_tty(self):
        ...
        await _run_interactive(
            ...
            extra_patches={
                ...
                "code_muse.session_storage.restore_autosave_interactively": AsyncMock(),
            },
        )
    ...
    async def test_autosave_load_tty_cancelled(self):
        ...
        with patch.dict(os.environ, {"MUSE_NO_TUI": ""}, clear=False):
            await _run_interactive(
                _mock_renderer(),
                _interactive_patches(),
                fake_input,
                extra_patches={
                    ...
                    "sys.stdin": mock_stdin,
                    "sys.stdout": mock_stdout,
                    "code_muse.command_line.autosave_menu.interactive_autosave_picker": AsyncMock(
                        return_value=None
                    ),
                },
            )
        ...
        with patch.dict(os.environ, {"MUSE_NO_TUI": ""}, clear=False):
            await _run_interactive(
                ...
                extra_patches={
                    ...
                    "code_muse.session_storage.load_session": MagicMock(
                        return_value=[MagicMock()]
                    ),
                    "code_muse.config.set_current_autosave_from_session_name": MagicMock(),
                    "code_muse.command_line.autosave_menu.display_resumed_history": MagicMock(),
                    "code_muse.cli_runner.loop.get_current_agent": MagicMock(
                        return_value=agent
                    ),
                },
    ...
    async def test_wiggum_loop(self):
        ...
        await _run_interactive(
            ...
            extra_patches={
                ...
                "code_muse.cli_runner.loop.get_wiggum_prompt": MagicMock(
                    return_value="repeat"
                ),
                "code_muse.cli_runner.loop.increment_wiggum_count": MagicMock(
                    return_value=1
                ),
                "code_muse.cli_runner.loop.stop_wiggum": MagicMock(),
            },
        )
    ...
    async def test_onboarding_chatgpt(self):
        ...
        patches["code_muse.command_line.onboarding_wizard.should_show_onboarding"] = (
        ...
        await _run_interactive(
            ...
            extra_patches={
                ...
                "code_muse.command_line.onboarding_wizard.run_onboarding_wizard": AsyncMock(
                    return_value="chatgpt"
                ),
                "code_muse.plugins.chatgpt_oauth.oauth_flow.run_oauth_flow": MagicMock(),
                "code_muse.config.set_model_name": MagicMock(),
            },
        )
    ...
    async def test_onboarding_claude(self):
        ...
        await _run_interactive(
            ...
            extra_patches={
                ...
                "code_muse.plugins.claude_code_oauth.register_callbacks._perform_authentication": MagicMock(),
                "code_muse.config.set_model_name": MagicMock(),
            },
        )
...
class TestInteractiveModeEdgeCases:
    ...
    async def test_autosave_load_no_tui_env(self):
        """Line 656: MUSE_NO_TUI=1 forces non-interactive picker."""
        call_count = 0
        ...
        with patch.dict(os.environ, {"MUSE_NO_TUI": "1"}, clear=False):
            await _run_interactive(
                _mock_renderer(),
                _interactive_patches(),
                fake_input,
...
class TestMainUvxAndEdgeCases:
    ...
    async def test_uvx_alternate_cancel_key(self):
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch(
                    "code_muse.uvx_detection.should_use_alternate_cancel_key",
                    return_value=True,
                )
            )
            stack.enter_context(
                patch("code_muse.terminal_utils.disable_windows_ctrl_c")
            )
            stack.enter_context(
                patch("code_muse.terminal_utils.set_keep_ctrl_c_disabled")
            )
    ...
    async def test_initial_command_awaiting_input(self):
        ...
        await _run_interactive(
            ...
            extra_patches={
                ...
                "code_muse.tools.command_runner.is_awaiting_user_input": MagicMock(
                    return_value=True
                ),
            },
        )
...
class TestRemainingEdgeCases:
    ...
    async def test_execute_single_prompt_success_path(self):
        ...
        from code_muse.cli_runner import execute_single_prompt
        ...
        with ExitStack() as stack:
            stack.enter_context(patch("code_muse.cli_runner.loop.get_current_agent"))
            stack.enter_context(
                patch(
...
class TestImportErrorFallbacks:
    ...
    async def test_is_awaiting_user_input_import_error(self):
        ...
        def fake_import(name, *args, **kwargs):
            if name == "code_muse.tools.command_runner":
...
class TestMainEntryAdditional:
    ...
    def test_keyboard_interrupt_stderr_output(self, mock_run):
        ...
        with ExitStack() as stack:
            stack.enter_context(patch("code_muse.cli_runner.reset_unix_terminal"))
            stack.enter_context(patch("code_muse.tools.command_runner.shutdown"))
            stack.enter_context(patch("code_muse.callbacks._shutdown_executor"))
            result = main_entry()
            assert result == 130
