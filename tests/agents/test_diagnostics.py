"""Tests for ``code_muse.agents._diagnostics`` (issue #296).
...
from code_muse.agents._diagnostics import (
...
class TestCheapPath:
    def test_plain_runtime_error_emits_summary_and_skips_deep_block(self):
        with patch("code_muse.agents._diagnostics.emit_info") as mock_emit:
            with patch("code_muse.agents._diagnostics.log_error") as mock_log:
    ...
    def test_log_failure_does_not_propagate(self):
        with (
            patch("code_muse.agents._diagnostics.emit_info"),
            patch(
                "code_muse.agents._diagnostics.log_error",
    ...
    def test_emit_info_failure_does_not_propagate(self):
        with (
            patch(
                "code_muse.agents._diagnostics.emit_info",
                side_effect=RuntimeError("display dead"),
            ),
            patch("code_muse.agents._diagnostics.log_error"),
        ):
...
class TestDeepPath:
    def test_trigger_phrase_enables_deep_diagnostics(self):
        with patch("code_muse.agents._diagnostics.emit_info") as mock_emit:
            with patch("code_muse.agents._diagnostics.log_error"):
                emit_exception_diagnostics(
