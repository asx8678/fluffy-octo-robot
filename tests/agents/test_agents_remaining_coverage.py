"""Tests targeting remaining uncovered lines in code_muse/agents/ (non-base_agent)."""
...
def test_helios_agent():
    from code_muse.agents.agent_helios import HeliosAgent
...
def test_planning_agent():
    from code_muse.agents.agent_planning import PlanningAgent
...
def test_muse_prompt_allows_callback_additions():
    """Verify that on_load_prompt() additions are included in assembled instructions."""
    from code_muse.agents._builder import _assemble_instructions
    from code_muse.agents.agent_muse import MuseAgent

    agent = MuseAgent()
    with patch("code_muse.callbacks.on_load_prompt") as mock_load:
        mock_load.return_value = ["extra_prompt_text"]
        instructions = _assemble_instructions(agent, "test-model")
        assert "extra_prompt_text" in instructions
...
def test_creator_agent_get_system_prompt_with_uc_tools():
    ...
    from code_muse.agents.agent_creator_agent import AgentCreatorAgent
...
def test_creator_validate_agent_json_valid():
    ...
    with patch(
        "code_muse.agents.agent_creator_agent.get_available_tool_names",
...
def test_creator_get_agent_file_path():
    ...
    agent = AgentCreatorAgent()
    with patch(
        "code_muse.agents.agent_creator_agent.get_user_agents_directory",
...
def test_agent_manager_is_process_alive_dead_process():
    ...
    from code_muse.agents.agent_manager import _is_process_alive
...
def test_agent_manager_discover_agents_error():
    ...
    from code_muse.agents.agent_manager import _discover_agents
...
def test_next_clone_index():
    ...
    from code_muse.agents.agent_manager import _next_clone_index
...
def test_clone_agent_failure():
    ...
    from code_muse.agents.agent_manager import clone_agent

    with patch("code_muse.agents.agent_manager.emit_warning"):
...
def test_fire_stream_event_import_error():
    ...
    from code_muse.agents.event_stream_handler import _fire_stream_event

    with patch("code_muse.callbacks.on_stream_event", side_effect=ImportError):
...
def test_fire_stream_event_exception():
    ...
    with patch("code_muse.callbacks.on_stream_event", side_effect=Exception("boom")):
        _fire_stream_event("test", {})  # Should not raise
...
def _make_json_agent(tmp_path, config):
    ...
    from code_muse.agents.json_agent import JSONAgent
...
def test_json_agent_uc_import_error(tmp_path):
    ...
    with patch.dict(
        "sys.modules", {"code_muse.plugins.universal_constructor.registry": None}
    ):
        tools = agent.get_available_tools()
...
def test_subagent_fire_callback_no_loop():
    ...
    from code_muse.agents.subagent_stream_handler import _fire_callback
...
def test_subagent_stream_handler_module():
    ...
    mod = importlib.import_module("code_muse.agents.subagent_stream_handler")
