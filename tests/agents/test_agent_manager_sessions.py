...
class TestSessionDataPersistence:
    ...
    def test_save_session_data_creates_directory(self, temp_session_dir):
        ...
        with patch(
            ...
            _save_session_data({"session_123": "muse"})
    ...
    def test_save_session_data_writes_json(self, temp_session_dir):
        ...
        with patch(
            ...
            assert loaded["session_123"] == "muse"
...
class TestDeadSessionCleanup:
    ...
    def test_cleanup_removes_dead_sessions(self):
        ...
        sessions = {
            "session_12345": "muse",  # Will be dead
            "session_99999": "planning-agent",  # Will be dead
        }

        with patch("code_muse.agents.agent_manager._is_process_alive") as mock_alive:
    ...
    def test_cleanup_mixed_sessions(self):
        ...
        with patch(
            "code_muse.agents.agent_manager._is_process_alive", side_effect=is_alive
        ):
            result = _cleanup_dead_sessions(sessions)
    ...
    def test_cleanup_handles_invalid_pid_conversion(self):
        ...
        sessions = {
            "session_invalid": "muse",  # Can't convert to int
            "session_12345": "planning-agent",
        }

        with patch("code_muse.agents.agent_manager._is_process_alive"):
            result = _cleanup_dead_sessions(sessions)
...
class TestSessionCaching:
    ...
    def test_ensure_session_cache_loaded_loads_once(self):
        ...
        with patch("code_muse.agents.agent_manager._load_session_data") as mock_load:
            mock_load.return_value = {"session_123": "muse"}
    ...
    def test_ensure_session_cache_loaded_updates_cache(self):
        ...
        with patch("code_muse.agents.agent_manager._load_session_data") as mock_load:
            ...
            assert test_sessions == am._SESSION_AGENTS_CACHE
