...
class TestAgentManagerErrors:
    ...
    def test_load_agent_invalid_name(self, mock_discover):
        ...
        with patch("code_muse.agents.agent_manager._AGENT_REGISTRY", {}):
            with pytest.raises(
    ...
    def test_load_agent_fallback_behavior(self, mock_discover):
        """Test load_agent fallback to muse when requested agent not found."""
        mock_discover.return_value = None

        # Mock registry with only muse available
        mock_agent_class = MagicMock(spec=BaseAgent)
        mock_agent_class.return_value.name = "muse"

        with patch(
            "code_muse.agents.agent_manager._AGENT_REGISTRY",
            {"muse": mock_agent_class},
        ):
            # Should fallback to muse instead of raising error
    ...
    def test_load_agent_corrupted_registry_entry(self, mock_discover):
        ...
        with patch(
            "code_muse.agents.agent_manager._AGENT_REGISTRY", {"bad-agent": 12345}
        ):
    ...
    @patch("code_muse.agents.agent_manager._CURRENT_AGENT", None)
    ...
    def test_set_current_agent_nonexistent(self, mock_save, mock_discover):
        ...
        with (
            ...
            patch("code_muse.agents.agent_manager.get_current_agent") as mock_current,
        ):
    ...
    def test_load_agent_case_sensitivity(self, mock_discover):
        ...
        with patch(
            ...
            with pytest.raises(ValueError, match="Agent 'muse' not found"):
                load_agent("muse")

            # Exact case should work
            result = load_agent("Muse")
            assert result is not None
            mock_agent_class.assert_called_once()
