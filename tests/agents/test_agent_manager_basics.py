...
from code_muse.agents.agent_manager import (
    _AGENT_REGISTRY,
    _discover_agents,
    _load_session_data,
    _save_session_data,
...
class TestAgentManagerBasics:
    ...
    def test_discover_agents_skips_internal_modules(
        ...
        mock_iter_modules.return_value = [
            ("code_muse.agents", "_internal", True),
            ("code_muse.agents", "base_agent", True),
            ("code_muse.agents", "json_agent", True),
            ("code_muse.agents", "agent_manager", True),
            ("code_muse.agents", "valid_agent", True),
        ]
    ...
    def test_discover_agents_handles_import_errors(
        ...
        mock_iter_modules.return_value = [("code_muse.agents", "broken_agent", False)]

        # Create a side effect that only fails for the broken agent module
        def mock_import_side_effect(module_name):
            if module_name == "code_muse.agents.broken_agent":
    ...
    def test_load_agent_not_found(self):
        ...
        with patch("code_muse.agents.agent_manager._discover_agents"):
            _AGENT_REGISTRY.clear()

            # The actual behavior is that it tries to fallback to muse
    ...
    def test_load_agent_fallback_to_code_muse(
        ...
        """Test fallback to muse agent when requested agent not found."""

        # Setup registry with only muse
        class MuseAgent(MockAgent):
            def __init__(self):
                super().__init__()
                self._name = "muse"

        mock_iter_modules.return_value = [("code_muse.agents", "code_muse", True)]

        mock_module = MagicMock()
        mock_module.MuseAgent = MuseAgent

        def mock_import_side_effect(module_name):
            if "code_muse" in module_name:
                return mock_module
            return MagicMock()
        ...
        # Should fallback to muse
        assert agent is not None
        assert agent.name == "muse"

    def test_refresh_agents(self):
        """Test refreshing agent discovery."""
        with patch("code_muse.agents.agent_manager._discover_agents") as mock_discover:
            refresh_agents()
    ...
    def test_session_data_persistence(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with (
                patch(
                    "code_muse.agents.agent_manager._get_session_file_path",
                    return_value=session_file,
                ),
                patch(
                    "code_muse.agents.agent_manager._is_process_alive",
    ...
    @patch("code_muse.agents.agent_manager._load_session_data")
    @patch("code_muse.agents.agent_manager.load_agent")
    @patch("code_muse.agents.agent_manager.get_current_agent_name")
