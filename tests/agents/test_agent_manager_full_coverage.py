"""Full coverage tests for code_muse/agents/agent_manager.py.
...
class TestGetCurrentAgent:
    ...
    @patch("code_muse.config.get_default_agent", return_value="default-agent")
...
class TestGetAgentDescriptionsFiltering:
    ...
    @patch("code_muse.config.get_pack_agents_enabled", return_value=True)
    @patch("code_muse.config.get_universal_constructor_enabled", return_value=False)
    def test_uc_disabled_filters_uc_agents(self, mock_uc, mock_pack):
        from code_muse.config import UC_AGENT_NAMES
...
class TestDeleteCloneUnlinkError:
    ...
    @patch("code_muse.agents.agent_manager._discover_agents")
    @patch("code_muse.config.get_user_agents_directory")
...
class TestDiscoverAgents:
    ...
    @patch("code_muse.agents.agent_manager.discover_json_agents", return_value={})
    @patch("code_muse.agents.agent_manager.emit_warning")
...
class TestGetAvailableAgents:
    ...
    @patch("code_muse.agents.agent_manager._discover_agents")
    @patch("code_muse.config.get_pack_agents_enabled", return_value=False)
    @patch("code_muse.config.get_universal_constructor_enabled", return_value=False)
    def test_filters_pack_agents(self, mock_uc, mock_pack, mock_discover):
        ...
        with (
            patch("code_muse.config.PACK_AGENT_NAMES", {"pack-leader"}),
            patch("code_muse.config.UC_AGENT_NAMES", set()),
        ):
    ...
    def test_filters_uc_agents(self, mock_uc, mock_pack, mock_discover):
        ...
        with (
            patch("code_muse.config.PACK_AGENT_NAMES", set()),
            patch("code_muse.config.UC_AGENT_NAMES", {"uc-agent"}),
        ):
            agents = am.get_available_agents()
    ...
    @patch("code_muse.agents.agent_manager._discover_agents")
    @patch("code_muse.config.get_pack_agents_enabled", return_value=True)
    @patch("code_muse.config.get_universal_constructor_enabled", return_value=True)
...
class TestCloneHelpers:
    ...
    def test_filter_available_tools(self):
        with patch(
            "code_muse.tools.get_available_tool_names", return_value=["tool1", "tool3"]
        ):
...
class TestCloneAgent:
    ...
    @patch(
        "code_muse.agents.agent_manager._filter_available_tools",
        side_effect=lambda x: x,
    )
    @patch("code_muse.agents.agent_manager.emit_success")
    ...
    @patch("code_muse.agents.agent_manager._discover_agents")
    @patch("code_muse.config.get_user_agents_directory")
    @patch("code_muse.config.get_agent_pinned_model", return_value="pinned-model")
    @patch(
        "code_muse.agents.agent_manager._filter_available_tools",
        side_effect=lambda x: x,
    )
...
class TestDeleteCloneAgent:
    ...
    @patch("code_muse.agents.agent_manager._discover_agents")
    @patch(
        "code_muse.agents.agent_manager.get_current_agent_name",
    ...
    @patch("code_muse.agents.agent_manager._discover_agents")
    @patch(
        "code_muse.agents.agent_manager.get_current_agent_name", return_value="other"
    )
    @patch("code_muse.agents.agent_manager.emit_warning")
...
class TestSetCurrentAgent:
    ...
    @patch("code_muse.agents.agent_manager._discover_agents")
    @patch("code_muse.agents.agent_manager.load_agent")
    @patch("code_muse.agents.agent_manager.on_agent_reload")
    @patch("code_muse.agents.agent_manager._save_session_data")
    def test_set_with_history_restore(
        ...
        assert new_agent == am._CURRENT_AGENT
...
class TestLoadAgent:
    ...
    @patch("code_muse.agents.agent_manager._discover_agents")
    def test_fallback_to_muse(self, mock_discover):
        from code_muse.agents.agent_muse import MuseAgent

        am._AGENT_REGISTRY["muse"] = MuseAgent
        agent = am.load_agent("nonexistent")
        assert agent.name == "muse"
