...
class TestFireCallback:
    ...
    def test_fire_callback_no_event_loop(self):
        ...
        with patch(
            "code_muse.agents.subagent_stream_handler.asyncio.get_running_loop",
    ...
    def test_fire_callback_import_error(self):
        ...
        with (
            ...
            patch.dict("sys.modules", {"code_muse.callbacks": None}),
        ):
            # Should not raise
            _fire_callback("part_start", {"index": 0}, "session-123")
...
class TestHandleEvent:
    ...
    async def test_handle_event_no_session_id(self, mock_manager):
        ...
        with patch(
            "code_muse.agents.subagent_stream_handler._fire_callback"
        ) as mock_fire:
            await _handle_event(
                event=event,
...
class TestSubagentStreamHandler:
    ...
    async def test_stream_handler_with_session_id(self, mock_ctx, mock_manager):
        ...
        with (
            patch(
                "code_muse.messaging.subagent_console.SubAgentConsoleManager.get_instance",
                return_value=mock_manager,
            ),
            patch("code_muse.messaging.get_session_context"),
        ):
            with patch("code_muse.agents.subagent_stream_handler._fire_callback"):
    ...
    async def test_stream_handler_fallback_to_context(self, mock_ctx, mock_manager):
        ...
        with (
            ...
            patch(
                "code_muse.messaging.get_session_context",
                return_value="context-session",
            ),
            patch("code_muse.agents.subagent_stream_handler._fire_callback"),
        ):
            await subagent_stream_handler(
                mock_ctx,
                self._create_async_events(events),
