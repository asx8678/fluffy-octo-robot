...
class TestProjectAgentIntegration:
    ...
    def test_discover_project_agent_via_cwd(self, tmp_path, monkeypatch):
        """Test that changing to a directory with .muse discovers agents."""
        # Create project structure: project/.muse/agents/
        project_dir = tmp_path / "myproject"
        project_dir.mkdir()
        agents_dir = project_dir / ".muse" / "agents"
        ...
        with patch(
            "code_muse.config.get_user_agents_directory",
    ...
    def test_no_project_agent_when_outside_project(self, tmp_path, monkeypatch):
        ...
        # Create different directory without .muse
    ...
    def test_project_agent_overrides_user_agent_via_cwd(self, tmp_path, monkeypatch):
        ...
        project_dir = tmp_path / "myproject"
        project_dir.mkdir()
        project_agents_dir = project_dir / ".muse" / "agents"
    ...
    def test_nested_project_discovery(self, tmp_path, monkeypatch):
        """Test that .muse is found from nested subdirectories."""
        # Create project/.muse/agents/
        project_dir = tmp_path / "myproject"
        project_dir.mkdir()
        agents_dir = project_dir / ".muse" / "agents"
        agents_dir.mkdir(parents=True)
        ...
        # the agent from nested directories (would need to walk up to find .muse)
