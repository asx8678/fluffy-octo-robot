...
from code_muse.callbacks import _callbacks, clear_callbacks, register_callback
...
async def test_success_only_when_result_returned(
    ...
    result = await _runtime.run(agent, "hello")
...
async def test_run_model_exception_reports_failure(
    run_end_calls: list[dict[str, Any]],
) -> None:
