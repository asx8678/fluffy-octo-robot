...
from code_muse.plugins.chatgpt_oauth.oauth_flow import (
    AuthBundle,
    TokenData,
    _CallbackHandler,
    _OAuthServer,
...
def mock_context():
    """Mock OAuth context."""
    from code_muse.plugins.chatgpt_oauth.utils import OAuthContext
...
class TestOAuthServer:
    ...
    def test_exchange_code_success(self, mock_post, mock_context, mock_tokens_data):
        ...
        # Mock JWT parsing
        with patch(
            "code_muse.plugins.chatgpt_oauth.oauth_flow.parse_jwt_claims"
        ) as mock_parse:
...
class TestCallbackHandler:
    ...
    def callback_handler(self, mock_server):
        ...
        with patch(
            "code_muse.plugins.chatgpt_oauth.oauth_flow._CallbackHandler.handle_one_request"
        ):
        ...
        with patch.object(callback_handler, "_send_failure") as mock_failure:
            with patch.object(callback_handler, "_shutdown") as mock_shutdown:
                ...
                mock_failure.assert_called_once_with(
                    404, "Callback endpoint not found for the muse parade."
                )
    ...
    @patch("code_muse.plugins.chatgpt_oauth.oauth_flow.save_tokens")
    ...
    def test_do_get_token_save_failure(self, mock_save_tokens, callback_handler):
        ...
        with patch.object(callback_handler, "_send_failure") as mock_failure:
            with patch.object(callback_handler, "_shutdown") as mock_shutdown:
                ...
                mock_failure.assert_called_once_with(
                    500, "Unable to persist auth file — a muse probably chewed it."
                )
                mock_shutdown.assert_called_once()
    ...
    def test_log_message_verbose_mode(self, callback_handler):
        ...
        with patch(
            "code_muse.plugins.chatgpt_oauth.oauth_flow.BaseHTTPRequestHandler.log_message"
        ) as mock_log:
...
class TestRunOAuthFlow:
    ...
    @patch("code_muse.plugins.chatgpt_oauth.oauth_flow.load_stored_tokens")
    @patch("code_muse.plugins.chatgpt_oauth.oauth_flow._OAuthServer")
    @patch("code_muse.plugins.chatgpt_oauth.oauth_flow.emit_warning")
    @patch("code_muse.plugins.chatgpt_oauth.oauth_flow.emit_info")
    ...
    @patch("code_muse.plugins.chatgpt_oauth.oauth_flow.load_stored_tokens")
    @patch("code_muse.plugins.chatgpt_oauth.oauth_flow._OAuthServer")
    @patch("code_muse.plugins.chatgpt_oauth.oauth_flow.emit_error")
    @patch("code_muse.plugins.chatgpt_oauth.oauth_flow.emit_info")
    ...
    @patch("code_muse.plugins.chatgpt_oauth.oauth_flow._OAuthServer")
    @patch("code_muse.plugins.chatgpt_oauth.oauth_flow.emit_info")
    @patch("code_muse.plugins.chatgpt_oauth.oauth_flow.emit_success")
    def test_successful_oauth_flow(
        ...
        with patch(
            "code_muse.plugins.chatgpt_oauth.oauth_flow.load_stored_tokens"
        ) as mock_reload:
            mock_reload.return_value = mock_tokens

            with patch(
                "code_muse.plugins.chatgpt_oauth.oauth_flow.add_models_to_extra_config"
            ) as mock_add:
                mock_add.return_value = True

                with (
                    patch(
                        "code_muse.plugins.chatgpt_oauth.utils.fetch_chatgpt_models",
    ...
    def test_authentication_timeout(
        ...
        with (
            patch(
                "code_muse.plugins.chatgpt_oauth.oauth_flow.CHATGPT_OAUTH_CONFIG",
                original_config,
            ),
            patch("threading.Thread"),
            patch("time.sleep"),
...
class TestShutdownAfterDelay:
    ...
    def test_shutdown_after_delay_calls_shutdown(self):
        ...
        with (
            patch(
                "code_muse.plugins.chatgpt_oauth.oauth_flow.threading.Thread",
                side_effect=capture_thread,
            ),
            patch("code_muse.plugins.chatgpt_oauth.oauth_flow.time.sleep"),
        ):
