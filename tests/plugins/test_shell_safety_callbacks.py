...
from code_muse.plugins.shell_safety.command_cache import CachedAssessment
from code_muse.plugins.shell_safety.register_callbacks import (
    register,
...
class TestShellSafetyCallbackOAuthBypass:
    ...
    async def test_callback_skips_for_oauth_model_anthropic(self):
        ...
        with patch(
            "code_muse.plugins.shell_safety.register_callbacks.get_global_model_name",
...
class TestShellSafetyCallbackYoloModeBypass:
    ...
    async def test_callback_skips_when_yolo_mode_false(self):
        ...
        with (
            ...
            patch(
                "code_muse.plugins.shell_safety.register_callbacks.get_yolo_mode",
                return_value=False,
            ),
        ):
            result = await shell_safety_callback(
...
class TestShellSafetyCallbackCacheHit:
    ...
    async def test_cached_assessment_blocked_high_risk(self):
        ...
        with (
            ...
            patch(
                "code_muse.plugins.shell_safety.register_callbacks.get_safety_permission_level",
                return_value="medium",
            ),
            patch(
                "code_muse.plugins.shell_safety.register_callbacks.get_cached_assessment",
                return_value=cached,
            ),
            patch(
                "code_muse.plugins.shell_safety.register_callbacks.emit_info"
            ) as mock_emit,
        ):
            result = await shell_safety_callback(
                context=None, command="rm -rf /", cwd=None, timeout=60
    ...
    async def test_cached_assessment_with_none_risk(self):
        ...
        with (
            ...
            patch("code_muse.plugins.shell_safety.register_callbacks.emit_info"),
        ):
            result = await shell_safety_callback(
...
class TestShellSafetyCallbackCacheMiss:
    ...
    async def test_llm_assessment_blocked_high_risk(self):
        ...
        with (
            ...
            patch(
                "code_muse.plugins.shell_safety.register_callbacks.cache_assessment"
            ) as mock_cache,
            patch.dict(
                "sys.modules",
                {
                    "code_muse.plugins.shell_safety.agent_shell_safety": MagicMock(
...
class TestRegisterCallback:
    ...
    def test_register_calls_register_callback(self):
        ...
        with patch(
            "code_muse.plugins.shell_safety.register_callbacks.register_callback"
        ) as mock_register:
            register()
            mock_register.assert_called_once_with(
                "run_shell_command", shell_safety_callback, priority=50
            )
    ...
    def test_module_auto_registers_on_import(self):
        ...
        with patch(
            ...
            import code_muse.plugins.shell_safety.register_callbacks as module
