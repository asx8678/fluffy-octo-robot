...
class TestCopilotAuth:
    ...
    def _make_auth(self, oauth_token: str = "ghp_fake123", host: str = "github.com"):
        ...
        from code_muse.plugins.copilot_auth.utils import get_valid_session_token
    ...
    @patch("code_muse.plugins.copilot_auth.utils.get_valid_session_token")
    ...
    def test_auth_flow_refreshes_on_every_call(self, mock_get_token):
        ...
        for _i, expected in enumerate(["token_1", "token_2", "token_3"], 1):
            request = httpx.Request(
                "POST", "https://api.githubcopilot.com/chat/completions"
            )
            flow = auth.auth_flow(request)
...
class TestCreateCopilotModel:
    ...
    MODULE = "code_muse.plugins.copilot_auth.register_callbacks"

    @patch("code_muse.plugins.copilot_auth.register_callbacks.get_token_for_host")
    def test_returns_none_when_no_token(self, mock_get_token):
        """Should return None and warn when no Copilot token is available."""
        mock_get_token.return_value = None

        from code_muse.plugins.copilot_auth.register_callbacks import (
    ...
    @patch("code_muse.plugins.copilot_auth.register_callbacks.get_valid_session_token")
    @patch("code_muse.plugins.copilot_auth.register_callbacks.get_token_for_host")
    ...
    @patch(
        "code_muse.plugins.copilot_auth.register_callbacks.get_api_endpoint_for_host"
    )
    @patch("code_muse.plugins.copilot_auth.register_callbacks.get_valid_session_token")
    @patch("code_muse.plugins.copilot_auth.register_callbacks.get_token_for_host")
