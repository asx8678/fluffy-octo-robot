...
from code_muse.plugins.synthetic_status.status_api import (
    SyntheticQuota,
...
def test_fetch_synthetic_quota_success():
    ...
    with patch("code_muse.plugins.synthetic_status.status_api.httpx.get") as mock_get:
...
def test_fetch_synthetic_quota_timeout():
    with patch(
        "code_muse.plugins.synthetic_status.status_api.httpx.get",
...
def test_is_synthetic_only_provider_configured_true():
    ...
    with patch(
        "code_muse.plugins.synthetic_status.register_callbacks.get_api_key",
...
def test_handle_provider_synthetic_status_command():
    ...
    with (
        patch(
            "code_muse.plugins.synthetic_status.register_callbacks.resolve_syn_api_key",
            return_value="test-key",
        ),
        patch(
            "code_muse.plugins.synthetic_status.register_callbacks.fetch_synthetic_quota",
            return_value=SyntheticQuotaResult(quota=quota),
        ),
        patch(
            "code_muse.plugins.synthetic_status.register_callbacks.emit_info"
        ) as mock_info,
    ):
...
def test_handle_status_ambiguous_provider_message():
    with (
        patch(
            "code_muse.plugins.synthetic_status.register_callbacks._is_synthetic_only_provider_configured",
            return_value=False,
        ),
        patch(
            "code_muse.plugins.synthetic_status.register_callbacks.emit_warning"
        ) as mock_warning,
    ):
