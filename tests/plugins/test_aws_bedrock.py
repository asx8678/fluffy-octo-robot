...
def tmp_extra_models(tmp_path):
    ...
    with patch(
        "code_muse.plugins.aws_bedrock.utils.get_extra_models_path", return_value=p
    ):
        yield p
...
class TestConfig:
    ...
    def test_models_list_has_expected_entries(self):
        from code_muse.plugins.aws_bedrock.config import MODELS
    ...
    def test_get_bedrock_region_env_override(self):
        from code_muse.plugins.aws_bedrock.config import get_bedrock_region
    ...
    def test_get_bedrock_region_default(self):
        ...
        with (
            ...
            patch(
                "code_muse.plugins.aws_bedrock.config._detect_region",
                return_value=None,
            ),
        ):
            import os
    ...
    def test_get_aws_profile_from_env(self):
        from code_muse.plugins.aws_bedrock.config import get_aws_profile
...
class TestBuildModelEntry:
    ...
    def test_basic_entry(self):
        from code_muse.plugins.aws_bedrock.utils import _build_model_entry
...
class TestAddBedrockModelsToConfig:
    ...
    def test_adds_all_models_and_variants(self, tmp_extra_models):
        from code_muse.plugins.aws_bedrock.utils import add_bedrock_models_to_config
    ...
    def test_returns_empty_on_save_failure(self, tmp_extra_models):
        ...
        with patch(
            "code_muse.plugins.aws_bedrock.utils.save_extra_models",
            return_value=False,
        ):
...
class TestRemoveBedrockModelsFromConfig:
    ...
    def test_removes_only_bedrock_entries(self, extra_models_with_bedrock):
        from code_muse.plugins.aws_bedrock.utils import (
...
class TestHandleCustomCommand:
    ...
    def test_returns_none_for_unknown_command(self):
        from code_muse.plugins.aws_bedrock.register_callbacks import (
    ...
    def test_returns_true_for_known_command(self):
        ...
        with patch(
            "code_muse.plugins.aws_bedrock.register_callbacks._handle_bedrock_status"
        ):
...
class TestCustomHelp:
    ...
    def test_help_entries(self):
        from code_muse.plugins.aws_bedrock.register_callbacks import _custom_help
...
class TestShouldUseThinkingSummary:
    ...
    def test_true_for_opus_4_7(self):
        from code_muse.model_utils import should_use_anthropic_thinking_summary
