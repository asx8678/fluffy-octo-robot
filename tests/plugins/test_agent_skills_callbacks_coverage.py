...
_CFG = "code_muse.plugins.agent_skills.config"
_DISC = "code_muse.plugins.agent_skills.discovery"
_META = "code_muse.plugins.agent_skills.metadata"
_PB = "code_muse.plugins.agent_skills.prompt_builder"
...
class TestGetSkillsPromptSection:
    ...
    def test_deactivated_skills_filtered(self):
        from code_muse.plugins.agent_skills.register_callbacks import (
            _deactivated_skills,
            _get_skills_prompt_section,
        )
        ...
        skill = MagicMock(has_skill_md=True)
        skill.name = "deactivated_skill"
        metadata = MagicMock()

        _deactivated_skills.add("deactivated_skill")
        try:
            with (
                patch(f"{_CFG}.get_skills_enabled", return_value=True),
                patch(f"{_CFG}.get_skill_directories", return_value=["/fake"]),
            ...
            _deactivated_skills.discard("deactivated_skill")
...
class TestInjectSkillsIntoPrompt:
    def test_no_skills(self):
        ...
        with patch(
            "code_muse.plugins.agent_skills.register_callbacks._get_skills_prompt_section",
            return_value=None,
        ):
...
class TestRegisterSkillsTools:
    def test_returns_tools(self):
        ...
        assert "run_skill_background" in names
        assert "check_skill_background" in names
...
_MSG = "code_muse.messaging"
_SKILLS_MENU = "code_muse.plugins.agent_skills.skills_menu"
_SKILLS_INSTALL = "code_muse.plugins.agent_skills.skills_install_menu"
