...
def _plugin_module():
    return importlib.import_module(
        "code_muse.plugins.prompt_newline.register_callbacks"
    )


def _config_module():
    return importlib.import_module("code_muse.plugins.prompt_newline.config")
...
def test_install_prompt_patch_is_idempotent():
    ...
    try:
        ...
        assert ptc._prompt_newline_original is original
    finally:
...
def test_patched_prompt_appends_newline_only_when_enabled():
    ...
    try:
        ...
        with patch(
            "code_muse.plugins.prompt_newline.register_callbacks.is_enabled",
            return_value=False,
        ):
...
def test_handle_command_persists_explicit_on(tmp_path, monkeypatch):
    # Point the config at a throwaway file so we don't trash real muse.cfg
    cfg_file = tmp_path / "muse.cfg"
    monkeypatch.setattr("code_muse.config.CONFIG_FILE", cfg_file)
    ...
    with (
        patch(
            "code_muse.plugins.prompt_newline.register_callbacks._emit_success"
        ) as mock_success,
        patch("code_muse.plugins.prompt_newline.register_callbacks._emit_info"),
    ):
...
def test_handle_command_flips_when_no_arg(tmp_path, monkeypatch):
    ...
    with (
        patch("code_muse.plugins.prompt_newline.register_callbacks._emit_success"),
        patch("code_muse.plugins.prompt_newline.register_callbacks._emit_info"),
    ):
...
def test_handle_command_rejects_garbage_arg(tmp_path, monkeypatch):
    cfg_file = tmp_path / "muse.cfg"
    monkeypatch.setattr("code_muse.config.CONFIG_FILE", str(cfg_file))

    module = _plugin_module()

    with patch(
        "code_muse.plugins.prompt_newline.register_callbacks._emit_error"
    ) as mock_error:
