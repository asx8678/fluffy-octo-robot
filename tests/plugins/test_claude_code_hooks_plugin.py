...
Tests for code_muse/plugins/claude_code_hooks/
...
Implementation targets:
  code_muse/plugins/claude_code_hooks/config.py
  code_muse/plugins/claude_code_hooks/register_callbacks.py
"""
...
class TestGetHooksConfigPaths:
    def test_returns_list(self):
        from code_muse.plugins.claude_code_hooks.config import get_hooks_config_paths
    ...
    def test_project_path_contains_claude_settings(self):
        ...
        assert ".claude" in str(paths[0]) and "settings.json" in str(paths[0])

    def test_global_path_contains_code_muse(self):
        from code_muse.plugins.claude_code_hooks.config import get_hooks_config_paths

        paths = get_hooks_config_paths()
        # Second path is user-level (~/.muse/hooks.json)
        assert ".muse" in str(paths[1]) and "hooks.json" in str(paths[1])
...
class TestLoadHooksConfigNoFiles:
    def test_returns_none_when_no_config_files(self):
        from code_muse.plugins.claude_code_hooks.config import load_hooks_config

        with (
            tempfile.TemporaryDirectory() as tmpdir,
            patch("pathlib.Path.cwd", return_value=Path(tmpdir)),
            patch(
                "code_muse.plugins.claude_code_hooks.config.GLOBAL_HOOKS_FILE",
                str(Path(tmpdir) / "nonexistent_hooks.json"),
            ),
        ):
...
class TestLoadHooksConfigGlobalLevel:
    def test_loads_from_global_hooks_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            global_hooks = str(Path(tmpdir) / "hooks.json")
    ...
    def test_project_level_merges_with_global(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            global_file = str(Path(tmpdir) / "hooks.json")
...
class TestOnPreToolCallHook:
    ...
    async def test_returns_none_when_no_engine(self):
        from code_muse.plugins.claude_code_hooks import register_callbacks
    ...
    async def test_returns_none_when_hook_allows(self):
        from code_muse.hook_engine.models import ProcessEventResult
        from code_muse.plugins.claude_code_hooks import register_callbacks
    ...
    async def test_passes_tool_name_and_args_to_engine(self):
        from code_muse.hook_engine.models import EventData, ProcessEventResult
        from code_muse.plugins.claude_code_hooks import register_callbacks
...
class TestCallbackRegistration:
    def test_pre_tool_call_registered(self):
        ...
        from code_muse.plugins.claude_code_hooks.register_callbacks import (
            on_pre_tool_call_hook,
        )

        registered_funcs = [f for _, f in _callbacks.get("pre_tool_call", [])]
        assert on_pre_tool_call_hook in registered_funcs
    ...
    def test_post_tool_call_registered(self):
        ...
        registered_funcs = [f for _, f in _callbacks.get("post_tool_call", [])]
        assert on_post_tool_call_hook in registered_funcs
