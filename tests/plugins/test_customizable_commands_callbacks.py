...
import code_muse.plugins.customizable_commands.register_callbacks as callbacks_module
from code_muse.callbacks import get_callbacks, register_callback
from code_muse.command_line.types import MarkdownCommandResult
from code_muse.plugins.customizable_commands.register_callbacks import (
...
class TestMarkdownCommandResult:
    ...
    def test_repr_is_dataclass(self):
        """Test that __repr__ is the standard dataclass repr."""
        result = MarkdownCommandResult("Hello")
        assert (
            repr(result) == "MarkdownCommandResult(content='Hello', is_markdown=True)"
        )
    ...
    def test_repr_with_long_content(self):
        ...
        assert (
            repr(result)
            == f"MarkdownCommandResult(content={content!r}, is_markdown=True)"
        )

    def test_repr_with_empty_content(self):
        """Test __repr__ with empty content."""
        result = MarkdownCommandResult("")
        assert repr(result) == "MarkdownCommandResult(content='', is_markdown=True)"
...
class TestLoadMarkdownCommands:
    ...
    def test_loads_md_files_from_claude_commands(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                "code_muse.plugins.customizable_commands.register_callbacks._COMMAND_DIRECTORIES",
                [str(cmd_dir)],
            ):
                _load_markdown_commands()
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                ...
                with patch.object(Path, "read_text", mock_read_text):
                    with patch(
                        "code_muse.plugins.customizable_commands.register_callbacks.emit_error"
                    ) as mock_emit:
                        _load_markdown_commands()
...
class TestCustomHelp:
    ...
    def test_reloads_commands_on_each_call(self):
        ...
        with patch(
            "code_muse.plugins.customizable_commands.register_callbacks._load_markdown_commands"
        ) as mock_load:
...
class TestHandleCustomCommand:
    ...
    def test_returns_markdown_result_for_valid_command(self):
        ...
        with patch(
            "code_muse.plugins.customizable_commands.register_callbacks.emit_info"
        ):
...
class TestModuleExports:
    ...
    def test_markdown_command_result_in_all(self):
        ...
        from code_muse.plugins.customizable_commands import register_callbacks
...
class TestGlobalCommands:
    ...
    def test_global_directory_in_command_directories(self):
        ...
        assert "~/.muse/commands" in _COMMAND_DIRECTORIES
        # Global should be first (lowest priority, gets overridden by project)
        assert _COMMAND_DIRECTORIES[0] == "~/.muse/commands"
