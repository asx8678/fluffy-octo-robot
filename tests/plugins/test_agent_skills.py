...
from code_muse.plugins.agent_skills.prompt_builder import (
...
class TestSkillDiscovery:
    ...
    def test_get_default_skill_directories(self):
        ...
        assert directories[0] == Path.home() / ".muse" / "skills"
        assert directories[1] == Path.cwd() / ".muse" / "skills"
    ...
    def test_refresh_skill_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "code_muse.plugins.agent_skills.discovery.discover_skills", mock_discover
        )
...
class TestSkillsConfig:
    ...
    def test_get_skill_directories_default(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "code_muse.plugins.agent_skills.config.get_value", mock_get_value
        )

        directories = get_skill_directories()
        assert len(directories) == 3
        # The tilde will be expanded to the actual home directory
        assert ".muse/skills" in str(directories[0])
        assert ".muse/skills" in str(directories[1])
        # The current directory path will contain the full path, ending with "skills"
        assert "skills" in str(directories[2])
    ...
    def test_get_skill_directories_from_config(self, monkeypatch):
        ...
        directories = get_skill_directories()
        assert directories == [Path("/path1"), Path("/path2")]
    ...
    def test_add_skill_directory_new(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "code_muse.plugins.agent_skills.config.get_skill_directories",
        ...
        monkeypatch.setattr(
            "code_muse.plugins.agent_skills.config.set_value", mock_set_value
        )
    ...
    def test_set_skill_disabled_add(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "code_muse.plugins.agent_skills.config.get_disabled_skills",
...
# Tests for new Skills Tools (consent, deactivate, background)


class TestActivateSkillConsent:
    """Tests for activate_skill consent gating."""

    @pytest.mark.asyncio
    @patch(
        "code_muse.plugins.agent_skills.config.get_skills_enabled", return_value=True
    )
    @patch(
        "code_muse.plugins.agent_skills.config.get_skill_directories",
        return_value=["/tmp"],
    )
    @patch("code_muse.plugins.agent_skills.discovery.discover_skills")
    async def test_consent_required_false(self, mock_disc, mock_dirs, mock_enabled):
        from code_muse.tools.skills_tools import register_activate_skill
        ...
        result = await captured["fn"](
            ctx, skill_name="test-skill", consent_confirmed=False
        )
        assert result.consent_required is True
        assert result.content == ""
        assert "May I activate" in result.message

    @pytest.mark.asyncio
    @patch("code_muse.tools.skills_tools.get_message_bus")
    @patch(
        "code_muse.plugins.agent_skills.metadata.get_skill_resources", return_value=[]
    )
    @patch(
        "code_muse.plugins.agent_skills.metadata.load_full_skill_content",
    ...
    async def test_consent_confirmed_true(
        ...
        ctx = MagicMock()
        result = await captured["fn"](
            ctx, skill_name="test-skill", consent_confirmed=True
        )
...
class TestDeactivateSkill:
    """Tests for deactivate_skill tool."""

    @pytest.mark.asyncio
    async def test_deactivate_skill(self):
        from code_muse.plugins.agent_skills.register_callbacks import (
            _deactivated_skills,
        )
        from code_muse.tools.skills_tools import register_deactivate_skill

        # Ensure clean state
        _deactivated_skills.discard("my-skill")
        ...
        agent.tool = tool
        register_deactivate_skill(agent)
        ctx = MagicMock()
        result = await captured["fn"](ctx, skill_name="my-skill")
        assert result.success is True
        assert "my-skill" in result.message
        assert "my-skill" in _deactivated_skills

        # Cleanup
        _deactivated_skills.discard("my-skill")

    def test_deactivated_skill_filtered_from_prompt(self, tmp_path):
        from code_muse.plugins.agent_skills.register_callbacks import (
            _deactivated_skills,
        ...
        skill_dir = tmp_path / "skills"
        skill_dir.mkdir()
        s = skill_dir / "filtered-skill"
        s.mkdir()
        (s / "SKILL.md").write_text(
            "---\nname: filtered-skill\ndescription: Should be filtered\n---\n"
        )

        _deactivated_skills.add("filtered-skill")
        try:
            # Patch config functions used inside _get_skills_prompt_section
            from code_muse.plugins.agent_skills import config as cfg_mod
            from code_muse.plugins.agent_skills import discovery as disc_mod
            from code_muse.plugins.agent_skills import metadata as meta_mod
            from code_muse.plugins.agent_skills import prompt_builder as pb_mod

            orig_get_skills_enabled = cfg_mod.get_skills_enabled
            orig_get_skill_directories = cfg_mod.get_skill_directories
            orig_get_disabled_skills = cfg_mod.get_disabled_skills
            orig_discover_skills = disc_mod.discover_skills
            orig_parse_skill_metadata = meta_mod.parse_skill_metadata
            orig_build_available_skills_xml = pb_mod.build_available_skills_xml
            orig_build_skills_guidance = pb_mod.build_skills_guidance

            cfg_mod.get_skills_enabled = lambda: True
            cfg_mod.get_skill_directories = lambda: [str(skill_dir)]
            cfg_mod.get_disabled_skills = lambda: set()
            disc_mod.discover_skills = lambda directories=None: orig_discover_skills(
                directories=[skill_dir]
            )

            def mock_parse(path):
                md = orig_parse_skill_metadata(path)
                if md and md.name == "filtered-skill":
                    # Return a real metadata so it would normally be included
                    return md
                return md

            meta_mod.parse_skill_metadata = mock_parse
            pb_mod.build_available_skills_xml = lambda skills: "<xml/>"
            pb_mod.build_skills_guidance = lambda: "guidance"

            try:
                result = _get_skills_prompt_section()
                assert result is None
            finally:
                cfg_mod.get_skills_enabled = orig_get_skills_enabled
                cfg_mod.get_skill_directories = orig_get_skill_directories
                cfg_mod.get_disabled_skills = orig_get_disabled_skills
                disc_mod.discover_skills = orig_discover_skills
                meta_mod.parse_skill_metadata = orig_parse_skill_metadata
                pb_mod.build_available_skills_xml = orig_build_available_skills_xml
                pb_mod.build_skills_guidance = orig_build_skills_guidance
        finally:
            _deactivated_skills.discard("filtered-skill")


class TestBackgroundSkill:
    """Tests for run_skill_background and check_skill_background."""

    @pytest.mark.asyncio
    @patch("code_muse.tools.skills_tools.subprocess.Popen")
    @patch("code_muse.tools.skills_tools.get_message_bus")
    @patch(
        "code_muse.plugins.agent_skills.metadata.parse_yaml_frontmatter",
        return_value={},
    )
    @patch(
        "code_muse.plugins.agent_skills.metadata.load_full_skill_content",
    ...
    async def test_run_skill_background(
        self,
        mock_en,
        mock_dirs,
        mock_disc,
        ...
        agent.tool = tool
        register_run_skill_background(agent)
        ctx = MagicMock()
        result = await captured["fn"](ctx, skill_name="bg-skill", task="do stuff")
        assert result.status == "started"
        assert result.job_id
        assert result.log_file
        assert result.job_id in _background_jobs

        # Cleanup
        _background_jobs.pop(result.job_id, None)
    ...
    async def test_check_skill_background(
        self,
        mock_en,
        mock_dirs,
        mock_disc,
        ...
        register_run_skill_background(agent)
        ctx = MagicMock()
        run_result = await captured["fn"](ctx, skill_name="bg-skill", task="do stuff")
        job_id = run_result.job_id

        # Write fake log so check can read it
        log_file = run_result.log_file
        from pathlib import Path

        Path(log_file).write_text("completed output", encoding="utf-8")
        ...
        agent2.tool = tool2
        register_check_skill_background(agent2)
        check_result = await captured2["fn"](ctx, job_id=job_id)
        assert check_result.status == "complete"
        assert check_result.result == "completed output"
