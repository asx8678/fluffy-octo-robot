...
def _plugin_module():
    return importlib.import_module("code_muse.plugins.pop_command.register_callbacks")
...
def test_parse_pop_count_rejects_invalid_integer():
    ...
    with patch(
        "code_muse.plugins.pop_command.register_callbacks.emit_error"
    ) as mock_error:
...
def test_handle_pop_command_pops_and_prunes_tail_fragments():
    ...
    with (
        patch.dict(
            sys.modules,
            {"code_muse.agents.agent_manager": _agent_manager_module(agent)},
        ),
        patch(
            "code_muse.plugins.pop_command.register_callbacks.emit_success"
        ) as mock_success,
        patch("code_muse.plugins.pop_command.register_callbacks.emit_info"),
    ):
...
def test_handle_pop_command_preserves_system_prompt_when_count_too_large():
    ...
    with (
        ...
        patch(
            "code_muse.plugins.pop_command.register_callbacks.emit_warning"
        ) as mock_warning,
        patch("code_muse.plugins.pop_command.register_callbacks.emit_success"),
        patch(
            "code_muse.plugins.pop_command.register_callbacks.emit_info"
        ) as mock_info,
    ):
