"""100% coverage tests for code_muse/plugins/agent_skills/downloader.py."""
...
from code_muse.plugins.agent_skills.downloader import (
...
def test_safe_rmtree_failure(tmp_path):
    with patch(
        "code_muse.plugins.agent_skills.downloader.shutil.rmtree",
...
def test_download_to_file_success(tmp_path):
    ...
    with patch(
        "code_muse.plugins.agent_skills.downloader.httpx.Client", return_value=mc
    ):
...
def test_safe_extract_zip_outside_dir(tmp_path):
    ...
    extract_dir = tmp_path / "out"
    with (
        zipfile.ZipFile(BytesIO(data)) as zf,
        patch(
            "code_muse.plugins.agent_skills.downloader._is_within_directory",
            return_value=False,
        ),
    ):
...
def test_stage_normalized_install_exception(tmp_path):
    with patch(
        "code_muse.plugins.agent_skills.downloader.shutil.copytree",
        side_effect=OSError,
    ):
        src = tmp_path / "src"
        src.mkdir()
...
def test_force_reinstall_rmtree_fails(tmp_path):
    ...
    skill_dir.mkdir()
    with patch(
        "code_muse.plugins.agent_skills.downloader._safe_rmtree", return_value=False
    ):
        r = download_and_install_skill(
...
def test_download_failure(tmp_path):
    with patch(
        "code_muse.plugins.agent_skills.downloader._download_to_file",
        return_value=False,
    ):
...
def test_bad_zip_file(tmp_path):
    with (
        ...
        patch(
            "code_muse.plugins.agent_skills.downloader.zipfile.ZipFile",
...
def test_unsafe_zip(tmp_path):
    with patch(
        ...
        with (
            ...
            patch(
                "code_muse.plugins.agent_skills.downloader._validate_zip_safety",
...
def test_extract_failure(tmp_path):
    with patch(
        ...
        with (
            ...
            patch(
                "code_muse.plugins.agent_skills.downloader._safe_extract_zip",
                return_value=False,
            ),
        ):
            r = download_and_install_skill("myskill", "http://x", target_dir=tmp_path)
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        "code_muse.plugins.agent_skills.downloader._determine_extracted_root",
                        return_value=None,
                    ):
                        r = download_and_install_skill(
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        with patch(
                            "code_muse.plugins.agent_skills.downloader._stage_normalized_install",
                            return_value=None,
                        ):
                            r = download_and_install_skill(
                                "myskill", "http://x", target_dir=tmp_path
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        with patch(
                            ...
                            with patch(
                                "code_muse.plugins.agent_skills.downloader.shutil.move",
                                side_effect=OSError,
                            ):
                                r = download_and_install_skill(
                                    "myskill", "http://x", target_dir=tmp_path
...
def test_full_success(tmp_path):
    ...
    with (
        ...
        patch("code_muse.plugins.agent_skills.downloader.refresh_skill_cache"),
    ):
        r = download_and_install_skill("myskill", "http://x", target_dir=tmp_path)
    assert r.success
...
def test_full_success_cache_refresh_fails(tmp_path):
    ...
    with (
        ...
        patch(
            "code_muse.plugins.agent_skills.downloader.refresh_skill_cache",
