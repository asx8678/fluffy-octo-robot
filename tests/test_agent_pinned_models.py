...
def mock_config_paths(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        tmp_config_dir = str(Path(tmp_dir) / ".muse")
        tmp_config_file = Path(tmp_config_dir) / "muse.cfg"
        monkeypatch.setattr("code_muse.config.CONFIG_DIR", tmp_config_dir)
        monkeypatch.setattr("code_muse.config.CONFIG_FILE", tmp_config_file)
        # Ensure the directory exists for the patched paths
        Path(tmp_config_dir).mkdir(parents=True, exist_ok=True)
        yield
