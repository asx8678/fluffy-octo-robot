...
TEST_CONFIG_PATH = str(Path(__file__).parent.parent / "code_muse" / "models.json")
...
def test_custom_openai_timeout_config(monkeypatch):
    ...
    with patch("code_muse.model_factory.create_async_client") as mock_client:
...
def test_custom_anthropic_timeout_config(monkeypatch):
    ...
    with (
        patch("code_muse.model_factory.ClaudeCacheAsyncClient") as mock_client,
        patch("code_muse.model_factory.make_anthropic_provider") as mock_provider,
        patch("code_muse.model_factory.AsyncAnthropic") as mock_anthropic,
        patch("code_muse.model_factory.get_http2", return_value=False),
    ):
...
def test_cerebras_timeout_config(monkeypatch):
    ...
    mock_client.assert_called_once_with(
        headers={"X-Api-Key": "ok", "X-Cerebras-3rd-Party-Integration": "muse"},
        verify=False,
        model_name="cerebras",
        timeout=600,
    )
...
def test_extra_models_json_decode_error(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "code_muse.model_factory.EXTRA_MODELS_FILE", str(extra_models_file)
    )
