...
def mock_spinner_registration():
    ...
    with (
        patch("code_muse.messaging.spinner.register_spinner"),
        patch("code_muse.messaging.spinner.unregister_spinner"),
    ):
        yield
...
class TestConsoleSpinnerInit:
    ...
    def test_init_creates_console_if_not_provided(self):
        ...
        from code_muse.messaging.spinner.console_spinner import ConsoleSpinner
    ...
    def test_init_registers_spinner(self):
        ...
        with patch("code_muse.messaging.spinner.register_spinner") as mock_register:
            from code_muse.messaging.spinner.console_spinner import ConsoleSpinner
...
class TestConsoleSpinnerStart:
    ...
    def test_start_sets_spinning_state(self):
        ...
        with (
            patch("code_muse.messaging.spinner.console_spinner.Live") as mock_live_cls,
...
class TestConsoleSpinnerStop:
    ...
    def test_stop_when_not_spinning_returns_early(self):
        ...
        with patch("code_muse.messaging.spinner.unregister_spinner") as mock_unreg:
            spinner.stop()
...
class TestConsoleSpinnerGeneratePanel:
    ...
    def test_generate_panel_when_paused_returns_empty(self):
        ...
        with patch(
            "code_muse.tools.command_runner.is_awaiting_user_input", return_value=False
        ):
    ...
    def test_generate_panel_when_awaiting_input_returns_empty(self):
        ...
        with patch(
            "code_muse.tools.command_runner.is_awaiting_user_input", return_value=True
        ):
            result = spinner._generate_spinner_panel()
    ...
    def test_generate_panel_includes_current_frame(self):
        ...
        spinner._paused = False
        spinner._frame_index = 0
        spinner._animations_enabled = False

        with patch(
            "code_muse.tools.command_runner.is_awaiting_user_input",
            return_value=False,
        ):
            result = spinner._generate_spinner_panel()
...
class TestConsoleSpinnerResume:
    ...
    def test_resume_restarts_live_display(self):
        ...
        with (
            ...
            patch(
                "code_muse.messaging.spinner.console_spinner.Live"
            ) as mock_live_class,
            patch.object(sys, "stdout", mock_stdout),
        ):
            mock_live = MagicMock()
...
class TestConsoleSpinnerContextManager:
    ...
    def test_context_manager_full_cycle(self):
        ...
        with (
            patch("code_muse.messaging.spinner.console_spinner.Live") as mock_live,
        ):
