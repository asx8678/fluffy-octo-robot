...
def test_completion_directory_listing(tmp_path):
    ...
    try:
        ...
        completions = list(completer.get_completions(doc, None))
        # In modern prompt_toolkit, display is a FormattedText:
...
def test_set_completer_excludes_muse_token(monkeypatch):
    # Ensure 'muse_token' is a config key but SetCompleter doesn't offer it
    monkeypatch.setattr(
        "code_muse.command_line.prompt_toolkit_completion.get_config_keys",
        lambda: ["muse_token", "user_name", "temp_dir"],
    )
    monkeypatch.setattr(
        "code_muse.command_line.prompt_toolkit_completion.get_value",
        lambda key: "sensitive_token_value" if key == "muse_token" else "normal_value",
    )
    completer = SetCompleter()

    # Test with full "muse_token" typed
    doc = Document(text="/set muse_token", cursor_position=len("/set muse_token"))
    completions = list(completer.get_completions(doc, None))
    assert completions == [], (
        "SetCompleter should not complete for 'muse_token' key directly"
    )

    # Test with partial "muse" that would match "muse_token"
    doc = Document(text="/set muse", cursor_position=len("/set muse"))
    completions = list(completer.get_completions(doc, None))
    assert completions == [], (
        "SetCompleter should not complete for 'muse_token' key even partially"
    )
...
def test_set_completer_no_match(monkeypatch):
    monkeypatch.setattr("code_muse.config.get_config_keys", lambda: ["actual_key"])
    completer = SetCompleter()
...
def test_cd_completer_sub_directory(setup_cd_test_dirs, monkeypatch):
    ...
    completions = list(completer.get_completions(doc, None))
    texts = sorted([c.text for c in completions])
    # Completions relative to 'base' typed in command ('dir1/')
    assert texts == sorted(["dir1/"])
    displays = sorted(["".join(item[1] for item in c.display) for c in completions])
    assert displays == sorted(["dir1/"])
...
def test_cd_completer_home_directory_expansion(setup_cd_test_dirs, monkeypatch):
    ...
    assert texts == sorted(["~/user/"])
    assert displays == sorted(["user/"])
...
@pytest.mark.asyncio
@patch("code_muse.command_line.prompt_toolkit_completion.PromptSession")
@patch("code_muse.command_line.prompt_toolkit_completion.FileHistory")
@patch("code_muse.command_line.prompt_toolkit_completion.merge_completers")
...
@pytest.mark.asyncio
@patch("code_muse.command_line.prompt_toolkit_completion.PromptSession")
@patch("code_muse.command_line.prompt_toolkit_completion.SafeFileHistory")
...
# To test key bindings, inspect KeyBindings passed to PromptSession
# from mock_prompt_session_cls.call_args


@pytest.mark.xfail(
    reason=(
        "Alt+M binding representation varies across prompt_toolkit versions;"
        " current implementation may not expose Keys.Escape + 'm' tuple."
    ),
    strict=False,
)
@pytest.mark.asyncio
