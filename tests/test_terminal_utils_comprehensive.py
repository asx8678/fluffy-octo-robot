...
import code_muse.terminal_utils  # noqa: E402
...
class TestWindowsANSIReset:
    ...
    def test_reset_ansi_handles_stderr_error(
        ...
        mock_stdout.write = MagicMock()
        mock_stdout.flush = MagicMock()
        mock_stderr_write.side_effect = OSError("Write failed")
...
class TestWindowsCtrlCDisable:
    ...
    @patch("platform.system")
    @patch("ctypes.windll.kernel32")
    @patch("code_muse.terminal_utils._original_ctrl_handler", None)
...
class TestWindowsCtrlCEnable:
    ...
    @patch("platform.system")
    @patch("code_muse.terminal_utils._original_ctrl_handler", 0x0007)
    @patch("ctypes.windll.kernel32")
    ...
    def test_enable_ctrl_c_no_restore_needed(self, mock_platform):
        ...
        with patch("code_muse.terminal_utils._original_ctrl_handler", None):
            result = enable_windows_ctrl_c()
...
class TestSetKeepCtrlCDisabled:
    ...
    def test_set_keep_ctrl_c_disabled_true(self):
        ...
        assert code_muse.terminal_utils._keep_ctrl_c_disabled is True

    def test_set_keep_ctrl_c_disabled_false(self):
        """Test setting keep Ctrl+C disabled to False."""
        set_keep_ctrl_c_disabled(False)

        assert code_muse.terminal_utils._keep_ctrl_c_disabled is False


class TestEnsureCtrlCDisabled:
    """Test ensure_ctrl_c_disabled function."""

    @patch("code_muse.terminal_utils._keep_ctrl_c_disabled", False)
    ...
    @patch("platform.system")
    @patch("code_muse.terminal_utils._keep_ctrl_c_disabled", True)
