...
class TestRunPromptWithAttachments:
    ...
    async def test_with_attachments_and_spinner(self):
        from code_muse.cli_runner import run_prompt_with_attachments
...
class TestMainEntry:
    ...
    def test_normal_exit(self, mock_run):
        ...
        mock_run.return_value = None
        with patch("code_muse.cli_runner.reset_unix_terminal"):
            with patch("code_muse.tools.command_runner.shutdown"):
                with patch("code_muse.callbacks._shutdown_executor"):
                    result = main_entry()
        assert result == 0
