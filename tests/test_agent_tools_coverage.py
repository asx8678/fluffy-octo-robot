...
Workflow-id tests were removed when durable execution moved to a plugin; see
``code_muse/plugins/durable_exec/`` for plugin-level tests (Phase 4).
"""
...
class TestGetSubagentSessionsDir:
    ...
    def test_returns_path_object(self):
        ...
        with patch("code_muse.tools.agent_tools.DATA_DIR", tempfile.gettempdir()):
    ...
    def test_creates_directory_if_not_exists(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("code_muse.tools.agent_tools.DATA_DIR", tmpdir):
                result = _get_subagent_sessions_dir()
...
class TestRegisterListAgentsExecution:
    ...
    def test_list_agents_returns_available_agents(self):
        ...
        # Note: get_banner_color is imported from code_muse.config inside the function
        with (
            patch(
                "code_muse.config.get_banner_color",
                return_value="blue",
            ),
            patch("code_muse.tools.agent_tools.emit_info"),
            patch(
                "code_muse.tools.agent_tools.generate_group_id",
                return_value="test-group",
            ),
            patch("code_muse.agents.get_available_agents") as mock_available,
            patch("code_muse.agents.get_agent_descriptions") as mock_descriptions,
        ):
    ...
    def test_list_agents_handles_exception(self):
        ...
        with (
            ...
            patch("code_muse.tools.agent_tools.emit_info"),
            patch("code_muse.tools.agent_tools.emit_error") as mock_emit_error,
            patch(
                "code_muse.tools.agent_tools.generate_group_id",
                return_value="test-group",
            ),
...
class TestRegisterInvokeAgentExecution:
    ...
    async def test_invoke_agent_model_not_found_error(self):
        ...
        with (
            ...
            patch("code_muse.tools.agent_tools.get_message_bus") as mock_bus,
            patch(
                "code_muse.tools.agent_tools.get_session_context",
                return_value="parent",
            ),
            ...
            patch(
                "code_muse.tools.agent_tools._load_session_history",
                return_value=[],
            ),
            patch(
                "code_muse.tools.agent_tools._generate_session_hash_suffix",
                return_value="abc123",
            ),
        ):
    ...
    async def test_invoke_agent_session_context_restored_on_error(self):
        ...
        with (
            ...
            patch(
                "code_muse.tools.agent_tools.set_session_context",
...
class TestInvokeAgentPartialSessionSaveOnCrash:
    ...
    async def test_partial_history_saved_when_run_crashes(self):
        ...
        with (
            ...
            patch("code_muse.tools.agent_tools._save_session_history") as mock_save,
        ):
            mock_bus.return_value.emit = MagicMock()
    ...
    async def test_save_failure_does_not_mask_original_error(self):
        ...
        with (
            ...
            patch(
                "code_muse.tools.agent_tools._save_session_history",
...
class TestListAgentsEmitsBannerAndInfo:
    ...
    def test_emits_banner_message(self):
        ...
        with (
            ...
            patch("code_muse.tools.agent_tools.emit_info") as mock_emit_info,
            patch(
                "code_muse.tools.agent_tools.generate_group_id",
