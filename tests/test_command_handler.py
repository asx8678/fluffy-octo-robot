...
def setup_messaging_mocks():
    ...
    mocks = {}
    patch_targets = [
        "code_muse.messaging.emit_info",
        "code_muse.messaging.emit_error",
        "code_muse.messaging.emit_warning",
        "code_muse.messaging.emit_success",
        "code_muse.messaging.emit_system_message",
    ]
...
def test_cd_show_lists_directories():
    ...
    try:
        with patch("code_muse.command_line.utils.make_directory_table") as mock_table:
...
def test_set_config_only_key():
    ...
    try:
        with (
            patch("code_muse.config.set_config_value") as mock_set_cfg,
            patch("code_muse.config.get_config_keys", return_value=["key"]),
        ):
...
def test_show_status():
    ...
    try:
        with (
            ...
            patch("code_muse.config.get_owner_name", return_value="Ivan"),
            patch("code_muse.config.get_agent_name", return_value="Biscuit"),
            patch("code_muse.config.get_yolo_mode", return_value=True),
        ):
...
def test_set_no_args_prints_usage():
    ...
    try:
        with patch("code_muse.config.get_config_keys", return_value=["foo", "bar"]):
...
def test_agent_switch_triggers_autosave_rotation():
    ...
    try:
        current_agent = SimpleNamespace(name="muse", display_name="Muse")
        ...
        with (
            ...
            patch(
                ...
                return_value={"muse": "Muse", "reviewer": "Reviewer"},
            ),
            patch(
            ...
            patch(
                "code_muse.agents.set_current_agent",
                return_value=True,
            ) as mock_set,
        ):
...
def test_agent_switch_same_agent_skips_rotation():
    ...
    try:
        ...
        with (
            ...
            result = handle_command("/agent muse")
            assert result is True
...
def test_tools_file_not_found():
    ...
    try:
        ...
        with patch("code_muse.tools.tools_content.tools_content", "# Mock content"):
            result = handle_command("/tools")
            assert result is True
            mock_emit_info.assert_called_once()
            # Check that emit_info was called with a Markdown object
...
class TestRegistryIntegration:
    ...
    def test_unregistered_command_shows_warning(self):
        ...
        with patch("code_muse.messaging.emit_warning") as mock_warn:
...
class TestSessionCommand:
    ...
    def test_session_show_current_id(self):
        ...
        with (
            patch("code_muse.config.get_current_autosave_id", return_value="test-id"),
            patch(
                "code_muse.config.get_current_autosave_session_name",
                return_value="test-session",
            ),
            patch("code_muse.config.AUTOSAVE_DIR", "/tmp/autosave"),
    ...
    def test_session_new_rotates(self):
        ...
        with (
            patch(
                "code_muse.config.rotate_autosave_id", return_value="new-id"
            ) as mock_rotate,
    ...
    def test_session_alias_works(self):
        ...
        with (
            ...
            patch("code_muse.config.AUTOSAVE_DIR", "/tmp"),
            patch("code_muse.messaging.emit_info") as mock_emit,
        ):
...
class TestCompactCommand:
    ...
    def test_compact_with_history(self):
        ...
        with (
            ...
            patch(
                "code_muse.config.get_compaction_strategy",
                return_value="summarization",
            ),
            patch("code_muse.config.get_protected_token_count", return_value=1000),
    ...
    def test_compact_empty_history(self):
        ...
        with (
            ...
            patch("code_muse.messaging.emit_warning") as mock_warn,
        ):
            result = handle_command("/compact")
            assert result is True
            mock_warn.assert_called_once()
    ...
    def test_compact_with_truncation_strategy(self):
        ...
        with (
            ...
            patch(
                "code_muse.config.get_compaction_strategy", return_value="truncation"
            ),
            patch("code_muse.config.get_protected_token_count", return_value=1000),
            patch(
                "code_muse.agents._compaction.truncate",
...
class TestReasoningCommand:
    ...
    def test_reasoning_set_low(self):
        ...
        with (
            patch("code_muse.config.set_openai_reasoning_effort") as mock_set,
            patch("code_muse.config.get_openai_reasoning_effort", return_value="low"),
            patch(
                "code_muse.agents.agent_manager.get_current_agent",
                return_value=mock_agent,
            ),
    ...
    def test_reasoning_invalid_level(self):
        ...
        with (
            patch(
                "code_muse.config.set_openai_reasoning_effort",
...
class TestGetCommandsHelp:
    ...
    def test_help_parses_tuple_format(self):
        ...
        with patch("code_muse.callbacks.on_custom_command_help") as mock_callback:
...
def test_pin_model_command_case_insensitive_agent():
    ...
    with (
        ...
        patch("code_muse.messaging.emit_success") as mock_emit_success,
        patch("code_muse.messaging.emit_error") as mock_emit_error,
    ):
...
def test_pin_model_unpin_case_insensitive():
    ...
    with (
        ...
        patch("code_muse.messaging.emit_error") as mock_emit_error,
        patch(
            "code_muse.command_line.config_commands.handle_unpin_command",
            return_value=True,
        ) as mock_unpin,
    ):
        from code_muse.command_line.config_commands import handle_pin_model_command
...
def test_unpin_command_case_insensitive_agent():
    ...
    with (
        ...
        from code_muse.command_line.config_commands import handle_unpin_command
...
def test_pin_model_completion_case_insensitive_agent():
    ...
    from code_muse.command_line.pin_command_completion import PinModelCompleter
...
def test_unpin_completion_case_insensitive_agent():
    ...
    from code_muse.command_line.pin_command_completion import UnpinCompleter
