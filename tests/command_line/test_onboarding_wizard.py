"""Tests for code_muse/command_line/onboarding_wizard.py"""
...
MODULE = "code_muse.command_line.onboarding_wizard"
...
class TestHasCompletedOnboarding:
    ...
    def test_returns_true_when_file_exists(self, mock_exists):
        from code_muse.command_line.onboarding_wizard import has_completed_onboarding
...
class TestMarkOnboardingComplete:
    ...
    def test_creates_file(self, m_open, m_mkdir):
        from code_muse.command_line.onboarding_wizard import mark_onboarding_complete
...
class TestShouldShowOnboarding:
    ...
    def test_returns_true_when_not_completed(self, mock_completed):
        ...
        with patch.dict(os.environ, {}, clear=False):
            ...
            os.environ.pop("MUSE_SKIP_TUTORIAL", None)
    ...
    def test_returns_false_when_env_skip_1(self, mock_completed):
        ...
        with patch.dict(os.environ, {"MUSE_SKIP_TUTORIAL": "1"}):
            assert should_show_onboarding() is False
    ...
    def test_returns_false_when_env_skip_true(self, mock_completed):
        ...
        with patch.dict(os.environ, {"MUSE_SKIP_TUTORIAL": "true"}):
            assert should_show_onboarding() is False
    ...
    def test_returns_false_when_env_skip_yes(self, mock_completed):
        ...
        with patch.dict(os.environ, {"MUSE_SKIP_TUTORIAL": "yes"}):
            assert should_show_onboarding() is False


class TestResetOnboarding:
    @patch(f"{MODULE}.ONBOARDING_COMPLETE_FILE")
    def test_removes_file(self, mock_file):
        mock_file.exists.return_value = True
        from code_muse.command_line.onboarding_wizard import reset_onboarding

        reset_onboarding()
        mock_file.unlink.assert_called_once()

    @patch(f"{MODULE}.ONBOARDING_COMPLETE_FILE")
    def test_no_op_when_missing(self, mock_file):
...
class TestOnboardingWizard:
    def _make_wizard(self):
        from code_muse.command_line.onboarding_wizard import OnboardingWizard
...
class TestRunOnboardingIfNeeded:
    ...
    async def test_skips_if_not_needed(self, mock_should):
        from code_muse.command_line.onboarding_wizard import run_onboarding_if_needed
