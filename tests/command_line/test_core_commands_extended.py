...
from code_muse.command_line.core_commands import (
...
class TestHandleHelpCommand:
    ...
    def test_help_command_with_emoji_content(self):
        ...
        mock_help_text = "[Run] Commands:\n• /help - Show help\n• /exit - Exit"

        with (
            patch(
                "code_muse.command_line.core_commands.get_commands_help",
...
class TestHandleCdCommand:
    ...
    def test_cd_windows_path_with_dot_uses_backslashsafely(self):
        ...
        with patch("os.name", "nt"), patch("code_muse.messaging.emit_success"):
                ...
                with patch("os.path.isabs", return_value=True):
                    with patch("os.path.isdir", return_value=True):
                        with patch("os.chdir") as mock_chdir:
                            ...
                            assert result is True
                            mock_chdir.assert_called_once_with(Path(windows_path))
    ...
    def test_cd_listing_with_permission_error(self):
        ...
        with (
            patch(
                "code_muse.command_line.core_commands.make_directory_table",
                side_effect=PermissionError("Access denied"),
            ),
            patch("code_muse.messaging.emit_error") as mock_error,
        ):
            result = handle_cd_command("/cd")
            assert result is True
    ...
    def test_cd_with_nonexistent_parent(self):
        ...
        with patch("code_muse.messaging.emit_error") as mock_error:
            with patch("os.path.expanduser", side_effect=lambda x: x):
                with patch("os.path.isabs", return_value=True):
...
class TestHandleToolsCommand:
    ...
    def test_tools_command_with_markdown_rendering(self):
        ...
        with (
            patch(
                "code_muse.command_line.core_commands.tools_content",
                mock_tools_content,
            ),
            patch("code_muse.messaging.emit_info") as mock_emit,
        ):
    ...
    def test_tools_command_with_empty_content(self):
        ...
        with patch("code_muse.command_line.core_commands.tools_content", ""):
    ...
    def test_tools_command_with_unicode_content(self):
        ...
        with (
            patch(
                "code_muse.command_line.core_commands.tools_content", unicode_content
            ),
            patch("code_muse.messaging.emit_info") as mock_emit,
        ):
            result = handle_tools_command("/tools")
...
class TestHandleExitCommand:
    ...
    def test_exit_command_with_success_message(self):
        ...
        with patch("code_muse.messaging.emit_success") as mock_success:
    ...
    def test_exit_command_with_emit_error(self):
        ...
        with patch(
            "code_muse.messaging.emit_success", side_effect=Exception("Emit failed")
        ):
...
class TestHandleAgentCommand:
    ...
    def test_agent_command_show_current_with_descriptions(self):
        ...
        with (
            patch(
                "code_muse.command_line.core_commands.interactive_agent_picker",
                side_effect=Exception("Picker failed"),
            ),
            patch("code_muse.agents.get_current_agent", return_value=mock_current),
            patch("code_muse.agents.get_available_agents", return_value=mock_agents),
            patch(
                "code_muse.agents.get_agent_descriptions",
                return_value=mock_descriptions,
            ),
            patch("code_muse.messaging.emit_info") as mock_info,
            patch("code_muse.messaging.emit_warning") as mock_warning,
            patch(
                "code_muse.config.finalize_autosave_session",
    ...
    def test_agent_command_successful_switch(self):
        ...
        with patch(
            ...
            with patch(
                "code_muse.agents.get_current_agent",
                side_effect=[mock_old_agent, mock_new_agent],
            ):
                with patch("code_muse.agents.set_current_agent", return_value=True):
                    with patch(
                        "code_muse.config.finalize_autosave_session",
    ...
    def test_agent_command_with_agent_argument(self):
        ...
        with patch("code_muse.agents.get_available_agents", return_value=mock_agents):
            with patch(
                "code_muse.agents.get_current_agent",
    ...
    def test_agent_command_invalid_agent_name(self):
        ...
        with patch("code_muse.agents.get_available_agents", return_value=mock_agents):
            with patch("code_muse.messaging.emit_error") as mock_error:
                with patch("code_muse.messaging.emit_warning") as mock_warning:
    ...
    def test_agent_command_switch_failure_handling(self):
        ...
        with (
            patch(
                "code_muse.agents.get_available_agents",
                return_value={"target": "Target"},
            ),
            patch("code_muse.agents.get_current_agent", return_value=mock_current),
            patch("code_muse.agents.set_current_agent", return_value=False),
        ):  # Switch fails
            with patch(
                "code_muse.config.finalize_autosave_session",
...
class TestInteractiveAgentPicker:
    ...
    def test_agent_picker_returns_selected_agent(self):
        ...
        with (
            ...
            patch("code_muse.agents.get_current_agent") as mock_get_current,
        ):
            # First call returns current agent, second call returns new agent
            mock_get_current.side_effect = [mock_current, mock_new_agent]
            with (
                patch(
                    "code_muse.agents.get_available_agents", return_value=mock_agents
                ),
                patch(
                    "code_muse.agents.get_agent_descriptions",
                    return_value=mock_descriptions,
                ),
                patch("code_muse.agents.set_current_agent", return_value=True),
                patch(
                    "code_muse.config.finalize_autosave_session",
    ...
    def test_agent_picker_integration(self):
        ...
        from code_muse.command_line.agent_menu import (
...
class TestHandleModelCommand:
    ...
    def test_model_command_interactive_success(self):
        ...
        with (
            patch(
                "code_muse.command_line.core_commands.interactive_model_picker",
                return_value="gpt-4",
            ),
            patch(
                "code_muse.command_line.model_picker_completion.set_active_model"
            ) as mock_set,
            patch(
                "code_muse.command_line.model_picker_completion.get_active_model",
                return_value="gpt-4",
            ),
            patch("code_muse.messaging.emit_success") as mock_success,
        ):
    ...
    def test_model_command_picker_error_fallback(self):
        ...
        with (
            ...
            patch(
                "code_muse.command_line.model_picker_completion.load_model_names",
    ...
    def test_model_command_with_valid_argument(self):
        ...
        with (
            patch(
                "code_muse.command_line.core_commands.update_model_in_input",
...
class TestInteractiveModelPicker:
    ...
    async def test_model_picker_displays_panel(
        ...
        with (
            ...
            patch("code_muse.tools.common.arrow_select_async", return_value="gpt-3.5"),
        ):
    ...
    async def test_model_picker_with_current_indicator(
        ...
        with (
            ...
            patch(
                "code_muse.tools.common.arrow_select_async",
...
class TestEdgeCasesAndErrorHandling:
    ...
    def test_commands_with_unicode_arguments(self):
        ...
        with patch("code_muse.messaging.emit_error"):
    ...
    def test_agent_command_with_unicode_agent_name(self):
        ...
        with patch("code_muse.messaging.emit_error"):
            with patch("code_muse.agents.get_available_agents", return_value={}):
    ...
    def test_model_command_with_unicode_model_name(self):
        ...
        with (
            patch(
                "code_muse.command_line.model_picker_completion.update_model_in_input",
                return_value="",
            ),
            patch(
                "code_muse.command_line.model_picker_completion.load_model_names",
    ...
    def test_tools_command_with_malformed_markdown(self):
        ...
        with (
            patch(
                "code_muse.command_line.core_commands.tools_content", malformed_content
            ),
            patch("code_muse.messaging.emit_info") as mock_info,
        ):
            result = handle_tools_command("/tools")
...
class TestIntegrationScenarios:
    ...
    def test_complete_agent_switch_workflow(self):
        ...
        with (
            ...
            with patch("code_muse.messaging.emit_info") as mock_info:
                result = handle_agent_command("/agent")
                assert result is True
    ...
    def test_error_recovery_scenarios(self):
        ...
        with (
            ...
            patch("code_muse.agents.get_current_agent"),
            patch(
                "code_muse.agents.get_available_agents",
...
class TestRemoveModelCommand:
    """Tests for the /remove_model command."""

    def test_remove_model_success(self, tmp_path):
        """Test removing an existing model from extra_models.json."""
        extra_models = {
            "custom-gpt-4": {"type": "openai", "name": "custom-gpt-4"},
            "another-model": {"type": "anthropic", "name": "claude-test"},
        }
        extra_file = tmp_path / "extra_models.json"
        extra_file.write_text(json.dumps(extra_models))

        with (
            patch(
                "code_muse.config.EXTRA_MODELS_FILE",
                str(extra_file),
            ),
            patch("code_muse.messaging.emit_success") as mock_success,
        ):
            result = handle_remove_model_command("/remove_model custom-gpt-4")
            assert result is True
            mock_success.assert_called_once()

        # Verify the model was removed
        updated = json.loads(extra_file.read_text())
        assert "custom-gpt-4" not in updated
        assert "another-model" in updated

    def test_remove_model_not_found(self, tmp_path):
        """Test removing a model that doesn't exist."""
        extra_models = {"existing-model": {"type": "openai", "name": "existing"}}
        extra_file = tmp_path / "extra_models.json"
        extra_file.write_text(json.dumps(extra_models))
        ...
        with (
            ...
            result = handle_remove_model_command("/remove_model missing-model")
            assert result is False
            mock_warning.assert_called_once()
            call_args = mock_warning.call_args[0][0]
            assert "missing-model" in call_args
            assert "existing-model" in call_args

    def test_remove_model_empty_extra_models(self, tmp_path):
        """Test removing from an empty extra_models.json."""
        extra_file = tmp_path / "extra_models.json"
        ...
        with (
            ...
            result = handle_remove_model_command("/remove_model some-model")
            assert result is False
            mock_warning.assert_called_once()
            assert "empty" in mock_warning.call_args[0][0].lower()

    def test_remove_model_no_argument(self):
        """Test /remove_model with no model name."""
        with patch("code_muse.messaging.emit_warning") as mock_warning:
            result = handle_remove_model_command("/remove_model")
            assert result is False
            mock_warning.assert_called_once()
            assert "usage" in mock_warning.call_args[0][0].lower()

    def test_remove_model_invalid_json(self, tmp_path):
        """Test handling invalid JSON in extra_models.json."""
        extra_file = tmp_path / "extra_models.json"
        extra_file.write_text("not json")
        ...
        with (
            ...
            result = handle_remove_model_command("/remove_model any-model")
            assert result is False
            mock_warning.assert_called_once()
            assert "parsing" in mock_warning.call_args[0][0].lower()
