...
from code_muse.command_line.skills_completion import (
    SkillsCompleter,
...
class TestLoadCatalogSkillIds:
    @patch("code_muse.plugins.agent_skills.skill_catalog.catalog")
    ...
    def test_import_failure(self):
        with patch.dict(
            "sys.modules", {"code_muse.plugins.agent_skills.skill_catalog": None}
        ):
            result = load_catalog_skill_ids()
            assert result == []
...
class TestSkillsCompleter:
    ...
    def test_get_skill_ids_refresh(self):
        ...
        with patch(
            "code_muse.command_line.skills_completion.load_catalog_skill_ids",
            return_value=["new"],
        ):
            result = self.completer._get_skill_ids()
            assert result == ["new"]
