"""Tests for code_muse/command_line/attachments.py — targeting 100% coverage."""
...
def test_normalise_path_invalid():
    with (
        patch(
            "code_muse.command_line.attachments.Path.absolute",
...
def test_normalise_path_expanduser_runtime_error():
    with (
        patch(
            "code_muse.command_line.attachments.Path.expanduser",
            side_effect=RuntimeError("Could not determine home directory."),
        ),
        pytest.raises(AttachmentParsingError, match="Invalid path"),
    ):
        _normalise_path("~/foo.png")
...
def test_determine_media_type_unknown_image_ext():
    ...
    with patch(
        "code_muse.command_line.attachments.mimetypes.guess_type",
...
@patch("code_muse.command_line.attachments.os.name", "nt")
...
class TestDetectPathTokens:
    ...
    def test_path_normalise_error(self):
        ...
        with patch(
            "code_muse.command_line.attachments._normalise_path",
    ...
    def test_candidate_path_normalise_error_in_span(self, tmp_path):
        ...
        with patch(
            "code_muse.command_line.attachments._normalise_path", side_effect=patched
        ):
    ...
    def test_link_attachment_detection(self):
        ...
        with patch(
            "code_muse.command_line.attachments._parse_link", return_value=mock_link
        ):
    ...
    def test_stripped_token_exceeds_max_in_main_loop(self):
        ...
        with patch(
            "code_muse.command_line.attachments._is_probable_path", side_effect=patched
        ):
...
class TestParsePromptAttachments:
    ...
    def test_unreadable_file_silently_skipped(self, tmp_path):
        ...
        f.write_bytes(b"x")
        with patch(
            "code_muse.command_line.attachments._load_binary",
    ...
    def test_link_attachment_in_detections(self):
        ...
        with patch(
            "code_muse.command_line.attachments._detect_path_tokens",
