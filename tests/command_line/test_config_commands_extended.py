...
class TestReasoningCommand:
    ...
    def test_reasoning_command_valid_efforts(self):
        ...
        for effort in valid_efforts:
            with patch("code_muse.config.set_openai_reasoning_effort") as mock_set:
                with patch(
...
class TestSetCommand:
    ...
    def test_set_command_valid_key_value(self):
        ...
        with patch("code_muse.config.set_config_value") as mock_set:
            with patch("code_muse.config.get_config_keys", return_value=["test_key"]):
    ...
    def test_set_command_no_arguments(self):
        ...
        with patch("code_muse.config.get_config_keys", return_value=["key"]):
            with patch("code_muse.messaging.emit_warning") as mock_warning:
    ...
    def test_set_command_configuration_failure(self):
        ...
        with patch(
            "code_muse.config.set_config_value", side_effect=Exception("Set failed")
        ):
...
class TestPinModelCommand:
    ...
    def test_pin_model_builtin_agent_success(self):
        ...
        with (
            ...
            patch("code_muse.config.set_agent_pinned_model") as mock_pin,
        ):
            with patch("code_muse.messaging.emit_success"):
                result = handle_pin_model_command("/pin_model test_agent gpt-4")
                assert result is True
...
class TestUnpinCommand:
    ...
    def test_unpin_model_builtin_agent_success(self):
        ...
        with (
            ...
            patch("code_muse.config.clear_agent_pinned_model") as mock_clear,
        ):
            with patch("code_muse.messaging.emit_success") as mock_success:
...
class TestDiffCommand:
    ...
    def test_diff_command_successful_configuration(self):
        ...
        with (
            patch(
                "code_muse.command_line.diff_menu.interactive_diff_picker",
                return_value=mock_result,
            ),
            patch("code_muse.config.set_diff_addition_color") as mock_set_add,
        ):
            with patch("code_muse.config.set_diff_deletion_color") as mock_set_del:
