...
def test_clipboard_pil_import_failure():
    ...
    mod_name = "code_muse.command_line.clipboard"
...
def test_load_context_completion_exception():
    ...
    with patch("code_muse.command_line.load_context_completion.Path") as mock_path:
...
def test_file_path_completion_absolute_and_tilde():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        testfile = str(Path(tmpdir) / "testfile.txt")
...
def test_file_path_completion_permission_error():
    ...
    with patch(
        "code_muse.command_line.file_path_completion.glob.glob",
        side_effect=PermissionError("denied"),
    ):
...
def test_prompt_toolkit_main_block():
    ...
    import code_muse.command_line.prompt_toolkit_completion as mod
...
def test_pin_completion_empty_partial_model():
    ...
    from code_muse.command_line.pin_command_completion import PinCompleter
    ...
    with (
        patch(
            "code_muse.command_line.pin_command_completion.load_model_names",
            return_value=["gpt-4", "claude-3"],
        ),
        patch(
            "code_muse.command_line.pin_command_completion.load_agent_names",
...
async def test_diff_menu_keybindings():
    ...
    from code_muse.command_line.diff_menu import _split_panel_selector

    choices = ["option1", "option2"]
    on_change = MagicMock()
    get_preview = MagicMock(return_value=ANSI("preview"))

    with patch("code_muse.command_line.diff_menu.Application") as mock_app_cls:
        mock_app = AsyncMock()
...
def test_config_set_compaction_strategy_not_in_keys():
    ...
    with patch(
        "code_muse.command_line.config_commands.get_config_keys", return_value=[]
    ):
...
def test_config_pin_list_agents():
    ...
    from code_muse.command_line.config_commands import handle_pin_model_command
    ...
    with (
        ...
        patch("code_muse.agents.json_agent.discover_json_agents", return_value={}),
        patch(
            "code_muse.agents.agent_manager.get_agent_descriptions",
...
async def test_colors_menu_split_panel_selector():
    ...
    with patch("code_muse.command_line.colors_menu.Application") as mock_app_cls:
        mock_app = AsyncMock()
        mock_app_cls.return_value = mock_app
        mock_app.run_async = AsyncMock()
...
def test_colors_menu_get_preview_text():
    ...
    from code_muse.command_line.colors_menu import (
        ColorConfiguration,
...
def test_add_model_menu_run():
    ...
    with (
        patch("code_muse.command_line.add_model_menu.Application") as mock_app_cls,
        patch("code_muse.command_line.add_model_menu.set_awaiting_user_input"),
        patch("sys.stdout"),
        patch("time.sleep"),
    ):
        mock_app = MagicMock()
...
def test_model_settings_menu_keybindings():
    ...
    from code_muse.command_line.model_settings_menu import ModelSettingsMenu

    with patch(
        "code_muse.command_line.model_settings_menu._load_all_model_names",
        return_value=["gpt-4"],
    ):
...
def test_model_settings_start_editing_choice():
    ...
    with patch(
        "code_muse.command_line.model_settings_menu._get_setting_choices",
...
async def test_autosave_menu_keybindings():
    ...
    from code_muse.command_line.autosave_menu import interactive_autosave_picker

    with (
        patch(
            "code_muse.command_line.autosave_menu._get_session_entries",
            return_value=[
                ("session1", {"timestamp": "2024-01-01T12:00:00", "messages": 10}),
                ("session2", {"timestamp": "2024-01-02T13:00:00", "messages": 20}),
            ],
        ),
        patch("code_muse.command_line.autosave_menu.Application") as mock_app_cls,
        patch("code_muse.command_line.autosave_menu.set_awaiting_user_input"),
        patch("sys.stdout"),
...
def test_autosave_render_message_browser():
    ...
    from code_muse.command_line.autosave_menu import _render_message_browser_panel
...
def test_autosave_line_144():
    ...
    from code_muse.command_line.autosave_menu import _get_session_entries

    with tempfile.TemporaryDirectory() as tmpdir:
        session_file = str(Path(tmpdir) / "empty_session.json")
...
def test_autosave_line_317():
    ...
    from code_muse.command_line.autosave_menu import _render_preview_panel
...
def test_agent_menu_render_preview_no_entry():
    ...
    from code_muse.command_line.agent_menu import _render_preview_panel
...
async def test_agent_menu_interactive_picker():
    ...
    with (
        patch(
            "code_muse.command_line.agent_menu._get_agent_entries",
            return_value=[
                ("default", "Default Agent", "builtin"),
                ("custom", "Custom Agent", "json"),
            ],
        ),
        patch("code_muse.command_line.agent_menu.Application") as mock_app_cls,
        patch("code_muse.command_line.agent_menu.set_awaiting_user_input"),
        patch("sys.stdout"),
        patch("asyncio.sleep", new_callable=AsyncMock),
    ):
        mock_app = AsyncMock()
...
def test_uc_menu_toggle_tool_meta_not_found():
    ...
    from code_muse.command_line.uc_menu import _toggle_tool_enabled
    from code_muse.plugins.universal_constructor.models import ToolMeta, UCToolInfo
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".py", delete=False) as f:
        ...
        with patch("code_muse.command_line.uc_menu.emit_error"):
...
def test_uc_menu_delete_tool():
    ...
    from code_muse.command_line.uc_menu import _delete_tool
    from code_muse.plugins.universal_constructor.models import ToolMeta, UCToolInfo

    with tempfile.TemporaryDirectory() as tmpdir:
        tool_file = str(Path(tmpdir) / "sub" / "test_tool.py")
        Path(tool_file).parent.mkdir(parents=True)
        ...
        with (
            patch("code_muse.plugins.universal_constructor.USER_UC_DIR", Path(tmpdir)),
            patch("code_muse.command_line.uc_menu.emit_success"),
        ):
            result = _delete_tool(tool)
            assert result is True
            assert not Path(tool_file).exists()
...
def test_uc_menu_delete_tool_exception():
    ...
    with (
        patch("code_muse.command_line.uc_menu.Path") as mock_path,
        patch("code_muse.command_line.uc_menu.emit_error"),
    ):
...
def test_uc_menu_highlight_python_line():
    ...
    from code_muse.command_line.uc_menu import _highlight_python_line
...
def test_uc_menu_render_source_panel():
    ...
    from code_muse.command_line.uc_menu import _render_source_panel
    from code_muse.plugins.universal_constructor.models import ToolMeta, UCToolInfo
...
def test_uc_menu_render_preview_panel_with_author():
    ...
    from code_muse.command_line.uc_menu import _render_preview_panel
    from code_muse.plugins.universal_constructor.models import ToolMeta, UCToolInfo
...
def test_uc_menu_render_menu_panel_variants():
    ...
    from code_muse.command_line.uc_menu import _render_menu_panel
    from code_muse.plugins.universal_constructor.models import ToolMeta, UCToolInfo
...
async def test_uc_menu_interactive_picker():
    ...
    from code_muse.command_line.uc_menu import interactive_uc_picker
    from code_muse.plugins.universal_constructor.models import ToolMeta, UCToolInfo
...
def test_uc_load_source_code():
    ...
    from code_muse.command_line.uc_menu import _load_source_code
    from code_muse.plugins.universal_constructor.models import ToolMeta, UCToolInfo
...
def test_core_commands_shlex_fallback():
    ...
    from code_muse.command_line.core_commands import handle_cd_command

    # Unbalanced quotes will cause shlex.split to fail, triggering fallback
    with patch("code_muse.command_line.core_commands.emit_error"):
...
def test_onboarding_slides_import_error():
    ...
    with patch.dict("sys.modules", {"rich.text": None}):
        ...
        from code_muse.command_line import onboarding_slides
...
def test_file_path_completion_absolute_path():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        test_file = str(Path(tmpdir) / "testfile.txt")
...
def test_add_model_menu_non_tool_calling_confirm():
    ...
    with (
        patch("code_muse.command_line.add_model_menu.Application") as mock_app_cls,
        patch("code_muse.command_line.add_model_menu.set_awaiting_user_input"),
        patch("code_muse.command_line.add_model_menu.safe_input", return_value="y"),
        patch("code_muse.command_line.add_model_menu.emit_warning"),
        patch("sys.stdout"),
        patch("time.sleep"),
    ):
        mock_app = MagicMock()
