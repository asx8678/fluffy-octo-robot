...
class TestGetCommandsHelp:
    def test_lazy_import(self):
        from code_muse.command_line.session_commands import get_commands_help
...
class TestHandleSessionCommand:
    def _run(self, command):
        from code_muse.command_line.session_commands import handle_session_command

        return handle_session_command(command)

    def test_session_show_id(self):
        with (
            patch("code_muse.config.get_current_autosave_id", return_value="abc123"),
            patch(
    ...
    def test_session_id_subcommand(self):
        with (
            patch("code_muse.config.get_current_autosave_id", return_value="xyz"),
            patch(
                "code_muse.config.get_current_autosave_session_name",
                return_value="s",
            ),
    ...
    def test_session_new(self):
        with (
            patch("code_muse.config.rotate_autosave_id", return_value="new123"),
            patch("code_muse.messaging.emit_success") as mock_s,
        ):
            assert self._run("/session new") is True
            assert "new123" in mock_s.call_args[0][0]

    def test_session_invalid(self):
        with patch("code_muse.messaging.emit_warning") as mock_w:
...
class TestHandleCompactCommand:
    def _run(self, cmd="/compact"):
        from code_muse.command_line.session_commands import handle_compact_command
    ...
    def test_no_history(self):
        ...
        with (
            ...
            patch("code_muse.messaging.emit_warning") as mw,
        ):
    ...
    def test_truncation_strategy(self):
        ...
        with (
            ...
            patch("code_muse.config.get_protected_token_count", return_value=50),
            patch("code_muse.agents._compaction.truncate", return_value=["m3"]),
            patch("code_muse.messaging.emit_info"),
            patch("code_muse.messaging.emit_success") as ms,
        ):
            assert self._run() is True
    ...
    def test_compaction_fails(self):
        ...
        with (
            ...
            patch("code_muse.config.get_protected_token_count", return_value=50),
            patch("code_muse.messaging.emit_info"),
            patch("code_muse.messaging.emit_error") as me,
        ):
            assert self._run() is True
    ...
    def test_zero_before_tokens(self):
        ...
        with (
            ...
            patch("code_muse.config.get_protected_token_count", return_value=0),
            patch("code_muse.messaging.emit_info"),
...
class TestHandleTruncateCommand:
    def _run(self, cmd):
        from code_muse.command_line.session_commands import handle_truncate_command
...
class TestHandleAutosaveLoadCommand:
    def test_returns_marker(self):
        from code_muse.command_line.session_commands import (
...
class TestHandleDumpContextCommand:
    ...
    def test_success(self):
        ...
        with (
            ...
            patch(
                "code_muse.command_line.session_commands.save_session",
                return_value=meta,
            ),
            patch("code_muse.messaging.emit_success"),
        ):
...
class TestHandleLoadContextCommand:
    ...
    def test_file_not_found_with_available(self):
        with (
            patch(
                "code_muse.command_line.session_commands.load_session",
                side_effect=FileNotFoundError(),
            ),
            patch(
                "code_muse.command_line.session_commands.list_sessions",
                return_value=["s1"],
            ),
            patch("code_muse.messaging.emit_error"),
            patch("code_muse.messaging.emit_info") as mi,
        ):
    ...
    def test_success(self):
        ...
        with (
            ...
            patch("code_muse.config.rotate_autosave_id", return_value="new_id"),
            patch("code_muse.messaging.emit_success"),
            patch("code_muse.command_line.autosave_menu.display_resumed_history"),
        ):
    ...
    def test_success_rotate_fails(self):
        ...
        with (
            ...
            patch(
                "code_muse.config.rotate_autosave_id",
