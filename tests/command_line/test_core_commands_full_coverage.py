"""Full coverage tests for code_muse/command_line/core_commands.py."""
...
class TestGetCommandsHelp:
    def test_lazy_import(self):
        from code_muse.command_line.core_commands import get_commands_help

        with patch(
            "code_muse.command_line.command_handler.get_commands_help",
            return_value="help",
        ):
...
class TestHandleHelpCommand:
    def test_help(self):
        from code_muse.command_line.core_commands import handle_help_command
...
class TestHandleToolsCommand:
    def test_tools(self):
        from code_muse.command_line.core_commands import handle_tools_command
...
class TestHandlePasteCommand:
    def test_no_image(self):
        from code_muse.command_line.core_commands import handle_paste_command

        with (
            patch(
                "code_muse.command_line.clipboard.has_image_in_clipboard",
                return_value=False,
            ),
    ...
    def test_success(self):
        ...
        with (
            ...
            patch(
                "code_muse.command_line.clipboard.capture_clipboard_image_to_pending",
                return_value="img.png",
            ),
            patch(
                "code_muse.command_line.clipboard.get_clipboard_manager",
...
class TestHandleTutorialCommand:
    def test_chatgpt(self):
        from code_muse.command_line.core_commands import handle_tutorial_command

        with (
            patch("code_muse.command_line.onboarding_wizard.reset_onboarding"),
            patch("concurrent.futures.ThreadPoolExecutor") as pool,
            patch("code_muse.messaging.emit_info"),
            patch("code_muse.plugins.chatgpt_oauth.oauth_flow.run_oauth_flow"),
            patch("code_muse.model_switching.set_model_and_reload_agent"),
        ):
    ...
    def test_claude(self):
        ...
        with (
            ...
            patch("code_muse.messaging.emit_info"),
            patch(
                "code_muse.plugins.claude_code_oauth.register_callbacks._perform_authentication"
            ),
            patch("code_muse.model_switching.set_model_and_reload_agent"),
        ):
            mock_future = MagicMock()
...
class TestHandleExitCommand:
    def test_exit(self):
        from code_muse.command_line.core_commands import handle_exit_command
    ...
    def test_exit_error(self):
        ...
        with patch("code_muse.messaging.emit_success", side_effect=Exception):
            assert handle_exit_command("/exit") is True


class TestHandleAgentCommand:
    def _mock_agent(self, name="muse", display="Muse", desc="A dog"):
        a = MagicMock()
        a.name = name
    ...
    def test_no_args_interactive_select(self):
        from code_muse.command_line.core_commands import handle_agent_command
        ...
        with (
            ...
            patch(
                "code_muse.command_line.core_commands.finalize_autosave_session",
                return_value="sess1",
            ),
    ...
    def test_no_args_already_current(self):
        ...
        with (
            patch("concurrent.futures.ThreadPoolExecutor") as pool,
            patch("code_muse.agents.get_current_agent", return_value=agent),
            patch("code_muse.messaging.emit_info"),
        ):
            mock_future = MagicMock()
            mock_future.result.return_value = "muse"
            pool.return_value.__enter__ = MagicMock(return_value=pool.return_value)
            pool.return_value.__exit__ = MagicMock(return_value=False)
            pool.return_value.submit.return_value = mock_future
            assert handle_agent_command("/agent") is True
    ...
    def test_no_args_picker_fails_fallback(self):
        ...
        with (
            ...
            patch(
                ...
                return_value={"muse": "Muse"},
            ),
            patch(
                "code_muse.agents.get_agent_descriptions",
                return_value={"muse": "desc"},
            ),
            patch("code_muse.messaging.emit_warning"),
            patch("code_muse.messaging.emit_info"),
        ):
    ...
    def test_with_name_switch(self):
        ...
        with (
            ...
            patch(
                "code_muse.agents.get_available_agents",
                return_value={"muse": "CP", "other": "Other"},
            ),
            patch(
                "code_muse.command_line.core_commands.finalize_autosave_session",
                return_value="s",
    ...
    def test_with_name_already_current(self):
        ...
        with (
            ...
            assert handle_agent_command("/agent muse") is True
...
class TestHandleModelCommand:
    def test_no_args_interactive_select(self):
        from code_muse.command_line.core_commands import handle_model_command

        with (
            patch("concurrent.futures.ThreadPoolExecutor") as pool,
            patch("code_muse.command_line.model_picker_completion.set_active_model"),
            patch("code_muse.messaging.emit_success"),
        ):
            mock_future = MagicMock()
...
class TestHandleAddModelCommand:
    def test_success(self):
        from code_muse.command_line.core_commands import handle_add_model_command

        with (
            patch("code_muse.tools.command_runner.set_awaiting_user_input"),
            patch(
                "code_muse.command_line.add_model_menu.interactive_model_picker",
                return_value=True,
            ),
            patch("code_muse.messaging.emit_info"),
        ):
...
class TestHandleModelSettingsCommand:
    def test_show_flag(self):
        from code_muse.command_line.core_commands import handle_model_settings_command

        with patch(
            "code_muse.command_line.model_settings_menu.show_model_settings_summary"
        ):
    ...
    def test_interactive_success(self):
        ...
        with (
            patch("code_muse.tools.command_runner.set_awaiting_user_input"),
            patch(
                "code_muse.command_line.model_settings_menu.interactive_model_settings",
                return_value=True,
            ),
            patch("code_muse.messaging.emit_success"),
            patch("code_muse.messaging.emit_info"),
...
class TestHandleWiggumCommand:
    def test_no_prompt(self):
        from code_muse.command_line.core_commands import handle_wiggum_command
    ...
    def test_with_prompt(self):
        ...
        with (
            patch("code_muse.command_line.wiggum_state.start_wiggum"),
            patch("code_muse.messaging.emit_success"),
            patch("code_muse.messaging.emit_info"),
        ):
...
class TestHandleWiggumStopCommand:
    def test_active(self):
        from code_muse.command_line.core_commands import handle_wiggum_stop_command

        with (
            patch(
                "code_muse.command_line.wiggum_state.is_wiggum_active",
                return_value=True,
            ),
            patch("code_muse.command_line.wiggum_state.stop_wiggum"),
            patch("code_muse.messaging.emit_success"),
        ):
