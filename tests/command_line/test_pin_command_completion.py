...
class TestGetPinnedModelForAgent:
    def test_from_config(self):
        ...
        with patch("code_muse.config.get_agent_pinned_model", return_value="gpt-4"):
    ...
    def test_config_exception(self):
        ...
        with (
            patch(
                "code_muse.config.get_agent_pinned_model",
                side_effect=Exception("fail"),
            ),
            patch(
                "code_muse.agents.json_agent.discover_json_agents",
...
class TestGetModelDisplayMeta:
    def test_with_pinned_agents(self):
        ...
        with (
            patch(
                "code_muse.config.get_agents_pinned_to_model",
                return_value=["a1"],
            ),
            patch(
                "code_muse.command_line.pin_command_completion._get_json_agents_for_model",
                return_value=["a2"],
            ),
        ):
    ...
    def test_no_pinned(self):
        ...
        with (
            patch("code_muse.config.get_agents_pinned_to_model", return_value=[]),
            patch(
                "code_muse.command_line.pin_command_completion._get_json_agents_for_model",
                return_value=[],
            ),
...
class TestGetAgentDisplayMeta:
    def test_with_pinned_model(self):
        ...
        with patch(
            "code_muse.command_line.pin_command_completion._get_pinned_model_for_agent",
            return_value="gpt-4",
        ):
...
class TestLoadModelNames:
    def test_delegates(self):
        from code_muse.command_line.pin_command_completion import load_model_names
...
class TestPinCompleter:
    ...
    def test_agent_then_space_shows_models(self):
        ...
        with (
            ...
            patch(
                "code_muse.command_line.pin_command_completion._get_model_display_meta",
                return_value="Model",
            ),
        ):
            completions = list(
