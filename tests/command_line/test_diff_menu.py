...
class TestLanguageSamples:
    ...
    def test_all_supported_languages_have_samples(self):
        ...
        from code_muse.command_line.diff_menu import LANGUAGE_SAMPLES
    ...
    def test_diff_samples_are_well_formatted(self):
        ...
        for _lang, (filename, diff) in LANGUAGE_SAMPLES.items():
            # Check diff format
...
class TestDiffConfiguration:
    ...
    @patch("code_muse.config.get_diff_addition_color")
    @patch("code_muse.config.get_diff_deletion_color")
...
class TestPreviewTextGeneration:
    ...
    @patch("code_muse.tools.common.format_diff_with_colors")
...
class TestSplitPanelSelector:
    ...
    async def test_basic_selector_functionality(self, mock_sleep, mock_stdout):
        ...
        with patch("code_muse.command_line.diff_menu.Application") as mock_app:
    ...
    async def test_left_panel_text_generation(self):
        ...
        with patch("code_muse.command_line.diff_menu.Application") as mock_app:
            ...
            with patch(
                "code_muse.command_line.diff_menu.Application", side_effect=capture_app
            ):
...
class TestColorMenuHandler:
    ...
    @pytest.mark.asyncio
    @patch("code_muse.command_line.diff_menu._split_panel_selector")
    ...
    @pytest.mark.asyncio
    @patch(
        "code_muse.command_line.diff_menu._split_panel_selector",
...
class TestInteractiveDiffPicker:
    ...
    async def test_complete_flow_with_changes(
        ...
        with patch(
            "code_muse.command_line.diff_menu._handle_color_menu",
...
class TestIntegrationScenarios:
        ...
        for lang_index in range(len(SUPPORTED_LANGUAGES)):
            ...
            try:
                ...
                with (
                    ...
                    with patch("code_muse.config.set_diff_deletion_color"):
                        with patch(
                            "code_muse.config.get_diff_addition_color",
                            return_value="#00ff00",
                        ):
                            with patch(
                                "code_muse.config.get_diff_deletion_color",
                                return_value="#ff0000",
                            ):
                                result = _get_preview_text_for_prompt_toolkit(config)
                                assert result is not None
    ...
    async def test_multiple_color_selections_and_language_switching(self):
        ...
        with (
            ...
            patch("code_muse.command_line.diff_menu._handle_color_menu"),
        ):
