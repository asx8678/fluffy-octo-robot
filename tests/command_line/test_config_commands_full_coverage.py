"""Full coverage tests for code_muse/command_line/config_commands.py."""
...
class TestGetCommandsHelp:
    def test_lazy_import(self):
        from code_muse.command_line.config_commands import get_commands_help
...
class TestHandleShowCommand:
    def _show_patches(self, effective_temp=0.7, global_temp=0.7, yolo=True):
        ...
        return [
            ...
            patch("code_muse.config.get_agent_name", return_value="Pup"),
            patch("code_muse.config.get_owner_name", return_value="Owner"),
            patch("code_muse.config.get_yolo_mode", return_value=yolo),
            patch("code_muse.config.get_auto_save_session", return_value=True),
            patch("code_muse.config.get_protected_token_count", return_value=50000),
            patch("code_muse.config.get_compaction_threshold", return_value=0.85),
            patch(
                "code_muse.config.get_compaction_strategy", return_value="truncation"
            ),
            patch("code_muse.config.get_temperature", return_value=global_temp),
            patch(
                "code_muse.config.get_effective_temperature",
                return_value=effective_temp,
            ),
            patch("code_muse.config.get_default_agent", return_value="planning-agent"),
            patch("code_muse.config.get_resume_message_count", return_value=50),
            patch(
                "code_muse.config.get_openai_reasoning_effort", return_value="medium"
            ),
            patch("code_muse.config.get_openai_verbosity", return_value="medium"),
            patch(
                "code_muse.keymap.get_cancel_agent_display_name", return_value="ctrl+c"
            ),
            patch("code_muse.messaging.emit_info"),
        ]

    def test_show_command(self):
        from code_muse.command_line.config_commands import handle_show_command
...
class TestHandleReasoningCommand:
    def test_no_args(self):
        from code_muse.command_line.config_commands import handle_reasoning_command

        with patch("code_muse.messaging.emit_warning") as warn:
    ...
    def test_valid(self):
        ...
        mock_agent = MagicMock()
        with (
            patch("code_muse.config.set_openai_reasoning_effort"),
            patch("code_muse.config.get_openai_reasoning_effort", return_value="high"),
            patch(
...
class TestHandleVerbosityCommand:
    def test_no_args(self):
        from code_muse.command_line.config_commands import handle_verbosity_command
    ...
    def test_valid(self):
        ...
        mock_agent = MagicMock()
        with (
            patch("code_muse.config.set_openai_verbosity"),
            patch("code_muse.config.get_openai_verbosity", return_value="low"),
            patch(
                "code_muse.agents.agent_manager.get_current_agent",
                return_value=mock_agent,
            ),
    ...
    def test_invalid(self):
        ...
        with (
            patch(
                "code_muse.config.set_openai_verbosity", side_effect=ValueError("bad")
            ),
...
class TestHandleSetCommand:
    ...
    def test_agent_reload_failure(self):
        ...
        with (
            ...
            patch("code_muse.messaging.emit_success"),
            patch("code_muse.messaging.emit_info"),
            patch("code_muse.messaging.emit_warning") as warn,
            patch("code_muse.agents.get_current_agent", return_value=mock_agent),
        ):
            assert handle_set_command("/set key value") is True
            warn.assert_called_once()
...
class TestGetJsonAgentsPinnedToModel:
    def test_returns_pinned(self, tmp_path):
        from code_muse.command_line.config_commands import (
...
class TestHandlePinModelCommand:
    def _make_patches(self, **overrides):
        defaults = {
            ...
            "get_agent_descriptions": {"muse": "Default agent"},
        }
    ...
    def test_pin_builtin_agent(self):
        ...
        mock_agent = MagicMock()
        mock_agent.name = "muse"
        patches = self._make_patches()
        with (
            patches[0],
            patches[1],
            patches[2],
            patch("code_muse.config.set_agent_pinned_model"),
            patch("code_muse.messaging.emit_success"),
            patch("code_muse.messaging.emit_info"),
            patch("code_muse.agents.get_current_agent", return_value=mock_agent),
        ):
            assert handle_pin_model_command("/pin_model muse gpt-5") is True
...
class TestHandleUnpinCommand:
    def test_no_args(self):
        ...
        with (
            ...
            patch("code_muse.config.get_agent_pinned_model", return_value=None),
            patch("code_muse.messaging.emit_warning"),
            patch("code_muse.messaging.emit_info"),
        ):
    ...
    def test_no_args_with_pinned(self):
        ...
        with (
            ...
            patch("code_muse.config.get_agent_pinned_model", return_value="gpt-5"),
    ...
    def test_unpin_builtin(self):
        ...
        with (
            ...
            patch("code_muse.config.clear_agent_pinned_model"),
            patch("code_muse.messaging.emit_success"),
            patch("code_muse.agents.get_current_agent", return_value=mock_agent),
            patch("code_muse.messaging.emit_info"),
        ):
            assert handle_unpin_command("/unpin muse") is True
...
class TestHandleDiffCommand:
    ...
    def test_with_result(self):
        from code_muse.command_line.config_commands import handle_diff_command

        pool = self._pool_mock({"add_color": "green", "del_color": "red"})
        with (
            patch("concurrent.futures.ThreadPoolExecutor", return_value=pool),
            patch("code_muse.config.set_diff_addition_color"),
            patch("code_muse.config.set_diff_deletion_color"),
        ):
    ...
    def test_error_applying(self):
        ...
        with (
            patch("concurrent.futures.ThreadPoolExecutor", return_value=pool),
            patch(
                "code_muse.config.set_diff_addition_color",
...
class TestHandleColorsCommand:
    ...
    def test_with_result(self):
        from code_muse.command_line.config_commands import handle_colors_command

        pool = self._pool_mock({"thinking": "red"})
        with (
            patch("concurrent.futures.ThreadPoolExecutor", return_value=pool),
            patch("code_muse.config.set_banner_color"),
            patch("code_muse.messaging.emit_success"),
        ):
    ...
    def test_error_applying(self):
        ...
        with (
            patch("concurrent.futures.ThreadPoolExecutor", return_value=pool),
            patch("code_muse.config.set_banner_color", side_effect=Exception("fail")),
            patch("code_muse.messaging.emit_error"),
        ):
            assert handle_colors_command("/colors") is True
...
class TestShowColorOptions:
    def test_additions(self):
        from code_muse.command_line.config_commands import _show_color_options
