...
class TestSafeFileHistory:
    def test_store_string_success(self, tmp_path):
        from code_muse.command_line.prompt_toolkit_completion import SafeFileHistory
...
class TestSetCompleter:
    ...
    def test_just_trigger_suggests_space(self):
        from code_muse.command_line.prompt_toolkit_completion import SetCompleter
    ...
    def test_shows_config_keys(self):
        ...
        with (
            patch(
                "code_muse.command_line.prompt_toolkit_completion.get_config_keys",
                return_value=["debug", "model", "yolo_mode", "muse_token"],
            ),
            patch(
                "code_muse.command_line.prompt_toolkit_completion.get_value",
                return_value="false",
            ),
        ):
            completions = list(c.get_completions(self._make_doc("/set "), None))
            texts = [c.text for c in completions]
            # model and muse_token excluded
            assert any("debug" in t for t in texts)
            assert not any("model" in t for t in texts)
            assert not any("muse_token" in t for t in texts)
...
class TestCDCompleter:
    ...
    def test_no_trigger(self):
        from code_muse.command_line.prompt_toolkit_completion import CDCompleter
    ...
    def test_completes_directories(self, tmp_path):
        ...
        c = CDCompleter()
        with patch(
            "code_muse.command_line.prompt_toolkit_completion.list_directory",
            return_value=(["mydir"], []),
        ):
...
class TestAgentCompleter:
    ...
    def test_no_trigger(self):
        from code_muse.command_line.prompt_toolkit_completion import AgentCompleter
    ...
    def test_shows_agents(self):
        ...
        with (
            ...
            patch(
                "code_muse.command_line.pin_command_completion._get_agent_display_meta",
...
class TestSlashCompleter:
    ...
    def test_no_slash(self):
        from code_muse.command_line.prompt_toolkit_completion import SlashCompleter
    ...
    def test_just_slash(self):
        ...
        with (
            patch(
                "code_muse.command_line.prompt_toolkit_completion.get_unique_commands",
    ...
    def test_custom_commands_list_format(self):
        ...
        with (
            ...
            patch("code_muse.plugins.load_plugin_callbacks"),
            patch(
                "code_muse.callbacks.on_custom_command_help",
    ...
    def test_custom_commands_exception(self):
        ...
        with (
            ...
            patch(
                "code_muse.plugins.load_plugin_callbacks",
                side_effect=Exception("fail"),
            ),
        ):
            # Should not raise
...
class TestGetPromptWithActiveModel:
    def test_basic(self):
        ...
        with (
            ...
            patch(
                "code_muse.config.get_agent_name",
                return_value="Biscuit",
            ),
            patch(
                "code_muse.command_line.prompt_toolkit_completion.get_active_model",
                return_value="gpt-4",
            ),
        ):
    ...
    def test_cwd_home_runtime_error(self):
        from code_muse.command_line.prompt_toolkit_completion import (
            get_prompt_with_active_model,
        )
...
class TestGetInputWithCombinedCompletion:
    ...
    async def test_basic(self):
        ...
        with (
            ...
            patch(
                "code_muse.command_line.prompt_toolkit_completion.PromptSession"
            ) as mock_cls,
        ):
...
class TestKeyBindings:
    ...
    def test_bracketed_paste_empty_with_image(self, captured_bindings):
        ...
        with (
            patch(
                "code_muse.command_line.prompt_toolkit_completion.has_image_in_clipboard",
                return_value=True,
            ),
            patch(
                "code_muse.command_line.prompt_toolkit_completion.capture_clipboard_image_to_pending",
    ...
    async def test_ctrl_v_with_image(self, captured_bindings):
    ...
    async def test_ctrl_v_no_image_macos(self, captured_bindings):
        handler = self._find_handler(captured_bindings, "c-v")
        event = MagicMock()
    ...
    async def test_ctrl_v_no_image_windows(self, captured_bindings):
        handler = self._find_handler(captured_bindings, "c-v")
        event = MagicMock()
    ...
    async def test_ctrl_v_no_image_linux(self, captured_bindings):
        handler = self._find_handler(captured_bindings, "c-v")
        event = MagicMock()
    ...
    async def test_ctrl_v_no_image_linux_xsel_fallback(self, captured_bindings):
        handler = self._find_handler(captured_bindings, "c-v")
        event = MagicMock()
    ...
    async def test_ctrl_v_exception(self, captured_bindings):
        handler = self._find_handler(captured_bindings, "c-v")
        event = MagicMock()
...
class TestAttachmentPlaceholderProcessorPaths:
    ...
    def test_no_detections(self):
        ...
        with patch(
            "code_muse.command_line.prompt_toolkit_completion._detect_path_tokens",
            return_value=([], []),
        ):
            result = proc.apply_transformation(mock_ti)
            assert result is not None
    ...
    def test_with_image_detection(self):
        ...
        with (
            ...
            patch(
                "code_muse.command_line.prompt_toolkit_completion._tokenise",
...
class TestClipboardCoverageAdditions:
    ...
    def test_safe_open_image_pil_unavailable(self):
        from code_muse.command_line.clipboard import _safe_open_image

        with patch("code_muse.command_line.clipboard.PIL_AVAILABLE", False):
    ...
    def test_safe_open_image_decompression_bomb(self):
        ...
        with (
            patch("code_muse.command_line.clipboard.PIL_AVAILABLE", True),
            patch("code_muse.command_line.clipboard.Image") as mock_img_mod,
        ):
    ...
    def test_get_linux_clipboard_image_no_tool(self):
        from code_muse.command_line.clipboard import _get_linux_clipboard_image

        with patch(
            "code_muse.command_line.clipboard._check_linux_clipboard_tool",
            return_value=None,
        ):
    ...
    def test_get_linux_clipboard_image_wl_paste(self):
        ...
        with (
            ...
            patch(
                "code_muse.command_line.clipboard.subprocess.run",
    ...
    def test_has_image_linux_no_tool(self):
        from code_muse.command_line.clipboard import has_image_in_clipboard

        with (
            patch("code_muse.command_line.clipboard.sys.platform", "linux"),
            patch(
                "code_muse.command_line.clipboard._check_linux_clipboard_tool",
                return_value=None,
            ),
    ...
    def test_get_clipboard_image_linux_large_no_pil(self):
        from code_muse.command_line.clipboard import get_clipboard_image
        ...
        with (
            patch("code_muse.command_line.clipboard.sys.platform", "linux"),
            patch(
                "code_muse.command_line.clipboard._get_linux_clipboard_image",
                return_value=large_bytes,
            ),
            patch("code_muse.command_line.clipboard.PIL_AVAILABLE", False),
        ):
    ...
    def test_get_clipboard_image_linux_large_with_pil(self):
        ...
        with (
            ...
            patch("code_muse.command_line.clipboard.PIL_AVAILABLE", True),
            patch(
                "code_muse.command_line.clipboard._safe_open_image",
                return_value=mock_img,
            ),
            patch(
                "code_muse.command_line.clipboard._resize_image_if_needed",
                return_value=mock_img,
            ),
        ):
    ...
    def test_get_clipboard_image_image_mode_conversion(self):
        ...
        with (
            patch("code_muse.command_line.clipboard.sys.platform", "darwin"),
            patch("code_muse.command_line.clipboard.PIL_AVAILABLE", True),
            patch("code_muse.command_line.clipboard.ImageGrab") as mock_grab,
            patch("code_muse.command_line.clipboard.Image") as mock_img_mod,
        ):
    ...
    def test_get_pending_images_no_binary_content(self):
        from code_muse.command_line.clipboard import ClipboardAttachmentManager

        mgr = ClipboardAttachmentManager()
        mgr.add_image(b"fake")

        with patch("code_muse.command_line.clipboard.BINARY_CONTENT_AVAILABLE", False):
            assert mgr.get_pending_images() == []

    def test_check_linux_clipboard_tool_timeout(self):
        import subprocess

        from code_muse.command_line.clipboard import _check_linux_clipboard_tool
    ...
    def test_resize_max_dimension_width(self):
        ...
        from code_muse.command_line.clipboard import _resize_image_if_needed
        ...
        with patch("code_muse.command_line.clipboard.Image") as mock_mod:
