"""Full coverage tests for code_muse/tools/common.py.
...
class TestShouldIgnorePath:
    def test_git_directory(self):
        from code_muse.tools.common import should_ignore_path
...
class TestShouldIgnoreDirPath:
    def test_git_dir(self):
        from code_muse.tools.common import should_ignore_dir_path
...
class TestGetLexerForExtension:
    def test_python_extension(self):
        from code_muse.tools.common import _get_lexer_for_extension
    ...
    def test_no_pygments(self):
        import code_muse.tools.common as mod
    ...
    def test_get_lexer_by_name_exception(self):
        ...
        from code_muse.tools.common import PYGMENTS_AVAILABLE, _get_lexer_for_extension

        if not PYGMENTS_AVAILABLE:
            pytest.skip("Pygments not available")
        with patch(
            "code_muse.tools.common.get_lexer_by_name",
...
class TestGetTokenColor:
    def test_returns_default_for_unknown(self):
        from code_muse.tools.common import _get_token_color
    ...
    def test_keyword_token(self):
        from code_muse.tools.common import PYGMENTS_AVAILABLE, _get_token_color
...
class TestHighlightCodeLine:
    ...
    def test_with_lexer_none(self):
        from code_muse.tools.common import _highlight_code_line
...
class TestExtractFileExtensionFromDiff:
    def test_with_python_file(self):
        from code_muse.tools.common import _extract_file_extension_from_diff
...
class TestBrightenHex:
    def test_no_change(self):
        from code_muse.tools.common import brighten_hex
...
class TestFormatDiffWithSyntaxHighlighting:
    def test_basic_diff(self):
        from code_muse.tools.common import _format_diff_with_syntax_highlighting
...
class TestFindBestWindow:
    def test_exact_match(self):
        from code_muse.tools.common import _find_best_window
...
class TestArrowSelectAsync:
    ...
    async def test_basic_selection(self):
        ...
        with patch("code_muse.tools.common.Application") as MockApp:
            ...
            with patch("code_muse.tools.common.arrow_select_async") as mock_sel:
...
class TestArrowSelect:
    def test_raises_in_async_context(self):
        ...
        from code_muse.tools.common import arrow_select
...
class TestGetUserApproval:
    def setup_method(self):
        self._auto_approve_patch = patch(
            "code_muse.config.get_auto_approve", return_value=False
        )
        self._auto_approve_patch.start()

    def teardown_method(self):
        self._auto_approve_patch.stop()

    def test_approve(self):
        from code_muse.tools.common import get_user_approval

        with patch("code_muse.tools.common.arrow_select", return_value="✓ Approve"):
            with patch("code_muse.tools.common.Console"):
                with patch("code_muse.tools.command_runner.set_awaiting_user_input"):
                    with patch("code_muse.tools.common.emit_info"):
                        with patch("code_muse.tools.common.emit_success"):
    ...
    def test_reject(self):
        ...
        with patch("code_muse.tools.common.arrow_select", return_value="✗ Reject"):
            with patch("code_muse.tools.common.Console"):
                with patch("code_muse.tools.command_runner.set_awaiting_user_input"):
                    with patch("code_muse.tools.common.emit_info"):
                        with patch("code_muse.tools.common.emit_error"):
                            confirmed, feedback = get_user_approval(
                                "Test", "content", agent_name="Biscuit"
                            )
    ...
    def test_reject_with_feedback(self):
        ...
        with (
            patch(
                "code_muse.tools.common.arrow_select",
                return_value="💬 Reject with feedback (tell Biscuit what to change)",
            ),
            patch("code_muse.tools.common.Prompt") as MockPrompt,
        ):
            ...
            with (
                ...
                patch("code_muse.tools.common.emit_info"),
            ):
                with patch("code_muse.tools.common.emit_error"):
                    with patch("code_muse.tools.common.emit_warning"):
                        confirmed, feedback = get_user_approval(
                            "Test", "content", agent_name="Biscuit"
                        )
        assert confirmed is False
    ...
    def test_reject_with_empty_feedback(self):
        ...
        with patch("code_muse.tools.common.arrow_select", return_value="💬 feedback"):
            with patch("code_muse.tools.common.Prompt") as MockPrompt:
    ...
    def test_eof_error(self):
        ...
        with patch("code_muse.tools.common.arrow_select", side_effect=EOFError):
            with patch("code_muse.tools.common.Console"):
                with patch("code_muse.tools.command_runner.set_awaiting_user_input"):
                    with patch("code_muse.tools.common.emit_info"):
                        with patch("code_muse.tools.common.emit_error"):
            ...
            with patch("code_muse.tools.common.Console"):
                with patch("code_muse.tools.command_runner.set_awaiting_user_input"):
                    with patch("code_muse.tools.common.emit_info"):
                        with patch("code_muse.tools.common.emit_success"):
                            with patch(
                                "code_muse.tools.common.format_diff_with_colors",
            ...
            with patch("code_muse.tools.common.Console"):
                with patch("code_muse.tools.command_runner.set_awaiting_user_input"):
                    with patch("code_muse.tools.common.emit_info"):
                        with patch("code_muse.tools.common.emit_success"):
                            with patch(
                                "code_muse.config.get_agent_name", return_value="buddy"
                            ):
...
class TestModuleConstants:
    def test_extension_to_lexer_name(self):
        from code_muse.tools.common import EXTENSION_TO_LEXER_NAME

        assert ".py" in EXTENSION_TO_LEXER_NAME
        assert EXTENSION_TO_LEXER_NAME[".py"] == "python"

    def test_token_colors_dict(self):
        from code_muse.tools.common import PYGMENTS_AVAILABLE, TOKEN_COLORS
