"""Full coverage tests for code_muse/tools/command_runner.py.
...
class TestTruncateLine:
    def test_short_line(self):
        from code_muse.tools.command_runner import _truncate_line

        assert _truncate_line("hello") == "hello"

    def test_exact_limit(self):
        from code_muse.tools.command_runner import MAX_LINE_LENGTH, _truncate_line
...
class TestProcessRegistration:
    ...
    def test_unregister_nonexistent(self):
        from code_muse.tools.command_runner import _unregister_process
...
class TestKillProcessGroup:
    ...
    def test_exception_in_kill(self):
        ...
        with patch("code_muse.tools.command_runner.emit_error"):
            # Should not raise
            _kill_process_group(proc)
...
class TestKillAllRunningShellProcesses:
    def test_kills_processes(self):
        ...
        with patch("code_muse.tools.command_runner._kill_process_group"):
...
class TestAwaitingUserInput:
    def test_set_and_check(self):
        ...
        with patch("code_muse.tools.command_runner.pause_all_spinners", create=True):
            pass
...
class TestSpawnCtrlXKeyListener:
    def test_no_tty(self):
        from code_muse.tools.command_runner import _spawn_ctrl_x_key_listener
...
class TestKeyboardContext:
    def test_acquire_release(self):
        import code_muse.tools.command_runner as mod
...
class TestShellCommandKeyboardContext:
    def test_context_manager(self):
        from code_muse.tools.command_runner import _shell_command_keyboard_context

        with (
            patch(
                "code_muse.tools.command_runner._spawn_ctrl_x_key_listener",
                return_value=None,
            ),
...
class TestRunShellCommandStreaming:
    def test_successful_command(self):
        ...
        with patch("code_muse.tools.command_runner.emit_shell_line"):
            with patch("code_muse.tools.command_runner.get_message_bus") as mock_bus:
    ...
    def test_silent_mode(self):
        ...
        with patch("code_muse.tools.command_runner.emit_shell_line") as mock_emit:
            result = run_shell_command_streaming(
...
class TestRunShellCommand:
    ...
    async def test_empty_command(self):
        from code_muse.tools.command_runner import run_shell_command

        ctx = MagicMock(spec=RunContext)
        with patch(
            "code_muse.callbacks.on_run_shell_command",
    ...
    async def test_yolo_mode_executes(self):
        ...
        with (
            ...
            patch("code_muse.tools.command_runner.is_subagent", return_value=False),
            patch(
                "code_muse.tools.command_runner._execute_shell_command",
                new_callable=AsyncMock,
    ...
    async def test_subagent_runs_silently(self):
        ...
        with (
            ...
            patch("code_muse.config.get_yolo_mode", return_value=False),
            patch("code_muse.tools.command_runner.is_subagent", return_value=True),
            patch(
                "code_muse.tools.command_runner._execute_shell_command",
                new_callable=AsyncMock,
                return_value=mock_output,
    ...
    async def test_background_command(self):
        ...
        with (
            ...
            with patch("code_muse.tools.command_runner.get_message_bus") as mock_bus:
                mock_bus.return_value = MagicMock()
                with patch("code_muse.tools.command_runner.emit_info"):
    ...
    async def test_user_rejected(self):
        ...
        with (
            ...
            with (
                patch(
                    "code_muse.tools.command_runner.get_user_approval_async",
                    new_callable=AsyncMock,
                    return_value=(False, None),
                ),
                patch("code_muse.config.get_agent_name", return_value="buddy"),
            ):
                result = await run_shell_command(ctx, "echo hi", timeout=10)
        ...
        try:
            with patch(
                ...
                with patch("code_muse.config.get_yolo_mode", return_value=False):
                    with patch(
                        "code_muse.tools.command_runner.is_subagent",
                        return_value=False,
                    ):
                        with patch("sys.stdin") as mock_stdin:
                            mock_stdin.isatty.return_value = True
...
class TestExecuteShellCommand:
    ...
    async def test_executes(self):
        ...
        with patch("code_muse.tools.command_runner.get_message_bus") as mock_bus:
            mock_bus.return_value = MagicMock()
            with patch("code_muse.messaging.spinner.pause_all_spinners"):
                with patch("code_muse.messaging.spinner.resume_all_spinners"):
                    with patch(
                        "code_muse.tools.command_runner._acquire_keyboard_context"
                    ):
                        with patch(
                            "code_muse.tools.command_runner._release_keyboard_context"
                        ):
                            with patch(
                                "code_muse.tools.command_runner._run_command_inner",
                                new_callable=AsyncMock,
...
class TestRunCommandInner:
    ...
    async def test_exception(self):
        from code_muse.tools.command_runner import _run_command_inner

        with (
            patch(
                "code_muse.tools.command_runner._run_command_sync",
                side_effect=Exception("boom"),
            ),
            patch("code_muse.tools.command_runner.emit_error"),
        ):
            try:
...
class TestShareYourReasoning:
    def test_basic(self):
        from code_muse.tools.command_runner import share_your_reasoning
...
class TestReasoningOutput:
    def test_default(self):
        from code_muse.tools.command_runner import ReasoningOutput
...
class TestSignalHandlers:
    def test_handle_ctrl_x_press(self):
        from code_muse.tools.command_runner import _handle_ctrl_x_press

        with (
            patch("code_muse.tools.command_runner.emit_warning"),
            patch(
                "code_muse.tools.command_runner.kill_all_running_shell_processes",
                return_value=0,
            ),
        ):
    ...
    def test_shell_sigint_handler(self):
        from code_muse.tools.command_runner import _shell_sigint_handler
...
class TestWin32PipeHasData:
    def test_posix_stub_returns_false(self):
        if not sys.platform.startswith("win"):
            from code_muse.tools.command_runner import _win32_pipe_has_data
...
class TestRunCommandSync:
    def test_basic_sync(self):
        from code_muse.tools.command_runner import _run_command_sync
