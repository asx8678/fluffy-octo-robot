"""Tests for code_muse.tools.command_runner core helper functions.
...
class TestSetAwaitingUserInput:
    ...
    def test_set_awaiting_true_calls_pause_spinners(self, monkeypatch):
        ...
        with patch.dict(
            "sys.modules", {"code_muse.messaging.spinner": mock_spinner_module}
        ):
    ...
    def test_set_awaiting_handles_import_error(self, monkeypatch):
        ...
        def mock_import(name, *args, **kwargs):
            if name == "code_muse.messaging.spinner":
                raise ImportError("No module named 'code_muse.messaging.spinner'")
