"""Tests for code_muse/tools/skills_tools.py - 100% coverage."""
...
class TestSkillModels:
    def test_skill_list_output(self):
        from code_muse.tools.skills_tools import SkillListOutput

        out = SkillListOutput(skills=[], total_count=0, query="test", error=None)
        assert out.total_count == 0

    def test_skill_activate_output(self):
        from code_muse.tools.skills_tools import SkillActivateOutput
...
class TestActivateSkill:
    @pytest.mark.asyncio
    @patch(
        "code_muse.plugins.agent_skills.config.get_skills_enabled", return_value=False
    )
    ...
    async def test_consent_required(self, mock_disc, mock_dirs, mock_enabled):
        skill = MagicMock()
        skill.name = "test"
        ...
        register_activate_skill(agent)
        ctx = MagicMock()
        result = await cap["fn"](ctx, skill_name="test", consent_confirmed=False)
    ...
    @patch(
        "code_muse.plugins.agent_skills.discovery.discover_skills",
    ...
    async def test_load_content_fails(
        ...
        register_activate_skill(agent)
        ctx = MagicMock()
        result = await cap["fn"](ctx, skill_name="test", consent_confirmed=True)
        assert result.error and "Failed to load" in result.error

    @pytest.mark.asyncio
    @patch("code_muse.tools.skills_tools.get_message_bus")
    @patch(
        "code_muse.plugins.agent_skills.metadata.get_skill_resources",
...
class TestListOrSearchSkills:
    ...
    async def test_disabled(self, mock_enabled):
        from code_muse.tools.skills_tools import register_list_or_search_skills
    ...
    @patch(
        "code_muse.plugins.agent_skills.config.get_disabled_skills", return_value=set()
    )
    @patch(
        "code_muse.plugins.agent_skills.config.get_skill_directories",
        return_value=["/tmp"],
    ...
    @pytest.mark.asyncio
    @patch("code_muse.tools.skills_tools.get_message_bus")
    @patch("code_muse.plugins.agent_skills.metadata.parse_skill_metadata")
    @patch("code_muse.plugins.agent_skills.discovery.discover_skills")
    @patch(
        "code_muse.plugins.agent_skills.config.get_skill_directories",
        return_value=["/tmp"],
    ...
    @patch("code_muse.tools.skills_tools.get_message_bus")
    @patch(
        "code_muse.plugins.agent_skills.metadata.parse_skill_metadata",
        return_value=None,
    )
    @patch("code_muse.plugins.agent_skills.discovery.discover_skills")
    @patch(
