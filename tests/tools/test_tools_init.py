"""Tests for code_muse/tools/__init__.py - 100% coverage."""
...
class TestLoadPluginTools:
    def test_loads_tools(self):
        from code_muse.tools import TOOL_REGISTRY, _load_plugin_tools

        with patch("code_muse.tools.on_register_tools") as mock_cb:
    ...
    def test_none_results(self):
        from code_muse.tools import _load_plugin_tools
    ...
    def test_exception_swallowed(self):
        ...
        with patch("code_muse.tools.on_register_tools", side_effect=Exception("boom")):
            _load_plugin_tools()  # should not raise


class TestHasExtendedThinkingActive:
    @patch("code_muse.config.get_global_model_name", return_value=None)
    def test_no_model(self, mock_model):
        from code_muse.tools import has_extended_thinking_active
    ...
    @patch("code_muse.model_utils.get_default_extended_thinking", return_value=False)
...
class TestRegisterToolsForAgent:
    @patch("code_muse.tools._load_plugin_tools")
    @patch("code_muse.tools.has_extended_thinking_active", return_value=False)
    def test_register_known_tool(self, mock_ext, mock_load):
        from code_muse.tools import TOOL_REGISTRY, register_tools_for_agent
    ...
    @patch("code_muse.tools._load_plugin_tools")
    @patch("code_muse.tools.has_extended_thinking_active", return_value=False)
    @patch("code_muse.tools.emit_warning")
    ...
    @patch("code_muse.tools._register_uc_tool_wrapper")
...
class TestRegisterUcToolWrapper:
    ...
    def test_tool_not_found(self, mock_warn, mock_reg):
        from code_muse.tools import _register_uc_tool_wrapper
...
class TestRegisterAllToolsAndGetNames:
    @patch("code_muse.tools.register_tools_for_agent")
    def test_register_all(self, mock_reg):
        from code_muse.tools import register_all_tools
...
class TestExtendedThinkingPromptNote:
    def test_constant_exists(self):
        from code_muse.tools import EXTENDED_THINKING_PROMPT_NOTE
