"""Tests for code_muse/tools/agent_tools.py - 100% coverage."""
...
class TestGenerateSessionHashSuffix:
    def test_returns_hex_string(self):
        from code_muse.tools.agent_tools import _generate_session_hash_suffix
...
class TestValidateSessionId:
    def test_valid_ids(self):
        from code_muse.tools.agent_tools import _validate_session_id
...
class TestSubagentSessionsDir:
    @patch("code_muse.tools.agent_tools.DATA_DIR", "/tmp/test_data")
    def test_creates_dir(self):
        from code_muse.tools.agent_tools import _get_subagent_sessions_dir
...
class TestSaveSessionHistory:
    def test_save_new_session(self, tmp_path):
        from code_muse.tools.agent_tools import _save_session_history

        with patch(
            "code_muse.tools.agent_tools._get_subagent_sessions_dir",
...
class TestLoadSessionHistory:
    def test_load_nonexistent(self, tmp_path):
        from code_muse.tools.agent_tools import _load_session_history
    ...
    def test_load_existing(self, tmp_path):
        ...
        # Current _save_session_history writes JSON to the .pkl compat file.
        pkl = tmp_path / "my-session.pkl"
        pkl.write_text(
            json.dumps(
                {
                    "schema": "code_muse.subagent.session.v1",
...
class TestModels:
    def test_agent_info(self):
        from code_muse.tools.agent_tools import AgentInfo

        a = AgentInfo(name="n", display_name="d", description="desc")
        assert a.name == "n"

    def test_list_agents_output(self):
        from code_muse.tools.agent_tools import ListAgentsOutput

        o = ListAgentsOutput(agents=[])
        assert o.error is None

    def test_agent_invoke_output(self):
        from code_muse.tools.agent_tools import AgentInvokeOutput
...
class TestRegisterListAgents:
    def test_register_and_call(self):
        from code_muse.tools.agent_tools import register_list_agents
        ...
        ctx = MagicMock()
        with (
            patch("code_muse.tools.agent_tools.generate_group_id", return_value="grp"),
            patch("code_muse.tools.agent_tools.emit_info"),
            patch(
                "code_muse.agents.get_available_agents", return_value={"a": "Agent A"}
            ),
            patch(
                "code_muse.agents.get_agent_descriptions", return_value={"a": "desc"}
            ),
            patch("code_muse.config.get_banner_color", return_value="blue"),
        ):
    ...
    def test_register_error(self):
        ...
        with (
            ...
            patch("code_muse.config.get_banner_color", return_value="blue"),
            patch(
                "code_muse.agents.get_available_agents", side_effect=Exception("boom")
            ),
        ):
            result = captured["fn"](ctx)
