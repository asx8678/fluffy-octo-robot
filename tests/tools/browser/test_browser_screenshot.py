...
class TestScreenshotCapture:
    ...
    async def test_capture_screenshot_with_save(self, tmp_path):
        ...
        with patch(
            "code_muse.tools.browser.browser_screenshot._TEMP_SCREENSHOT_ROOT",
            tmp_path,
        ):
    ...
    async def test_capture_with_emit_messages(self):
        ...
        with patch(
            "code_muse.tools.browser.browser_screenshot.emit_success"
        ) as mock_emit:
            result = await _capture_screenshot(
                mock_page,
...
class TestTakeScreenshot:
    ...
    async def test_take_screenshot_success(self):
        ...
        with (
            patch(
                "code_muse.tools.browser.browser_screenshot.get_session_browser_manager",
                return_value=mock_manager,
            ),
            patch("code_muse.tools.browser.browser_screenshot.emit_info"),
        ):
    ...
    async def test_take_screenshot_no_page(self):
        ...
        with (
            ...
            with patch("code_muse.tools.browser.browser_screenshot.emit_error"):
