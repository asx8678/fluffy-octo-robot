...
class TestLoadPluginBrowserTypes:
    def test_load_plugin_browser_types_success(self):
        from code_muse.tools.browser import browser_manager as bm

        bm._BROWSER_TYPES_LOADED = False
        bm._CUSTOM_BROWSER_TYPES = {}

        with patch(
            "code_muse.tools.browser.browser_manager.on_register_browser_types",
            create=True,
        ):
...
class TestInitializeBrowser:
    ...
    async def test_initialize_browser_default_chromium(self):
        from code_muse.tools.browser.browser_manager import BrowserManager
    ...
    async def test_initialize_browser_custom_type(self):
        ...
        with patch("code_muse.tools.browser.browser_manager.emit_info"):
            await mgr._initialize_browser()
            assert mgr._initialized is True
...
class TestCleanupSilent:
    ...
    async def test_cleanup_silent_mode(self):
        ...
        with (
            patch("code_muse.tools.browser.browser_manager.emit_warning") as mock_warn,
            patch("code_muse.tools.browser.browser_manager.emit_success") as mock_succ,
        ):
    ...
    async def test_cleanup_non_silent_storage_success(self):
        ...
        with (
            patch("code_muse.tools.browser.browser_manager.emit_success") as mock_succ,
            patch("code_muse.tools.browser.browser_manager.emit_warning"),
            patch("code_muse.tools.browser.browser_manager.emit_info"),
        ):
    ...
    async def test_cleanup_outer_exception(self):
        ...
        with (
            patch("code_muse.tools.browser.browser_manager.emit_success"),
            patch("code_muse.tools.browser.browser_manager.emit_warning"),
        ):
            await mgr._cleanup(silent=False)
            assert mgr._initialized is False
