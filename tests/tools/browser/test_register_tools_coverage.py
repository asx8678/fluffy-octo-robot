...
class TestBrowserControlRegister:
    ...
    async def test_browser_initialize(self):
        from code_muse.tools.browser.browser_control import register_initialize_browser

        agent = _make_agent()
        register_initialize_browser(agent)
        fn = agent._registered["browser_initialize"]
        with patch(
            "code_muse.tools.browser.browser_control.initialize_browser",
    ...
    async def test_browser_close(self):
        from code_muse.tools.browser.browser_control import register_close_browser

        agent = _make_agent()
        register_close_browser(agent)
        fn = agent._registered["browser_close"]
        with patch(
            "code_muse.tools.browser.browser_control.close_browser",
            return_value={"success": True},
        ):
            r = await fn(_mock_context())
            assert r["success"] is True
    ...
    async def test_browser_status(self):
        from code_muse.tools.browser.browser_control import register_get_browser_status

        agent = _make_agent()
        register_get_browser_status(agent)
        fn = agent._registered["browser_status"]
        with patch(
            "code_muse.tools.browser.browser_control.get_browser_status",
            return_value={"success": True},
        ):
            r = await fn(_mock_context())
            assert r["success"] is True
    ...
    async def test_browser_new_page(self):
        from code_muse.tools.browser.browser_control import register_create_new_page

        agent = _make_agent()
        register_create_new_page(agent)
        fn = agent._registered["browser_new_page"]
        with patch(
            "code_muse.tools.browser.browser_control.create_new_page",
            return_value={"success": True},
        ):
            r = await fn(_mock_context())
            assert r["success"] is True
    ...
    async def test_browser_list_pages(self):
        from code_muse.tools.browser.browser_control import register_list_pages

        agent = _make_agent()
        register_list_pages(agent)
        fn = agent._registered["browser_list_pages"]
        with patch(
            "code_muse.tools.browser.browser_control.list_pages",
            return_value={"success": True},
        ):
            r = await fn(_mock_context())
            assert r["success"] is True
...
class TestBrowserNavigationRegister:
    ...
    async def test_browser_navigate(self):
        from code_muse.tools.browser.browser_navigation import register_navigate_to_url

        agent = _make_agent()
        register_navigate_to_url(agent)
        fn = agent._registered["browser_navigate"]
        with patch(
            "code_muse.tools.browser.browser_navigation.navigate_to_url",
            return_value={"success": True},
        ):
    ...
    async def test_browser_get_page_info(self):
        from code_muse.tools.browser.browser_navigation import register_get_page_info

        agent = _make_agent()
        register_get_page_info(agent)
        fn = agent._registered["browser_get_page_info"]
        with patch(
            "code_muse.tools.browser.browser_navigation.get_page_info",
            return_value={"success": True},
        ):
            r = await fn(_mock_context())
            assert r["success"] is True
    ...
    async def test_browser_go_back(self):
        from code_muse.tools.browser.browser_navigation import register_browser_go_back

        agent = _make_agent()
        register_browser_go_back(agent)
        fn = agent._registered["browser_go_back"]
        with patch(
            "code_muse.tools.browser.browser_navigation.go_back",
            return_value={"success": True},
        ):
            r = await fn(_mock_context())
            assert r["success"] is True
    ...
    async def test_browser_go_forward(self):
        ...
        with patch(
            "code_muse.tools.browser.browser_navigation.go_forward",
            return_value={"success": True},
        ):
            r = await fn(_mock_context())
            assert r["success"] is True
    ...
    async def test_browser_reload(self):
        from code_muse.tools.browser.browser_navigation import register_reload_page

        agent = _make_agent()
        register_reload_page(agent)
        fn = agent._registered["browser_reload"]
        with patch(
            "code_muse.tools.browser.browser_navigation.reload_page",
            return_value={"success": True},
        ):
            r = await fn(_mock_context())
            assert r["success"] is True
    ...
    async def test_browser_wait_for_load(self):
        ...
        with patch(
            "code_muse.tools.browser.browser_navigation.wait_for_load_state",
            return_value={"success": True},
        ):
            r = await fn(_mock_context())
            assert r["success"] is True
...
class TestBrowserInteractionsRegister:
    ...
    async def test_click(self):
        from code_muse.tools.browser.browser_interactions import register_click_element

        agent = _make_agent()
        register_click_element(agent)
        fn = agent._registered["browser_click"]
        with patch(
            "code_muse.tools.browser.browser_interactions.click_element",
            return_value={"success": True},
        ):
    ...
    async def test_double_click(self):
        ...
        with patch(
            "code_muse.tools.browser.browser_interactions.double_click_element",
            return_value={"success": True},
        ):
            r = await fn(_mock_context(), selector="#x")
            assert r["success"] is True
    ...
    async def test_hover(self):
        from code_muse.tools.browser.browser_interactions import register_hover_element

        agent = _make_agent()
        register_hover_element(agent)
        fn = agent._registered["browser_hover"]
        with patch(
            "code_muse.tools.browser.browser_interactions.hover_element",
            return_value={"success": True},
        ):
            r = await fn(_mock_context(), selector="#x")
            assert r["success"] is True
    ...
    async def test_set_text(self):
        ...
        with patch(
            "code_muse.tools.browser.browser_interactions.set_element_text",
            return_value={"success": True},
        ):
    ...
    async def test_get_text(self):
        ...
        with patch(
            "code_muse.tools.browser.browser_interactions.get_element_text",
            return_value={"success": True},
        ):
            r = await fn(_mock_context(), selector="#x")
            assert r["success"] is True
    ...
    async def test_get_value(self):
        ...
        with patch(
            "code_muse.tools.browser.browser_interactions.get_element_value",
            return_value={"success": True},
        ):
            r = await fn(_mock_context(), selector="#x")
            assert r["success"] is True
    ...
    async def test_select_option(self):
        from code_muse.tools.browser.browser_interactions import register_select_option

        agent = _make_agent()
        register_select_option(agent)
        fn = agent._registered["browser_select_option"]
        with patch(
            "code_muse.tools.browser.browser_interactions.select_option",
            return_value={"success": True},
        ):
    ...
    async def test_check(self):
        from code_muse.tools.browser.browser_interactions import register_browser_check

        agent = _make_agent()
        register_browser_check(agent)
        fn = agent._registered["browser_check"]
        with patch(
            "code_muse.tools.browser.browser_interactions.check_element",
            return_value={"success": True},
        ):
            r = await fn(_mock_context(), selector="#x")
            assert r["success"] is True
    ...
    async def test_uncheck(self):
        ...
        with patch(
            "code_muse.tools.browser.browser_interactions.uncheck_element",
            return_value={"success": True},
        ):
            r = await fn(_mock_context(), selector="#x")
            assert r["success"] is True
...
class TestBrowserLocatorsRegister:
    ...
    async def test_find_by_role(self):
        from code_muse.tools.browser.browser_locators import register_find_by_role

        agent = _make_agent()
        register_find_by_role(agent)
        fn = agent._registered["browser_find_by_role"]
        with patch(
            "code_muse.tools.browser.browser_locators.find_by_role",
            return_value={"success": True},
        ):
    ...
    async def test_find_by_text(self):
        from code_muse.tools.browser.browser_locators import register_find_by_text

        agent = _make_agent()
        register_find_by_text(agent)
        fn = agent._registered["browser_find_by_text"]
        with patch(
            "code_muse.tools.browser.browser_locators.find_by_text",
            return_value={"success": True},
        ):
    ...
    async def test_find_by_label(self):
        from code_muse.tools.browser.browser_locators import register_find_by_label

        agent = _make_agent()
        register_find_by_label(agent)
        fn = agent._registered["browser_find_by_label"]
        with patch(
            "code_muse.tools.browser.browser_locators.find_by_label",
            return_value={"success": True},
        ):
    ...
    async def test_find_by_placeholder(self):
        ...
        with patch(
            "code_muse.tools.browser.browser_locators.find_by_placeholder",
            return_value={"success": True},
        ):
    ...
    async def test_find_by_test_id(self):
        from code_muse.tools.browser.browser_locators import register_find_by_test_id

        agent = _make_agent()
        register_find_by_test_id(agent)
        fn = agent._registered["browser_find_by_test_id"]
        with patch(
            "code_muse.tools.browser.browser_locators.find_by_test_id",
            return_value={"success": True},
        ):
    ...
    async def test_xpath_query(self):
        from code_muse.tools.browser.browser_locators import register_run_xpath_query

        agent = _make_agent()
        register_run_xpath_query(agent)
        fn = agent._registered["browser_xpath_query"]
        with patch(
            "code_muse.tools.browser.browser_locators.run_xpath_query",
            return_value={"success": True},
        ):
    ...
    async def test_find_buttons(self):
        from code_muse.tools.browser.browser_locators import register_find_buttons

        agent = _make_agent()
        register_find_buttons(agent)
        fn = agent._registered["browser_find_buttons"]
        with patch(
            "code_muse.tools.browser.browser_locators.find_buttons",
            return_value={"success": True},
        ):
            r = await fn(_mock_context())
            assert r["success"] is True
    ...
    async def test_find_links(self):
        from code_muse.tools.browser.browser_locators import register_find_links

        agent = _make_agent()
        register_find_links(agent)
        fn = agent._registered["browser_find_links"]
        with patch(
            "code_muse.tools.browser.browser_locators.find_links",
            return_value={"success": True},
        ):
            r = await fn(_mock_context())
            assert r["success"] is True
...
class TestBrowserScriptsRegister:
    ...
    async def test_execute_js(self):
        from code_muse.tools.browser.browser_scripts import register_execute_javascript

        agent = _make_agent()
        register_execute_javascript(agent)
        fn = agent._registered["browser_execute_js"]
        with patch(
            "code_muse.tools.browser.browser_scripts.execute_javascript",
            return_value={"success": True},
        ):
    ...
    async def test_scroll(self):
        from code_muse.tools.browser.browser_scripts import register_scroll_page

        agent = _make_agent()
        register_scroll_page(agent)
        fn = agent._registered["browser_scroll"]
        with patch(
            "code_muse.tools.browser.browser_scripts.scroll_page",
            return_value={"success": True},
        ):
            r = await fn(_mock_context())
            assert r["success"] is True
    ...
    async def test_scroll_to_element(self):
        from code_muse.tools.browser.browser_scripts import register_scroll_to_element

        agent = _make_agent()
        register_scroll_to_element(agent)
        fn = agent._registered["browser_scroll_to_element"]
        with patch(
            "code_muse.tools.browser.browser_scripts.scroll_to_element",
            return_value={"success": True},
        ):
            r = await fn(_mock_context(), selector="#x")
            assert r["success"] is True
    ...
    async def test_set_viewport(self):
        from code_muse.tools.browser.browser_scripts import register_set_viewport_size

        agent = _make_agent()
        register_set_viewport_size(agent)
        fn = agent._registered["browser_set_viewport"]
        with patch(
            "code_muse.tools.browser.browser_scripts.set_viewport_size",
            return_value={"success": True},
        ):
    ...
    async def test_wait_for_element(self):
        from code_muse.tools.browser.browser_scripts import register_wait_for_element

        agent = _make_agent()
        register_wait_for_element(agent)
        fn = agent._registered["browser_wait_for_element"]
        with patch(
            "code_muse.tools.browser.browser_scripts.wait_for_element",
            return_value={"success": True},
        ):
            r = await fn(_mock_context(), selector="#x")
            assert r["success"] is True
    ...
    async def test_highlight_element(self):
        ...
        with patch(
            "code_muse.tools.browser.browser_scripts.highlight_element",
            return_value={"success": True},
        ):
            r = await fn(_mock_context(), selector="#x")
            assert r["success"] is True
    ...
    async def test_clear_highlights(self):
        ...
        with patch(
            "code_muse.tools.browser.browser_scripts.clear_highlights",
            return_value={"success": True},
        ):
            r = await fn(_mock_context())
            assert r["success"] is True
...
class TestBrowserScreenshotRegister:
    ...
    async def test_screenshot_analyze(self):
        ...
        with patch(
            "code_muse.tools.browser.browser_screenshot.take_screenshot",
            return_value={"success": True},
        ):
            r = await fn(_mock_context())
            assert r["success"] is True
...
class TestBrowserWorkflowsRegister:
    ...
    async def test_save_workflow(self):
        from code_muse.tools.browser.browser_workflows import register_save_workflow

        agent = _make_agent()
        register_save_workflow(agent)
        fn = agent._registered["browser_save_workflow"]
        with patch(
            "code_muse.tools.browser.browser_workflows.save_workflow",
            return_value={"success": True},
        ):
    ...
    async def test_list_workflows(self):
        from code_muse.tools.browser.browser_workflows import register_list_workflows

        agent = _make_agent()
        register_list_workflows(agent)
        fn = agent._registered["browser_list_workflows"]
        with patch(
            "code_muse.tools.browser.browser_workflows.list_workflows",
            return_value={"success": True},
        ):
            r = await fn(_mock_context())
            assert r["success"] is True
    ...
    async def test_read_workflow(self):
        from code_muse.tools.browser.browser_workflows import register_read_workflow

        agent = _make_agent()
        register_read_workflow(agent)
        fn = agent._registered["browser_read_workflow"]
        with patch(
            "code_muse.tools.browser.browser_workflows.read_workflow",
            return_value={"success": True},
        ):
