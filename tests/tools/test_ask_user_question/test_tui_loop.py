...
from code_muse.tools.ask_user_question.tui_loop import TUIResult, run_question_tui
...
async def _run_with_handler_callback(state, callback):
    ...
    with (
        patch("code_muse.tools.ask_user_question.tui_loop.Application") as MockApp,
        patch("code_muse.tools.ask_user_question.tui_loop.create_output"),
...
class TestPanelRendering:
    ...
    async def test_panel_lambdas_callable(self):
        ...
        with (
            ...
            with (
                patch(
                    "code_muse.tools.ask_user_question.tui_loop.FormattedTextControl",
                    CaptureFTC,
                ),
                patch(
                    "code_muse.tools.ask_user_question.tui_loop.Window",
                    return_value=MagicMock(),
                ),
                patch(
                    "code_muse.tools.ask_user_question.tui_loop.VSplit",
                    return_value=MagicMock(),
                ),
                patch(
                    "code_muse.tools.ask_user_question.tui_loop.Frame",
                    return_value=MagicMock(),
                ),
                patch("code_muse.tools.ask_user_question.tui_loop.Layout"),
            ):
