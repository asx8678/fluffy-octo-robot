"""Tests for code_muse.tools.tools_content.

This module tests the tools_content string constant that provides
user-facing documentation about Muse's available tools.
"""
...
spec = importlib.util.spec_from_file_location(
    ...
    Path(__file__).parent.parent.parent / "code_muse" / "tools" / "tools_content.py",
)
...
class TestToolsContentFormatting:
    ...
    def test_contains_dog_emoji(self):
        ...
        assert "[Run]" in tools_content, (
            "Expected run token [Run] for brand consistency"
        )
