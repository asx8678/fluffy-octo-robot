...
from code_muse.plugins.universal_constructor.runner import (
    _cap_output,
    run_tool_subprocess,
)
...
class TestToolNameValidation:
    ...
    def test_reserved_check_bypass_with_flag(self):
        # check_reserved=False allows reserved names (for namespace segments)
        assert validate_tool_name("os", check_reserved=False) is None
        assert validate_tool_name("json", check_reserved=False) is None
        # check_reserved=True (default) still rejects
        assert validate_tool_name("os", check_reserved=True) is not None
    ...
    def test_full_tool_name_reserved_namespace_allowed(self):
        # Namespace segments matching reserved module names are allowed
        # because they don't shadow modules (invoked as uc:json.validate_files)
        assert validate_full_tool_name("json.validate_files") is None
        assert validate_full_tool_name("os.system_info") is None
        assert validate_full_tool_name("sys.get_platform") is None

    def test_full_tool_name_reserved_leaf_still_rejected(self):
        # The final (leaf) segment still can't be a reserved name
        assert validate_full_tool_name("json") is not None
        assert validate_full_tool_name("os") is not None
        # A single reserved name is the leaf, so it's rejected
        assert validate_full_tool_name("mytool.os") is not None
    ...
    def test_namespace_validation_reserved_allowed(self):
        # Namespace segments can be reserved module names
        assert validate_namespace("json") is None
        assert validate_namespace("os.sys") is None
        assert validate_namespace("json.api") is None
...
class TestRegistryHardening:
    ...
    def test_scan_skips_disabled_tools_without_import(self):
        from code_muse.plugins.universal_constructor.registry import UCRegistry
