...
def test_shell_safety_loaded_at_default_permission_level():
    ...
    from code_muse.plugins import _load_builtin_plugins

    plugins_dir = Path(__file__).parent.parent.parent / "code_muse" / "plugins"
    with patch("code_muse.config.get_safety_permission_level", return_value="medium"):
...
def test_background_job_registry_lists_and_stops_job(tmp_path):
    from code_muse.tools.background_jobs import (
        _BACKGROUND_JOBS,
