...
class TestChatGPTTokenStorage:
    ...
    @patch("code_muse.plugins.chatgpt_oauth.utils.get_token_storage_path")
    ...
    def test_load_tokens_warns_and_fixes_broad_mode(
        ...
        mock_get_path.return_value = token_file
        with caplog.at_level(logging.WARNING, logger="code_muse.secret_storage"):
...
class TestChatGPTLogRedaction:
    ...
    def test_refresh_failure_does_not_log_token_body(self, mock_post, caplog):
        ...
        with patch(
            "code_muse.plugins.chatgpt_oauth.utils.load_stored_tokens"
        ) as mock_load:
            mock_load.return_value = {"refresh_token": "rt"}
            with caplog.at_level(
                logging.ERROR, logger="code_muse.plugins.chatgpt_oauth.utils"
            ):
                chatgpt_refresh()
...
class TestClaudeLogRedaction:
    ...
    def test_exchange_non_json_does_not_log_raw_body(self, mock_post, caplog):
        ...
        with caplog.at_level(
            logging.ERROR, logger="code_muse.plugins.claude_code_oauth.utils"
        ):
    ...
    def test_refresh_failure_does_not_log_token_body(self, mock_post, caplog):
        ...
        with (
            ...
            patch(
                "code_muse.plugins.claude_code_oauth.utils.is_token_expired",
                return_value=True,
            ),
        ):
            mock_load.return_value = {"refresh_token": "rt"}
...
class TestClaudeCallbackSecurity:
    ...
    def test_callback_log_redacts_query(self, caplog):
        ...
        with (
            ...
            caplog.at_level(
                logging.INFO,
                logger="code_muse.plugins.claude_code_oauth.register_callbacks",
            ),
        ):
            handler.do_GET()
...
class TestChatGPTCallbackSecurity:
    ...
    def test_valid_state_exchanges_code(self, _handler):
        ...
        with (
            patch(
                "code_muse.plugins.chatgpt_oauth.oauth_flow.save_tokens",
                return_value=True,
            ),
