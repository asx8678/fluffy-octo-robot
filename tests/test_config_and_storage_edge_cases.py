...
from code_muse import callbacks, session_storage
...
def mock_config_paths(monkeypatch, tmp_path):
    ...
    mock_config_dir = str(tmp_path / ".config" / "code_muse")
    mock_config_file = tmp_path / ".config" / "code_muse" / "muse.cfg"
    mock_data_dir = str(tmp_path / ".local" / "share" / "code_muse")
    mock_cache_dir = str(tmp_path / ".cache" / "code_muse")
    mock_state_dir = str(tmp_path / ".local" / "state" / "code_muse")
...
def _write_config(path: str, cfg) -> None:
    import io
...
class TestSubagentVerbose:
    ...
    def test_get_subagent_verbose_returns_false_by_default(self, mock_config_paths):
        ...
        config["muse"] = {}
        Path(mock_cfg_dir).mkdir(parents=True, exist_ok=True)
        _write_config(mock_cfg_file, config)
    ...
    def test_get_subagent_verbose_returns_true_for_truthy_values(
        ...
        Path(mock_cfg_dir).mkdir(parents=True, exist_ok=True)
        config = configparser.ConfigParser()
        config["muse"] = {"subagent_verbose": truthy_value}
        _write_config(mock_cfg_file, config)
    ...
    def test_get_subagent_verbose_returns_false_for_falsy_values(
        ...
        Path(mock_cfg_dir).mkdir(parents=True, exist_ok=True)
        config = configparser.ConfigParser()
        config["muse"] = {"subagent_verbose": falsy_value}
        _write_config(mock_cfg_file, config)
...
class TestAllowRecursion:
    ...
    def test_get_allow_recursion_respects_explicit_false(self, mock_config_paths):
        ...
        config = configparser.ConfigParser()
        config["muse"] = {"allow_recursion": "false"}
        Path(mock_cfg_dir).mkdir(parents=True, exist_ok=True)
        _write_config(mock_cfg_file, config)
    ...
    def test_get_allow_recursion_respects_truthy_values(
        ...
        Path(mock_cfg_dir).mkdir(parents=True, exist_ok=True)
        config = configparser.ConfigParser()
        config["muse"] = {"allow_recursion": truthy}
        _write_config(mock_cfg_file, config)
...
class TestPuppyTokens:
    """Test muse token configuration."""

    def test_get_muse_token_returns_value_if_set(self, mock_config_paths):
        """Test getting muse token when it's set."""
        mock_cfg_dir, mock_cfg_file, _ = mock_config_paths

        config = configparser.ConfigParser()
        config["muse"] = {"muse_token": "secret-token-123"}
        Path(mock_cfg_dir).mkdir(parents=True, exist_ok=True)
        _write_config(mock_cfg_file, config)

        result = cp_config.get_muse_token()
        assert result == "secret-token-123"

    def test_get_muse_token_returns_none_if_not_set(self, mock_config_paths):
        """Test getting muse token when it's not set."""
        mock_cfg_dir, mock_cfg_file, _ = mock_config_paths
    ...
    def test_set_muse_token_persists_value(self, mock_config_paths):
        """Test setting muse token."""
        mock_cfg_dir, mock_cfg_file, _ = mock_config_paths
        ...
        cp_config.set_muse_token("new-token-456")

        saved_config = configparser.ConfigParser()
        saved_config.read(mock_cfg_file)
        assert saved_config["muse"]["muse_token"] == "new-token-456"
...
class TestXDGDirectoryHandling:
    ...
    def test_get_xdg_dir_respects_environment_variable(self):
        ...
        with patch.dict(os.environ, {"XDG_CONFIG_HOME": "/custom/config"}):
            ...
            assert str(result) == "/custom/config/muse"

    def test_get_xdg_dir_defaults_to_home_when_no_env_var(self):
        """Test fallback to ~/.muse when env var not set."""
        with patch.dict(os.environ, {}, clear=True):
            with patch("os.path.expanduser") as mock_expand:
                mock_expand.return_value = "/home/user"
                result = cp_config._get_xdg_dir("XDG_CONFIG_HOME", ".config")
                assert str(result) == "/home/user/.muse"
...
class TestConfigKeys:
    ...
    def test_get_config_keys_includes_custom_keys(self, mock_config_paths):
        ...
        config = configparser.ConfigParser()
        config["muse"] = {"custom_key_1": "value1", "custom_key_2": "value2"}
        Path(mock_cfg_dir).mkdir(parents=True, exist_ok=True)
        _write_config(mock_cfg_file, config)
