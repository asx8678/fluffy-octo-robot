...
class TestConfigExtendedPart1:
    """Test basic config operations in code_muse/config.py"""

    @pytest.fixture
    def temp_config_dir(self):
        """Create a temporary config directory for isolated testing"""
        with tempfile.TemporaryDirectory() as temp_dir:
            config_file = Path(temp_dir) / "muse.cfg"
    ...
    def test_get_yolo_mode_default(self, temp_config_dir):
        """Test get_yolo_mode returns False (safe) when not set"""
        temp_dir, config_file = temp_config_dir
    ...
    def test_get_agent_name_default(self, temp_config_dir):
        """Test get_agent_name returns default when not set"""
        temp_dir, config_file = temp_config_dir
