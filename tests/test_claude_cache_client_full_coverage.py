"""Full coverage tests for code_muse/claude_cache_client.py."""
...
class TestCheckStoredExpiry:
    def test_with_tokens_expired(self):
        ...
        with patch.dict(
            "sys.modules",
            {
                "code_muse.plugins.claude_code_oauth": MagicMock(),
                "code_muse.plugins.claude_code_oauth.utils": mock_module,
            },
        ):
...
class TestPatchAnthropic:
    ...
    async def test_patches_create(self):
        ...
        with patch("code_muse.claude_cache_client.AsyncAnthropic", type(mock_client)):
...
class TestSendFlow:
    ...
    async def test_auth_error_refresh_fails(self):
        ...
        with (
            ...
            c = ClaudeCacheAsyncClient()
            req = httpx.Request("POST", "https://api.com/v1/messages", content=b"{}")
            result = await c.send(req)
    ...
    async def test_already_attempted_refresh(self):
        ...
        with (
            ...
            patch.object(ClaudeCacheAsyncClient, "_refresh_claude_oauth_token"),
        ):
            c = ClaudeCacheAsyncClient()
