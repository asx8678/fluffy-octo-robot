...
from code_muse import session_storage
...
def mock_interactive_imports(
    ...
    async def _manager():
        ...
        patches = [
            ...
            patch(
                "code_muse.config.set_current_autosave_from_session_name",
                MagicMock(),
            ),
        ]
        ...
        if mock_load_session is not None:
            patches.append(
                patch(
                    "code_muse.session_storage.load_session",
                    mock_load_session,
                )
            )
...
class TestRestoreAutosaveDisplayFormatting:
    ...
    async def test_displays_unknown_for_missing_metadata(self, tmp_path):
        ...
        # Provide metadata with missing fields so the session is listed
        # but timestamp and message_count are absent.
        (tmp_path / "missing_info_meta.json").write_text(
            json.dumps({"session_name": "missing_info"}), encoding="utf-8"
        )
...
class TestCleanupSessionsEdgeCases:
    ...
    def test_cleanup_fewer_than_max(self, tmp_path):
        ...
        for name in ("session1", "session2"):
            (tmp_path / f"{name}.pkl").write_bytes(b"dummy")
            (tmp_path / f"{name}_meta.json").write_text(
                json.dumps({"session_name": name}), encoding="utf-8"
            )
