...
class TestGetApiKey:
    ...
    def test_get_api_key_from_config_first(self):
        ...
        with patch("code_muse.model_factory.get_value", return_value="config-key"):
    ...
    def test_get_api_key_falls_back_to_env(self):
        ...
        with patch("code_muse.model_factory.get_value", return_value=None):
            with patch.dict(os.environ, {"TEST_API_KEY": "env-key"}):
                result = get_api_key("TEST_API_KEY")
    ...
    def test_get_api_key_case_insensitive_config_lookup(self):
        ...
        with patch("code_muse.model_factory.get_value") as mock_get_value:
...
class TestMakeModelSettings:
    ...
    def test_make_model_settings_returns_dict(self):
        ...
        from code_muse.model_factory import make_model_settings
    ...
    def test_make_model_settings_foundry_gpt5_uses_responses_fields(self):
        ...
        with (
            ...
            patch(
                "code_muse.config.get_openai_reasoning_effort",
                return_value="medium",
            ),
            patch(
                "code_muse.config.get_openai_reasoning_summary",
                return_value="auto",
            ),
            patch(
                "code_muse.config.get_openai_verbosity",
                return_value="medium",
            ),
        ):
    ...
    def test_make_model_settings_parallel_tool_calls_always_enabled(self):
        """Test parallel_tool_calls is never explicitly set — read-only tools are safe concurrently."""
        from code_muse.model_factory import make_model_settings

        # Parallel tool calls are always enabled now; read-only tools are safe
        # to run concurrently, and destructive tools still go through approval.
        with patch("code_muse.config.get_yolo_mode", return_value=False):
            settings = make_model_settings("gpt-4o", max_tokens=5000)
            assert "parallel_tool_calls" not in settings

        with patch("code_muse.config.get_yolo_mode", return_value=True):
            settings = make_model_settings("gpt-4o", max_tokens=5000)
            assert "parallel_tool_calls" not in settings
...
class TestZaiChatModel:
    ...
    def test_zai_chat_model_process_response(self):
        ...
        from code_muse.model_factory import ZaiChatModel
...
class TestGetCustomConfig:
    ...
    def test_get_custom_config_env_var_in_header(self):
        ...
        with patch(
            "code_muse.model_factory.get_api_key", return_value="resolved-token"
        ):
    ...
    def test_get_custom_config_inline_env_vars_with_spaces(self):
        ...
        with (
            patch("code_muse.model_factory.get_api_key", side_effect=mock_get_api_key),
            patch("code_muse.model_factory.emit_warning"),
        ):
            url, headers, verify, api_key, timeout = get_custom_config(config)
    ...
    def test_get_custom_config_api_key_from_env(self):
        ...
        with patch(
            "code_muse.model_factory.get_api_key", return_value="resolved-api-key"
        ):
            url, headers, verify, api_key, timeout = get_custom_config(config)
...
class TestLoadConfigExtended:
    ...
    def test_load_config_multiple_callbacks_warning(self):
        ...
        from code_muse.model_factory import (
            ModelFactory,
    ...
    def test_load_config_filtered_claude_models(self):
        ...
        with patch("code_muse.model_factory.callbacks.get_callbacks", return_value=[]):
            with patch(
                "builtins.open",
                MagicMock(
                ...
                with patch("code_muse.model_factory.pathlib.Path") as mock_path_class:
                    # Set up path mocking
                    mock_main = MagicMock()
        ...
        with patch("code_muse.model_factory.callbacks.get_callbacks", return_value=[]):
            with patch("builtins.open", MagicMock()) as mock_open:
                ...
                with patch("code_muse.model_factory.pathlib.Path") as mock_path_class:
                    ...
                    with patch.dict(
                        "sys.modules",
                        {"code_muse.plugins.claude_code_oauth.utils": None},
                    ):
                        with patch(
                            "code_muse.plugins.claude_code_oauth.utils.load_claude_models_filtered",
...
class TestClaudeCodeModel:
    ...
    def test_claude_code_model_basic(self):
        ...
        with patch(
            ...
            with patch("code_muse.http_utils.get_cert_bundle_path", return_value=None):
                with patch("code_muse.http_utils.get_http2", return_value=True):
                    with patch("code_muse.claude_cache_client.ClaudeCacheAsyncClient"):
                        with patch("anthropic.AsyncAnthropic"):
                            with patch(
                                "code_muse.claude_cache_client.patch_anthropic_client_messages"
                            ):
                                with patch(
                                    "code_muse.plugins.claude_code_oauth.register_callbacks.make_anthropic_provider"
                                ):
                                    with patch(
    ...
    def test_claude_code_provider_name_is_distinct(self):
        ...
        from code_muse.provider_identity import AliasedAnthropicProvider
    ...
    def test_claude_code_model_interleaved_thinking_header(self):
        ...
        with patch(
            ...
            with patch("code_muse.http_utils.get_cert_bundle_path", return_value=None):
                with patch("code_muse.http_utils.get_http2", return_value=True):
                    with patch(
                        "code_muse.claude_cache_client.ClaudeCacheAsyncClient"
                    ) as mock_client:
                        with patch("anthropic.AsyncAnthropic"):
                            with patch(
                                "code_muse.claude_cache_client.patch_anthropic_client_messages"
    ...
    def test_claude_code_oauth_refresh(self):
        ...
        with patch(
            ...
            with patch(
                "code_muse.plugins.claude_code_oauth.register_callbacks.get_valid_access_token",
                return_value="new-refreshed-token",
            ):
                with patch(
                    "code_muse.http_utils.get_cert_bundle_path", return_value=None
                ):
                    with patch("code_muse.http_utils.get_http2", return_value=True):
                        with patch(
                            "code_muse.claude_cache_client.ClaudeCacheAsyncClient"
    ...
    def test_claude_code_missing_api_key(self):
        ...
        with patch(
            ...
            with patch(
                "code_muse.plugins.claude_code_oauth.register_callbacks.emit_warning"
            ) as mock_warn:
                with patch(
                    "code_muse.config.get_effective_model_settings", return_value={}
                ):
...
class TestCustomAnthropicModel:
    ...
    def test_custom_anthropic_with_api_key(self):
        ...
        with patch("code_muse.model_factory.get_cert_bundle_path", return_value=None):
            with patch("code_muse.model_factory.get_http2", return_value=True):
                with patch("code_muse.model_factory.ClaudeCacheAsyncClient"):
                    with patch("code_muse.model_factory.AsyncAnthropic"):
                        with patch(
    ...
    def test_custom_anthropic_missing_api_key(self):
        ...
        with (
            patch("code_muse.model_factory.emit_warning") as mock_warn,
            patch("code_muse.config.get_effective_model_settings", return_value={}),
        ):
...
class TestCustomGeminiModel:
    ...
    def test_custom_gemini_basic(self):
        ...
        with patch("code_muse.model_factory.create_async_client"):
            with patch("code_muse.model_factory.GeminiModel") as mock_model:
...
class TestCerebrasModel:
    ...
    def test_cerebras_model_basic(self):
        ...
        with (
            patch("code_muse.model_factory.create_async_client") as mock_create_client,
            patch("code_muse.model_factory.CerebrasProvider"),
        ):
            with patch("code_muse.model_factory.OpenAIChatModel") as mock_model:
                ...
                headers = call_args[1]["headers"]
                assert headers.get("X-Cerebras-3rd-Party-Integration") == "muse"
    ...
    def test_cerebras_zai_model_profile(self):
        ...
        with patch("code_muse.model_factory.create_async_client") as mock_create:
            # Return None to skip actual client creation
            mock_create.return_value = None
            with patch("code_muse.model_factory.CerebrasProvider"):
                with patch("code_muse.model_factory.OpenAIChatModel") as mock_model:
...
class TestOpenAICodexModels:
    ...
    def test_openai_codex_uses_responses_model(self):
        ...
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
            with patch("code_muse.model_factory.make_openai_provider"):
                with patch(
                    "code_muse.model_factory.OpenAIResponsesModel"
                ) as mock_responses:
                    with patch("code_muse.model_factory.OpenAIChatModel"):
...
class TestOpenAIProviderIdentity:
    def test_custom_openai_provider_name_uses_resolved_identity(self):
        ...
        from code_muse.model_factory import ModelFactory
        from code_muse.provider_identity import AliasedOpenAIProvider
...
class TestZaiApiModel:
    ...
    def test_zai_api_model_basic(self):
        ...
        with (
            ...
            patch("code_muse.model_factory.make_openai_provider") as mock_provider,
        ):
...
class TestAzureOpenAIExtended:
    ...
    def test_azure_openai_with_max_retries(self):
        ...
        with patch("code_muse.model_factory.AsyncAzureOpenAI") as mock_azure:
            with patch("code_muse.model_factory.make_openai_provider"):
                with patch("code_muse.model_factory.OpenAIChatModel"):
...
class TestRoundRobinExtended:
    ...
    def test_round_robin_with_rotate_every(self):
        ...
        with patch.dict(os.environ, {"OPENAI_API_KEY": "test-key"}):
            with patch("code_muse.model_factory.RoundRobinModel") as mock_rr:
    ...
    def test_gemini_oauth_missing_token(self):
        ...
        with (
            patch.dict(
                sys.modules,
                {
                    "code_muse.plugins.gemini_oauth": MagicMock(),
                    "code_muse.plugins.gemini_oauth.utils": mock_utils,
                    "code_muse.plugins.gemini_oauth.config": mock_config,
                },
            ),
            patch("code_muse.model_factory.emit_warning") as mock_warn,
        ):
...
class TestChatGPTOAuthErrorPaths:
    ...
    def test_chatgpt_oauth_plugin_not_available(self):
        ...
        with (
            ...
            pytest.raises(ValueError, match="Unsupported model type: chatgpt_oauth"),
        ):
    ...
    def test_chatgpt_oauth_missing_token(self):
        ...
        from code_muse.model_factory import ModelFactory
        from code_muse.plugins.chatgpt_oauth.register_callbacks import (
...
class TestContext1MBetaHeader:
    ...
    def test_no_context_length_key(self):
        from code_muse.model_factory import _build_anthropic_beta_header
