...
class TestGetMuseVersion:
    def test_returns_version(self):
        from code_muse.pydantic_patches import _get_muse_version
...
class TestPatchUserAgent:
    ...
    def test_kimi_model_returns_kimi_ua(self):
        ...
        with patch("code_muse.config.get_global_model_name", return_value="kimi-test"):
            ua = pydantic_models.get_user_agent()
            assert ua == "KimiCLI/0.63"

    def test_non_kimi_returns_code_muse_ua(self):
        from code_muse.pydantic_patches import patch_user_agent

        patch_user_agent()
        import pydantic_ai.models as pydantic_models

        with patch("code_muse.config.get_global_model_name", return_value="gpt-4"):
            ua = pydantic_models.get_user_agent()
            assert "Muse" in ua
    ...
    def test_get_model_name_exception(self):
        ...
        with patch("code_muse.config.get_global_model_name", side_effect=Exception):
            ua = pydantic_models.get_user_agent()
            assert "Muse" in ua
...
class TestPatchMessageHistoryCleaning:
    def test_patches_clean_message_history(self):
        from code_muse.pydantic_patches import patch_message_history_cleaning
...
def _process_message_history_available():
    try:
        import pydantic_ai._agent_graph as _pag

        return hasattr(_pag, "_process_message_history")
    except ImportError:
        return False
...
class TestPatchProcessMessageHistory:
    @pytest.mark.skipif(
        not _process_message_history_available(),
        reason="_process_message_history removed in newer pydantic-ai",
    )
    @pytest.mark.anyio
    async def test_patched_process_runs_processors(self):
        from code_muse.pydantic_patches import patch_process_message_history
...
class TestPatchToolCallJsonRepair:
    def test_patches_tool_manager(self):
        from code_muse.pydantic_patches import patch_tool_call_json_repair
...
class TestPatchToolCallCallbacks:
    def test_patches_tool_manager(self):
        from code_muse.pydantic_patches import patch_tool_call_callbacks
