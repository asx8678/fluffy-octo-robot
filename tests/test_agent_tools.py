...
class TestAgentTools:
    ...
    def test_invoke_agent_imports_load_muse_rules_from_builder(self):
        """Regression: ``load_muse_rules`` is a free function in ``_builder``,
        ...
        'load_muse_rules'``) ship to prod. Pin the actual contract now.
        """
        from code_muse.agents import _builder
        ...
        assert not hasattr(BaseAgent, "load_muse_rules")

        # The free function is the canonical entry point.
        assert callable(_builder.load_muse_rules)
...
class TestSessionSaveLoad:
    ...
    def test_save_and_load_roundtrip(self, temp_session_dir, mock_messages):
        ...
        with patch(
            ...
            for _i, (loaded, original) in enumerate(
                zip(loaded_messages, mock_messages, strict=False)
            ):
    ...
    def test_save_with_invalid_session_id_raises_error(
        ...
        with (
            ...
            pytest.raises(ValueError, match="must be kebab-case"),
        ):
            _save_session_history(
