...
def test_integration_env_env(integration_env: dict[str, str]) -> None:
    ...
    assert integration_env["MUSE_TEST_FAST"] == "1"
...
def test_harness_cleanup_terminates_and_removes_temp_home(
    ...
    old_selective_cleanup = os.environ.get("MUSE_SELECTIVE_CLEANUP")
    os.environ["MUSE_SELECTIVE_CLEANUP"] = "false"
    try:
        cli_harness.cleanup(result)
    finally:
        if old_selective_cleanup is None:
            os.environ.pop("MUSE_SELECTIVE_CLEANUP", None)
        else:
            os.environ["MUSE_SELECTIVE_CLEANUP"] = old_selective_cleanup
...
def test_selective_cleanup_only_removes_test_files(
    ...
    old_selective_cleanup = os.environ.get("MUSE_SELECTIVE_CLEANUP")
    os.environ["MUSE_SELECTIVE_CLEANUP"] = "true"
    try:
        cli_harness.cleanup(result)
    finally:
        if old_selective_cleanup is None:
