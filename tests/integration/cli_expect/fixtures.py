...
def live_cli(cli_harness: CliHarness) -> Generator[SpawnResult]:
    ...
    env = os.environ.copy()
    env.setdefault("MUSE_TEST_FAST", "1")
...
def satisfy_initial_prompts(result: SpawnResult, skip_autosave: bool = True) -> None:
    """Complete the muse agent name and owner prompts if they appear; otherwise continue."""
    try:
        result.child.expect("What should we name the agent?", timeout=10)
