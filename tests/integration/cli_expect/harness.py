...
CONFIG_TEMPLATE: Final[str] = """[muse]
agent_name = IntegrationPup
owner_name = MuseTester
...
class CliHarness:
    ...
    def spawn(
        ...
        if existing_home is not None:
            ...
            config_dir = temp_home / ".config" / "code_muse"
            code_muse_dir = temp_home / ".muse"
            config_dir.mkdir(parents=True, exist_ok=True)
            code_muse_dir.mkdir(parents=True, exist_ok=True)
            write_config = not (config_dir / "muse.cfg").exists()
        else:
            temp_home = pathlib.Path(
                tempfile.mkdtemp(prefix=f"code_muse_home_{_random_name()}_")
            )
            config_dir = temp_home / ".config" / "code_muse"
            code_muse_dir = temp_home / ".muse"
            config_dir.mkdir(parents=True, exist_ok=True)
        ...
        if write_config:
            # Write config to both XDG config dir and ~/.muse for compatibility
            (config_dir / "muse.cfg").write_text(CONFIG_TEMPLATE, encoding="utf-8")
            (code_muse_dir / "muse.cfg").write_text(CONFIG_TEMPLATE, encoding="utf-8")

        log_path = temp_home / f"cli_output_{uuid.uuid4().hex}.log"
        cmd_args = ["muse"] + (args or [])

        spawn_env = os.environ.copy()
        spawn_env.update(env or {})
        spawn_env["HOME"] = str(temp_home)
        spawn_env.pop("PYTHONPATH", None)  # avoid accidental venv confusion
        # Clear XDG vars so the spawned CLI uses ~/.muse (temp home)
        ...
        spawn_env["MUSE_SKIP_TUTORIAL"] = "1"
    ...
    def cleanup(self, result: SpawnResult) -> None:
        """Terminate the child, remove test-created files unless kept."""
        keep_home = os.getenv("MUSE_KEEP_TEMP_HOME") in {
            "1",
            "true",
            "TRUE",
            "True",
...
def spawned_cli(
    ...
    try:
        result.child.expect("What should we name the agent?", timeout=15)
        result.sendline("\r")
