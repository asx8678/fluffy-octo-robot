...
Drives a REAL ``MuseAgent`` through ``run`` with a pre-populated
...
def _fireworks_key_available() -> bool:
    """True if FIREWORKS_API_KEY is set via env OR in muse.cfg."""
...
pytestmark = pytest.mark.skipif(
    ...
    reason="FIREWORKS_API_KEY not set (env or muse.cfg); live compaction tests skipped.",
)
...
# MUSE_TEST_FAST=1 are set. Those gates exist for pexpect-harness tests;
...
def _build_huge_history(
    ...
    # run, but the accumulator may care). Ensure the last element is
...
@pytest.fixture
def pinned_code_muse_agent(monkeypatch):
    """Fresh MuseAgent pinned to firepass-kimi-k2p5-turbo.
    ...
    # `from code_muse.config import foo` captures a binding at import time, so
...
async def test_live_compaction_truncation_strategy(
    pinned_code_muse_agent, huge_history, monkeypatch
):
    ...
    result = await agent.run("What is 2+2? Reply with just the number, nothing else.")
...
@pytest.mark.asyncio
async def test_live_no_compaction_under_threshold(pinned_code_muse_agent, monkeypatch):
    ...
    from code_muse.agents._history import hash_message
...
async def test_live_orphan_tool_call_does_not_block_compaction(
    pinned_code_muse_agent, monkeypatch
):
    ...
    result = await agent.run("What is 5+5? Reply with just the number.")
    assert result is not None, "run returned None"
