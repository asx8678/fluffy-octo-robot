...
# and getting a 401 that we then swallow in run's broad except*.
_FAKE_CI_KEYS = {"fake-key-for-ci-testing", ""}


def _syn_key_available() -> bool:
    """True if SYN_API_KEY is set to a real-looking value (env OR muse.cfg)."""
...
pytestmark = pytest.mark.skipif(
    ...
    reason="SYN_API_KEY not set (env or muse.cfg); GLM-5.1 fallback test skipped.",
)
...
def glm51_agent(monkeypatch):
    """Fresh MuseAgent pinned to synthetic-GLM-5.1 (200k ctx)."""
    ...
    # `from code_muse.config import foo` captures bindings at import time, so
...
async def test_summarization_oversize_falls_back_to_truncation(
    ...
    from code_muse.agents import _runtime as _runtime_mod
    ...
    # -- Kick the run ---------------------------------------------------------
    result = await glm51_agent.run("hi")

    # -- Rate-limit guard ------------------------------------------------------
    # Live integration tests can hit 429s from the provider. If run
