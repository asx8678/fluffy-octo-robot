...
from code_muse import terminal_utils
...
class TestDetectTruecolorSupport:
    ...
    def test_rich_fallback_truecolor(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "code_muse.terminal_utils.Console", mock_console_cls, raising=False
        )
...
class TestPrintTruecolorWarning:
    ...
    def test_fallback_to_plain_print(self, monkeypatch, capsys):
        monkeypatch.setattr(terminal_utils, "detect_truecolor_support", lambda: False)
        # Make rich.console.Console raise ImportError on instantiation so the
        # fallback to plain print is exercised.  This works for both pure-Python
        # and Cython-compiled code (which bypasses __import__ monkeypatching).

        def broken_console(*a, **k):
            raise ImportError("no rich")

        monkeypatch.setattr("rich.console.Console", broken_console)
        terminal_utils.print_truecolor_warning(console=None)
        captured = capsys.readouterr()
        assert "WARNING: TERMINAL DOES NOT SUPPORT TRUECOLOR" in captured.out
