...
from code_muse.session_storage import SessionMetadata


@pytest.fixture
def mock_config_paths(monkeypatch):
    mock_home = "/mock_home"
    mock_config_dir = str(Path(mock_home) / ".muse")
    mock_config_file = Path(mock_config_dir) / "muse.cfg"
    mock_contexts_dir = Path(mock_config_dir) / "contexts"
    mock_autosave_dir = Path(mock_config_dir) / "autosaves"

    monkeypatch.setattr(_paths, "CONFIG_DIR", mock_config_dir)
    monkeypatch.setattr(_paths, "CONFIG_FILE", mock_config_file)
    monkeypatch.setattr(_paths, "CONTEXTS_DIR", mock_contexts_dir)
    monkeypatch.setattr(_paths, "AUTOSAVE_DIR", mock_autosave_dir)
...
class TestAutoSaveSessionFunctionality:
    @patch("code_muse.config.session.get_auto_save_session")
    ...
    @patch("code_muse.session_storage.save_session")
    @patch("code_muse.config.session.get_current_autosave_session_name")
    @patch("code_muse.config.session.datetime.datetime")
    @patch("code_muse.config.session.get_auto_save_session")
    ...
    def test_auto_save_session_if_enabled_success(
        ...
        _session._autosave_counter = 3  # bypass throttle
...
class TestFinalizeAutoSaveSession:
    @patch("code_muse.config.session.rotate_autosave_id", return_value="fresh_id")
    @patch("code_muse.config.session.auto_save_session_if_enabled", return_value=True)
    ...
    @patch("code_muse.config.session.rotate_autosave_id", return_value="fresh_id")
    @patch("code_muse.config.session.auto_save_session_if_enabled", return_value=False)
