"""Tests for code_muse.messaging.rich_renderer."""
...
from code_muse.messaging.bus import MessageBus
from code_muse.messaging.messages import (
...
@patch("code_muse.messaging.rich_renderer.is_subagent", return_value=False)
...
@patch("code_muse.messaging.rich_renderer.is_subagent", return_value=True)
@patch("code_muse.messaging.rich_renderer.get_subagent_verbose", return_value=False)
...
def test_get_banner_color(renderer):
    with patch("code_muse.config.get_banner_color", return_value="blue"):
...
def test_should_suppress_subagent_output(renderer):
    with patch("code_muse.messaging.rich_renderer.is_subagent", return_value=False):
        assert not renderer._should_suppress_subagent_output()
    with patch("code_muse.messaging.rich_renderer.is_subagent", return_value=True):
        with patch(
            "code_muse.messaging.rich_renderer.get_subagent_verbose",
            return_value=True,
        ):
            assert not renderer._should_suppress_subagent_output()
        with patch(
