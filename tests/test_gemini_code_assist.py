"""Tests for code_muse/gemini_code_assist.py - full coverage."""
...
from code_muse.gemini_code_assist import GeminiCodeAssistModel, StreamedResponse
...
class TestGeminiCodeAssistModel:
    ...
    def test_build_request_tool_return_json_error(self, model, default_params):
        ...
        with patch(
            "code_muse.gemini_code_assist.json.dumps", side_effect=TypeError("bad")
        ):
            body = model._build_request(msgs, None, default_params)
        fr = body["request"]["contents"][0]["parts"][0]["functionResponse"]
        # Falls back to str()
