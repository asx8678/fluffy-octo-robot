...
class TestEmitUcMessage:
    def test_emits(self):
        with patch("code_muse.tools.universal_constructor.get_message_bus") as mb:
...
class TestHandleListAction:
    ...
    async def test_list_empty(self):
        ...
        with (
            ...
            patch("code_muse.tools.universal_constructor.get_message_bus"),
        ):
...
class TestHandleCallAction:
    ...
    async def test_no_tool_name(self):
        with patch("code_muse.tools.universal_constructor.get_message_bus"):
    ...
    async def test_call_success(self):
        ...
        mock_tool = MagicMock()
        mock_tool.meta.enabled = True
        mock_tool.source_path = "/fake/tool.py"
        mock_tool.function_name = "x"
        mock_registry.get_tool.return_value = mock_tool
        mock_registry.get_tool_function.return_value = lambda: "result"
        mock_run_result = {"success": True, "result": "result", "execution_time": 0.1}
        with (
            patch(
                "code_muse.plugins.universal_constructor.registry.get_registry",
                return_value=mock_registry,
    ...
    async def test_call_json_string_args_parsed_and_forwarded(self):
        ...
        mock_run_result = {
            "success": True,
            "result": {"got": {"subject": "a knight", "pixel_grid": 64}},
            "execution_time": 0.1,
        }
        with (
            patch(
    ...
    async def test_call_type_error(self):
        ...
        mock_run_result = {
            "success": False,
            "error": "Invalid arguments: wrong args",
            "execution_time": 0.1,
        }
        with (
            patch(
...
class TestHandleUpdateAction:
    ...
    async def test_update_success(self, tmp_path):
        ...
        with (
            ...
            patch(
                "code_muse.plugins.universal_constructor.USER_UC_DIR",
                tmp_path,
            ),
        ):
            result = await universal_constructor_impl(
