...
class TestCallbacksExtended:
    """Test code_muse/callbacks.py callback system."""
    ...
    async def test_execute_callbacks_with_exception(self):
        ...
        with patch("code_muse.callbacks.logger") as mock_logger:
            results = await on_startup()
    ...
    def test_register_callback_with_priority(self):
        """Test callback registration with explicit priority."""
        order: list[str] = []
        ...
        register_callback("startup", low_priority, priority=1)
        register_callback("startup", high_priority, priority=10)

        callbacks = get_callbacks("startup")
        assert len(callbacks) == 2
        # Higher priority should come first
        assert callbacks[0] == high_priority
        assert callbacks[1] == low_priority

    def test_priority_sort_order(self):
        """Test that callbacks are executed in priority order (highest first)."""
        order: list[int] = []
        ...
        register_callback("startup", cb_10, priority=10)
        register_callback("startup", cb_50, priority=50)
        register_callback("startup", cb_30, priority=30)

        results = get_callbacks("startup")
        assert [r for r in results] == [cb_50, cb_30, cb_10]

    def test_same_priority_maintains_registration_order(self):
        """Test that equal-priority callbacks keep their registration order."""
        ...
        register_callback("startup", first, priority=5)
        register_callback("startup", second, priority=5)
        register_callback("startup", third, priority=5)

        callbacks = get_callbacks("startup")
        assert callbacks == [first, second, third]

    def test_duplicate_registration_with_different_priority(self):
        """Test that duplicate callback registration is still rejected even with different priority."""

        def test_callback():
            return "test"

        register_callback("startup", test_callback, priority=1)
        register_callback("startup", test_callback, priority=99)

        callbacks = get_callbacks("startup")
        assert len(callbacks) == 1
        # Original registration (priority=1) should win
        assert callbacks[0] == test_callback
