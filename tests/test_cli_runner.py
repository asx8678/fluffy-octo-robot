...
class TestModelValidation:
    ...
    @patch("code_muse.config.set_model_name")
...
class TestAgentValidation:
    ...
    @patch("code_muse.agents.agent_manager.set_current_agent")
...
class TestAPIKeyLoading:
    """Test API key loading from muse.cfg."""

    @patch("code_muse.config.load_api_keys_to_environment")
...
class TestMessageRendering:
    ...
    @patch("code_muse.messaging.get_global_queue")
    @patch("code_muse.messaging.get_message_bus")
    def test_queue_and_bus_initialized(self, mock_bus, mock_queue):
        """Test that both message bus and queue are initialized."""
        from code_muse.messaging import get_global_queue, get_message_bus
...
class TestTerminalReset:
    ...
    @patch("code_muse.terminal_utils.reset_windows_terminal_ansi")
    def test_windows_ansi_reset_function_exists(self, mock_reset_ansi):
        """Test that Windows ANSI reset function can be called."""
        from code_muse.terminal_utils import reset_windows_terminal_ansi
    ...
    def test_unix_terminal_reset_function_exists(self, mock_reset_unix):
        ...
        from code_muse.terminal_utils import reset_unix_terminal
...
class TestCancelAgentKeyValidation:
    ...
    @patch("code_muse.keymap.validate_cancel_agent_key")
    def test_cancel_key_validation_called(self, mock_validate):
        """Test that cancel key validation can be called."""
        from code_muse.keymap import validate_cancel_agent_key
...
class TestUVXDetection:
    ...
    def test_uvx_detection_can_be_imported(self):
        ...
        try:
            from code_muse import uvx_detection
...
class TestConfigInitialization:
    ...
    @patch("code_muse.config.ensure_config_exists")
    def test_ensure_config_exists_called(self, mock_ensure):
        """Test that ensure_config_exists is called."""
        from code_muse.config import ensure_config_exists

        ensure_config_exists()
        mock_ensure.assert_called_once()

    @patch("code_muse.config.initialize_command_history_file")
    def test_command_history_initialized(self, mock_init):
        """Test that command history file is initialized."""
        from code_muse.config import initialize_command_history_file
...
class TestPortAvailability:
    ...
    @patch("code_muse.http_utils.find_available_port")
...
class TestAgentRunning:
    ...
    @patch("code_muse.agents.agent_manager.get_current_agent")
