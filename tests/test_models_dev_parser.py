...
class TestModelsDevRegistryAPIFetching:
    ...
    @patch("code_muse.models_dev_parser.httpx.Client")
...
class TestModelsDevRegistryDataLoading:
    ...
    def test_load_data_top_level_not_object(self):
        ...
        with (
            ...
            patch("pathlib.Path.exists", return_value=True),
            pytest.raises(ValueError, match="Top-level JSON must be an object"),
        ):
            ModelsDevRegistry(json_path="/tmp/models.json")
...
class TestConvertToMuseConfig:
    """Tests for Muse configuration conversion."""
    ...
    def test_convert_basic(self, sample_data):
        """Test basic conversion to Muse config."""
        provider, model = sample_data
        config = convert_to_muse_config(model, provider)
...
class TestModelsDevRegistryUncoveredPaths:
    ...
    def test_load_data_bundled_json_decode_error(
        ...
        with patch("code_muse.models_dev_parser.httpx.Client") as mock_client_class:
            mock_client = MagicMock()
            mock_client_class.return_value.__enter__.return_value = mock_client
...
class TestBundledFileNotFound:
    ...
    def test_no_api_no_bundled_raises_file_not_found(self, mock_client_class):
        ...
        with (
            ...
            pytest.raises(FileNotFoundError, match="No data source available"),
        ):
            ModelsDevRegistry()
...
class TestMainBlock:
    ...
    @patch("code_muse.models_dev_parser.emit_error")
    @patch("code_muse.models_dev_parser.emit_info")
    @patch("code_muse.models_dev_parser.ModelsDevRegistry")
    def test_main_block_success(self, mock_registry_cls, mock_info, mock_error):
        ...
        with patch.dict("sys.modules", {}, clear=False):
            runpy.run_module("code_muse.models_dev_parser", run_name="__main__")
    ...
    def test_main_block_file_not_found(self, mock_error):
        ...
        with patch(
            "code_muse.models_dev_parser.ModelsDevRegistry",
