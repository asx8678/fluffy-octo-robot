...
import code_muse.status_display
from code_muse.status_display import CURRENT_TOKEN_RATE, StatusDisplay
...
class TestStatusDisplay:
    ...
    def test_calculate_rate_with_previous_data(self, status_display):
        ...
        assert rate == code_muse.status_display.CURRENT_TOKEN_RATE
    ...
    def test_calculate_rate_negative_rates_handled(self, status_display):
        ...
        rate = status_display._calculate_rate()
        assert rate >= 0
        assert code_muse.status_display.CURRENT_TOKEN_RATE >= 0
    ...
    def test_update_rate_from_sse(self, status_display):
        ...
        assert code_muse.status_display.CURRENT_TOKEN_RATE == 50
    ...
    def test_get_current_rate_static(self):
        ...
        try:
            code_muse.status_display.CURRENT_TOKEN_RATE = 42.0
            assert StatusDisplay.get_current_rate() == 42.0
        finally:
            code_muse.status_display.CURRENT_TOKEN_RATE = original_rate
    ...
    def test_status_message_rotation(self, status_display):
        ...
        for _i in range(messages_count + 2):
            text = status_display._get_status_text()
    ...
    async def test_start(self, status_display):
        ...
        with patch("code_muse.status_display.asyncio.create_task") as mock_create_task:
            mock_task = MagicMock()
    ...
    async def test_stop_after_start(self, status_display):
        ...
        with patch("code_muse.status_display.asyncio.create_task") as mock_create_task:
            ...
            with patch("code_muse.messaging.emit_info") as mock_emit_info:
                status_display.start()
                status_display.stop()
                ...
                assert status_display.start_time is None
                assert status_display.token_count == 0
                assert code_muse.status_display.CURRENT_TOKEN_RATE == 0.0
    ...
    async def test_update_display_integration(self, status_display):
        ...
        with (
            patch("code_muse.status_display.asyncio.create_task"),
            patch("code_muse.status_display.Live") as mock_live,
            patch("code_muse.messaging.emit_info") as mock_emit_info,
        ):
    ...
    def test_loading_messages_content(self):
        ...
        # Should be muse-themed
        muse_terms = ["muse", "paws", "tail", "barking", "panting"]
        combined_text = " ".join(messages).lower()
        has_muse_theme = any(term in combined_text for term in muse_terms)
        assert has_muse_theme
    ...
    def test_panel_styling(self, status_display):
        ...
        assert panel.title == "[bold blue]Muse Status[/bold blue]"
